...
"""
    ...
    def _recorded_model_id(self) -> Optional[str]:
        """The model this turn's requests actually go to.

        ``self.model_id`` keeps the configured local id even under
        ``--use-claude``, where the SDK substitutes the Claude model at send
        time — so a record built from it would name a model the turn never
        touched.
        """
        chat = getattr(self, "chat", None)
        effective = getattr(chat, "effective_model", None) if chat else None
        return effective or getattr(self, "model_id", None)
    ...
    def _publish_turn_metrics(self, record: Optional[Dict[str, Any]]) -> None:
        ...
        Optional hook: a console that does not define it (a third-party one
        not deriving from ``OutputHandler``) simply does not receive metrics.
        """
        if not record:
            return
    ...
    def _execute_tool(self, tool_name: str, tool_args: Dict[str, Any]) -> Any:
        """Execute a tool, timing it for the turn record when one is active.

        Wraps rather than inlines: the impl returns from a dozen places, and a
        total that missed the refusal paths would read as agent overhead.
        """
        recorder = getattr(self, "_turn_recorder", None)
        # Only the outermost call is timed. A tool body may call another tool
        # (CodeAgent's orchestration does), and timing both would count the
        # inner one's seconds twice, pushing tool_s past the turn's own total.
        if recorder is None or getattr(self, "_tool_timing_depth", 0):
            return self._execute_tool_impl(tool_name, tool_args)
        ...
        try:
            result = self._execute_tool_impl(tool_name, tool_args)
    ...
    def _execute_tool_impl(self, tool_name: str, tool_args: Dict[str, Any]) -> Any:
        """
    ...
    def _process_query_impl(
        ...
        # set). Its total is the latency a user feels, so it is sealed at the
        # answer rather than at function exit.
