...
def _tar_entries(handle: tarfile.TarFile, dest: Path, allow_links: bool) -> list:
    ...
    for member in handle.getmembers():
        ...
        if member.isdir():
            ...
            entries.append(
                _Entry(member.name, target, "file", member, mode=member.mode)
            )
...
def _zip_entries(handle: zipfile.ZipFile, dest: Path, allow_links: bool) -> list:
    ...
    for info in handle.infolist():
        ...
        if info.is_dir() or file_type == stat.S_IFDIR:
            ...
            entries.append(_Entry(info.filename, target, "file", info))
        else:
            raise _refuse_special(info.filename)
    return entries


def _check_links(entries: list, dest: Path) -> None:
    for entry in entries:
        if entry.kind in ("symlink", "hardlink"):
            _link_pointee(entry, dest)


def _write_file(source, entry: _Entry) -> None:
    ...
    *,
    ...
    if kind == "zip":
        with zipfile.ZipFile(archive_path) as handle:
            ...
            _check_links(entries, root)
            for entry in entries:
                if entry.kind == "dir":
                    entry.target.mkdir(parents=True, exist_ok=True)
                elif entry.kind == "file":
                    _write_file(handle.open(entry.member), entry)
    elif kind == "tar":
