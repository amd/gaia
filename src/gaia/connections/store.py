...
def verify_keyring_backend() -> None:
    """
    Raise ``ConnectionsError`` if the active keyring is one of the refused
...
def _wrap_keyring_call(operation: str):
    ...
    ``ConnectionsError`` with actionable text per CLAUDE.md."""
    ...
    *,
    ...
    try:
        ...
        raise ConnectionsError(
            ...
            f"or `gaia connections connect {provider}`."
        ) from e
