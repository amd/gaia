...
"""
Provider abstraction for ``gaia.connections``.

Defines:
- ``ConnectionRequirement``: declared on agent classes via the
  ``REQUIRED_CONNECTIONS`` ClassVar; surfaced to AgentUI's consent dialog and
...
class ConnectionRequirement:
    """
    Declared on agent classes as ``REQUIRED_CONNECTIONS = [ConnectionRequirement(...)]``.
