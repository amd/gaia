...
"""
Agent-identity context propagation for ``gaia.connections``.
...
- ``_agent_context(agent_id)`` — **PRIVATE**. Only the agent runtime calls
  ...
  the public ``gaia.connections`` API surface, so it cannot forge an agent
...
def _agent_context(agent_id: str) -> Iterator[None]:
    ...
    ``from gaia.connections.context import _agent_context``. The connections
    public API (``gaia.connections.__init__``) does NOT re-export this name,
