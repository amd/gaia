...
"""
Per-agent grants ledger at ``~/.gaia/connections/grants.json``.
...
GRANTS_FILE = Path.home() / ".gaia" / "connections" / "grants.json"
...
def _grants_path() -> Path:
    ...
    return Path.home() / ".gaia" / "connections" / "grants.json"
...
def load_grants() -> Dict[str, Dict[str, List[str]]]:
    ...
    A corrupted file raises ``ConnectionsError`` with the path and the
    ...
    try:
        ...
        raise ConnectionsError(
            ...
            "or via `gaia connections grants grant ...`."
        ) from e
    except OSError as e:
        raise ConnectionsError(
