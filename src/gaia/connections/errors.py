...
"""
Exception hierarchy for ``gaia.connections``.
...
class ConnectionsError(Exception):
    """Base class for every error raised by ``gaia.connections``."""


class ConfigurationError(ConnectionsError):
    """Required configuration (env var, runbook entry) is missing."""


class AuthRequiredError(ConnectionsError):
    """
    ...
    def _default_message(self) -> str:
        ...
        if self.reason is AuthRequiredError.Reason.NOT_CONNECTED:
            return (
                ...
                "AgentUI, or run `gaia connections connect "
        ...
        if self.reason is AuthRequiredError.Reason.AGENT_NOT_GRANTED:
            ...
            return (
                ...
                f"`gaia connections grants grant {self.provider or '<provider>'} "
        ...
        if self.reason is AuthRequiredError.Reason.CONNECTION_MISSING_SCOPES:
            ...
            return (
                ...
                f"or run `gaia connections connect {self.provider or '<provider>'} "
                "--scopes <scope> ...`. "
                "See docs/sdk/infrastructure/connections.mdx."
            )
        if self.reason is AuthRequiredError.Reason.REAUTH_REQUIRED:
            return (
                ...
                f"Connections, or run `gaia connections connect "
                f"{self.provider or '<provider>'}`. "
...
class ConnectionRevokedError(ConnectionsError):
    ...
    def __init__(self, provider: str, *, message: str | None = None):
        ...
        super().__init__(
            ...
            or (
                ...
                f"Settings → Connections, or run `gaia connections connect "
                f"{provider}`. See docs/security/connections.mdx."
            )
        )


class ScopeMismatchError(ConnectionsError):
...
class ConsentDeniedError(ConnectionsError):
    """User denied consent in OAuth flow (``?error=access_denied``)."""


class FlowTimeoutError(ConnectionsError):
    """OAuth flow exceeded its 120-second callback timeout."""


class FlowInProgressError(ConnectionsError):
