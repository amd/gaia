...
"""
CLI for ``gaia connections {connect|status|disconnect|grants ...}``.

Each subcommand is a thin wrapper that calls into ``gaia.connections.api``.
...
``gaia connections ...`` from the top-level CLI dispatcher.
"""
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Register ``gaia connections`` and its subcommands."""
    p = subparsers.add_parser(
        "connections",
        help="Manage OAuth connections (Google, …) and per-agent grants",
        description=(
            "Manage OAuth connections (Google, future Microsoft) and "
            "per-agent grants. Connect once, then grant individual "
            "agents the scopes they need."
        ),
    )
    sub = p.add_subparsers(
        dest="connections_action",
        metavar="<subcommand>",
        help="Subcommand",
    )
...
def handle(args: argparse.Namespace) -> int:
    """Dispatch a parsed ``gaia connections ...`` command. Returns exit code."""
    action = getattr(args, "connections_action", None)
    if action is None:
        sys.stderr.write(
            "gaia connections: missing subcommand. " "Try 'gaia connections --help'.\n"
        )
        return 2
    ...
    sys.stderr.write(f"gaia connections: unknown subcommand {action!r}\n")
    return 2
...
def _handle_grants(args: argparse.Namespace) -> int:
    ...
    sys.stderr.write(
        "gaia connections grants: missing subcommand. "
        "Try 'gaia connections grants --help'.\n"
    )
    return 2


def main(argv: Sequence[str] | None = None) -> int:
    """Standalone entry point — useful for ``python -m gaia.connections.cli``
