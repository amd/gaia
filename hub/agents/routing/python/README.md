...
pip install -e hub/agents/routing/python    # editable, for development
uv pip install "gaia-agent-routing @ git+https://github.com/amd/gaia.git#subdirectory=hub/agents/routing/python"  # works today without a repo checkout
```
...
```bash
pip install -e ".[test]"
pytest hub/agents/routing/python/tests/ -x
```
