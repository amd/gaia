...
| `publish_to_r2.py` | **The release upload path.** POSTs each artifact + `gaia-agent.yaml` to the Agent Hub Worker's `POST /publish` (`release_agent_email.yml`), which stores immutably under `agents/email/<ver>/` and rebuilds `index.json`. Per-platform binaries go as buffered multipart (checksummed server-side); the large whole-package `.zip` is **streamed** as raw `application/octet-stream` with R2 verifying the client SHA-256, so the 177 MB body never OOMs the Worker ([#1848](https://github.com/amd/gaia/issues/1848)). |
