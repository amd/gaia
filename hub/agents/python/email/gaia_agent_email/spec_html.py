...
def render_endpoint_spec_html() -> str:
    ...
    init_block = (
        ...
        f"probes the local Lemonade Server and confirms the triage model is "
        f"downloaded — so a host can verify &ldquo;ready to triage,&rdquo; not "
        f"just &ldquo;process up.&rdquo; Read-only: probes only, no model pull.</p>"
