...
def _build_tree(root: Path, version: str, *, include_optional: bool = True) -> None:
    ...
    email = root / "hub" / "agents" / "python" / "email"
    npm = root / "hub" / "agents" / "npm" / "agent-email"
...
def tree(tmp_path, monkeypatch):
    ...
    email = tmp_path / "hub" / "agents" / "python" / "email"
    npm = tmp_path / "hub" / "agents" / "npm" / "agent-email"
...
def test_api_version_is_not_touched(tree):
    ...
    text = (tree / "hub/agents/python/email/gaia_agent_email/version.py").read_text()
...
@pytest.mark.parametrize(
    "rel,marker,bad",
    [
        ("hub/agents/python/email/gaia-agent.yaml", "version: 1.2.3", "version: 9.9.9"),
        (
            "hub/agents/python/email/pyproject.toml",
            'version = "1.2.3"',
            'version = "9.9.9"',
        ),
        (
...
def test_stamp_syncs_every_target(tree):
    ...
    vpy = tree / "hub/agents/python/email/gaia_agent_email/version.py"
    ...
    assert stamp.main(["--check"]) == 0
    lock = json.loads(
        (tree / "hub/agents/npm/agent-email/binaries.lock.json").read_text()
    )
...
def test_stamp_is_idempotent_and_minimal(tree):
    lock_path = tree / "hub/agents/npm/agent-email/binaries.lock.json"
