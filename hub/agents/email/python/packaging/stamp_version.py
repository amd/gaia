...
Usage:
  python hub/agents/email/python/packaging/stamp_version.py
      # read AGENT_VERSION from version.py and stamp every present target to match

  python hub/agents/email/python/packaging/stamp_version.py --check
...
# packaging/ -> python/ : the email package root holds every Python-side target;
...
)  # hub/agents/email/python -> repo root
NPM_ROOT = REPO_ROOT / "hub" / "agents" / "email" / "npm"
