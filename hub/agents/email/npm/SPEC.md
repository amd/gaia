...
- **GAIA splits Microsoft into two connectors** — `microsoft` (personal,
  ...
  whichever connector is connected, so the work path resolves automatically; you
  only need `GAIA_EMAIL_ACCOUNT_TYPE=work` to override the default for a Gmail
  mailbox (Gmail has no `tid` claim to derive from).
