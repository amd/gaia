...
# ---------------------------------------------------------------------------
# Full access never reaches this transport (#3373)
#
# Full access lifts the shell guardrails as well as the confirmation prompt, so an
...
def test_the_request_body_cannot_ask_for_full_access(built):
    """extra='forbid' is what makes this unreachable; pin it, so adding a
    full-access field fails here instead of shipping."""
...
def test_the_http_transport_exposes_no_full_access_control():
    """The stdio control channel carries a full_access verb; HTTP has no equivalent
    ...
    assert not [p for p in paths if "bypass" in p.lower() or "full" in p.lower()]
