...
6 exposed ops (6 REST functional, no MCP surface) and their eval coverage:
...
- Registered agent-loop tools: **83** (CORE 15 + 21 bundles; bundles overlap CORE and each other by design, so per-bundle counts sum past the unique total)
  - `clipboard`: 2
  - `code_index`: 4
  - `data`: 6
  - `desktop`: 3
...
- Skill-library tools (framework `SKILL_LIBRARY_TOOL_NAMES`, all in the union): **8**
  ...
  - `skill_status`
  - `unload_skill`
- REST functional verbs: **6** (10 total operations in the sidecar contract, including the health/version/init probes)
  - `memory`
  - `query`
  - `query/{run_id}/cancel`
