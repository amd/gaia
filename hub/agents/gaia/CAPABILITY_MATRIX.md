...
5 exposed ops (5 REST functional, no MCP surface) and their eval coverage:
...
| `query/{run_id}/confirm` | REST | no quality eval (contract-tested only) |
...
- Skill-library tools (framework `SKILL_LIBRARY_TOOL_NAMES`, all in the union): **8**
  ...
  - `skill_status`
  - `unload_skill`
- REST functional verbs: **5** (9 total operations in the sidecar contract, including the health/version/init probes)
  - `memory`
  - `query`
  - `query/{run_id}/cancel`
