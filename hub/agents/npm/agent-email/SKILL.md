...
- **`health()` is liveness-only.** A green `/health` means the REST surface is up,
  ...
  /v1/email/init`, #1795) — it probes Lemonade + the triage model and returns
  `{ ready, hint }`. The wrapper parses both the 200 (ready) and 503 (not-ready)
  responses into a value, so branch on `.ready` / read `.hint` rather than catching
  an error. `POST /v1/email/init` streams a model-pull.
...
- **Some capabilities are agent-loop-only — no REST endpoint, no client method.**
  ...
  follow-up method — they don't exist (it does not move `SCHEMA_VERSION`).
