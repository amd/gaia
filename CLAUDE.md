...
│   │   └── registry.py # Python custom-agent registry + KNOWN_TOOLS map
...
New agents are registered as Python `agent.py` files (subclasses of `Agent` with `AGENT_ID` / `AGENT_NAME` class attributes) discovered by [`src/gaia/agents/registry.py`](src/gaia/agents/registry.py). The registry exposes `KNOWN_TOOLS` — a curated map of reusable tool mixins consumed by the BuilderAgent template ([`src/gaia/agents/builder/template.py`](src/gaia/agents/builder/template.py)) when scaffolding a new `agent.py`:
...
When adding a new tool, register it in `KNOWN_TOOLS` so the BuilderAgent template scaffolds the right import and base class for it.
...
### Development
- **gaia-agent-builder** — Creating a new GAIA agent (Python `agent.py`). Not for tuning an existing agent's prompt or adding a single tool.
