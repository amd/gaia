...
### Default Models
- The default chat model follows the hardware. `gaia init` picks `user.Qwen3.8-Flash-Next-GGUF` (`LARGE_DEFAULT_MODEL_NAME`) where it fits — a 128 GB Strix Halo — and records it as `default_model` in `~/.gaia/config.json`; every other PC gets `Gemma-4-E4B-it-GGUF` (`DEFAULT_MODEL_NAME`). "Fits" is [`src/gaia/llm/model_fit.py`](src/gaia/llm/model_fit.py), read off Lemonade's `/system-info`; the TUI picker applies the same rule from `tui/internal/lemonade/recommended_models.json` (a drift test pins the two).
