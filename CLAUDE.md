...
- `weekly-audit-patterns` — the proactive **nightly** Claude audit (`.github/workflows/claude-weekly-audit.yml`): the day-of-week mode selector, the stable dedup-key scheme, the four dimensions (and which one owns the Fail-Loudly check), why findings are filtered in synthesis rather than in the lenses, and the `bug`→`auto-fix` promotion path. Security is a separate nightly workflow (`claude-security-audit.yml`) with its own private code-scanning channel. Read before editing either workflow, changing their cadence, or how findings are filed/deduped.
