...
The default construction registers **69 tools** — the 55 of the Chat agent's `full` profile, plus 7 skill-library tools, 4 code-search tools, and the `load_tools` escape hatch. Grouped by what they're for:
...
How long an allowed command may run depends on what it is — a test run gets 15 minutes, a build or install 30, a network call 5, and everything else the usual 30 seconds. These limits do not grant execution permission: for example, running `pytest` requires a loaded skill with a `shell:execute:pytest` grant, and ungranted installers remain refused. `wait_for_condition` covers the other half of waiting: give it a command that succeeds once the thing you are waiting for is ready (a file written, a server answering) and the agent waits in one step instead of re-checking in a loop. Stop terminates a running command's process tree instead of waiting for its full timeout.
