...
The default construction registers **73 tools** — the 55 of the Chat agent's `full` profile, plus 7 skill-library tools, 4 code-search tools, 5 read-only Outlook tools, `remember_skill_lesson`, and the `load_tools` escape hatch. Grouped by what they're for:
