...
- 🪟 **Windows CUA demo** — runs a computer-use agent via the Windows MCP server
...
<Steps>
  ...
  <Step title="Run the Windows CUA demo">
    ```bat
...
4. Agent executes each tool through the MCP client via JSON-RPC 2.0 over stdio
5. Tool results feed back to the LLM until a final answer is produced
