...
<Note>
  The command does not detach on its own — it holds the terminal for as long as the adapter polls, so that `gaia telegram stop` has a live process to signal. Add `&` or run it under `nohup`/`systemd` to get your prompt back.
</Note>
...
The adapter deletes `~/.gaia/telegram.pid` when it exits. `stop` signals the recorded PID only after confirming it is still a `gaia telegram start` process; a leftover PID file that names any other process is removed without signalling it.
