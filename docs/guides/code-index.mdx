...
Custom Python agents compose `CodeIndexToolsMixin` directly (see the [Custom Agents guide](/guides/custom-agent)); `code_index` is also registered in `KNOWN_TOOLS` ([`src/gaia/agents/registry.py`](https://github.com/amd/gaia/blob/main/src/gaia/agents/registry.py)) so the BuilderAgent template can scaffold imports for new agents that opt into it. For SDK-level composition see the [SDK reference](/sdk/sdks/code-index).
