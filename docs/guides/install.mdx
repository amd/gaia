...
GAIA follows the platform convention: removing the app leaves your user data by default (chats, documents, downloaded models) behind. To remove the data too, run the `gaia uninstall` CLI from a terminal — it has tiered flags so you can decide exactly what to keep.
