...
It activates only on the `doc` profile — the SDK with
`ChatAgentConfig(prompt_profile="doc")`, or `gaia eval agent --agent-type doc`,
an explicit override since the eval otherwise scores the flagship `gaia` agent.
The `doc` id is no longer offered in the agent picker, but it still resolves, so
the eval command is unchanged.
