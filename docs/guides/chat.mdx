...
<Note>
  **PDF Indexing Requirements:** Processing PDFs with images requires a Vision Language Model (VLM). GAIA uses `Qwen2.5-VL-7B-Instruct-GGUF` by default for extracting text from images in PDFs.
