...
@dataclass
class ChatConfig:
    model: str = "Qwen3-Coder-30B-A3B-Instruct-GGUF"  # Recommended model for chat
    max_tokens: int = 512
...
<Note>
  **Document Q&A:** To work with documents, see the [Document Q&A (RAG)](#document-qa-rag) section below for RAG-specific commands.
</Note>
...
<Note>
  **PDF Indexing Requirements:** Processing PDFs with images requires a Vision Language Model (VLM). GAIA uses `Qwen3-VL-4B-Instruct-GGUF` by default for extracting text from images in PDFs. The model will be automatically downloaded on first use if not already installed (requires [Lemonade Server](https://amd-gaia.ai/setup#install-lemonade-server)).
</Note>
...
<AccordionGroup>
  <Accordion title="Session Management">
    ...
    - `/sessions` - List all saved sessions
    - `/reset` - Clear conversation and remove all indexed documents
  </Accordion>
