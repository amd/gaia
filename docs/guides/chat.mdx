...
<AccordionGroup>
  ...
  <Accordion title="Session Management">
    - `/resume [id]` - Resume a previous session (or list if no id provided)
  ...
  <Accordion title="Document Management">
    - `/index <path>` - Index a document or directory for RAG
    - `/watch <dir>` - Watch a directory for changes and auto-index
    - `/list` - List all indexed documents
    - `/status` - Show system status (RAG, sessions, etc.)
  </Accordion>

  <Accordion title="Debug & Observability">
    - `/chunks [file]` - View indexed chunks for a file
    ...
    - `/clear-cache` - Clear RAG cache (force re-indexing)
    - `/search-debug` - Enable search debugging output
  </Accordion>
</AccordionGroup>
...
<Note>
  **PDF Indexing Requirements:** Processing PDFs with images requires a Vision Language Model (VLM). GAIA uses `Qwen3-VL-4B-Instruct-GGUF` by default for extracting text from images in PDFs. Ensure the VLM model is installed via [Lemonade Server](https://amd-gaia.ai/setup#install-lemonade-server) before indexing PDFs with images.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Voice Mode: No Module Named 'pip'">
    ...
    **Root Cause:** Some virtual environments don't include pip by default. The `gaia talk` command attempts to install missing dependencies automatically, but this requires pip to be available.
  </Accordion>

  <Accordion title="PDF Issues">
    - Ensure PDF has extractable text (not scanned images, unless VLM is available)
    - Check file is not password-protected
    - Verify file is not corrupted
    - If you see "tuple index out of range" errors, the PDF may have complex formatting or embedded images that require VLM processing
  </Accordion>
