...
The `gaia sd` command shows GAIA's agent capabilities through a practical example: an LLM analyzes your simple prompt, adds helpful keywords (lighting, style, quality), then generates an image with Stable Diffusion.

**Key features:**
- **100% Local** - Everything runs on Ryzen AI (LLM + Stable Diffusion)
- **Easy to use** - Just type what you want, the agent adds technical details
- **Demonstrates SDToolsMixin** - Shows how to build agents with image generation tools

**Example:** Type "robot kitten" → Agent adds "glowing LED eyes, soft lighting, detailed metallic texture, 4K" → Better result
