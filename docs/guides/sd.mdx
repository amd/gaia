...
<Info>
  ...
  **SDK Reference:** [SDToolsMixin API](/sdk/mixins/tool-mixins#95-stable-diffusion-mixin) • [VLMToolsMixin API](/sdk/mixins/tool-mixins#96-vision-language-model-mixin)
</Info>
...
class SDAgent(Agent, SDToolsMixin, VLMToolsMixin):
    def __init__(self, config):
        ...
        super().__init__(
            model_id=config.model_id,  # Qwen3-4B for prompt enhancement
...
3. **Reflection Phase** - LLM examines results
   ...
   - Can replan if errors occur (max 5 steps by default)
...
|-------|------|--------------|-----------|
| **Qwen3-4B** (LLM) | Orchestration & Enhancement | 8K | Every request - plans workflow, enhances prompts |
