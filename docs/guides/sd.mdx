...
description: "Generate images using gaia sd - an agent that enhances prompts with an LLM"
---

<Info>
  **CLI Command:** `gaia sd` - Demonstrates SDToolsMixin with LLM-powered prompt enhancement
</Info>

# Multi-Modal Image Generation

The `gaia sd` command demonstrates GAIA's multi-modal capabilities by combining three AI models:

1. **LLM** (Qwen3-4B) - Enhances your prompt with quality keywords
2. **Stable Diffusion** - Generates the image locally on Ryzen AI
3. **VLM** (Qwen3-VL-4B) - Analyzes the image and creates a creative story

**Key features:**
- **100% Local** - All three models run on Ryzen AI (no cloud)
- **Multi-modal** - Text → Image → Vision → Story
- **Easy** - Type one sentence, get image + narrative
- **Demonstrates mixins** - Shows SDToolsMixin + VLMToolsMixin integration

**Complete flow:**
```
You: "robot kitten"
↓
LLM: Adds "glowing LED eyes, soft lighting, detailed, 4K"
↓
SD: Generates image
↓
VLM: "A cute mechanical kitten with silver body and cyan glowing eyes..."
↓
VLM: Story about Whiskers the robot kitten's first adventure
↓
Result: Image + creative 2-3 paragraph story
```
...
See the complete AI workflow: LLM enhances your prompt → SD generates the image → VLM analyzes it → VLM creates a creative story. All running locally on Ryzen AI in ~20 seconds total.
...
graph LR
    A["You Type:<br/><b>'robot kitten'</b>"] --> B["🧠 Agent<br/><small>Analyzes</small>"]

    B --> C["✨ Enhances<br/><small>Adds Keywords</small>"]

    C --> D["Enhanced:<br/><i>'robotic kitten,<br/>LED eyes,<br/>soft lighting,<br/>detailed'</i>"]

    D --> E["🖼️ SD<br/><small>Ryzen AI</small>"]
...
|-------|-------|---------|-------------------|
| **SDXL-Turbo** (default) | ~17s | ⭐⭐⭐ | Adds artistic style, detailed lighting scenarios |
| **SD-Turbo** | ~13s | ⭐⭐ | Keeps it concise, focuses on key elements |
| **SD-1.5** | ~88s | ⭐⭐⭐ | Balanced keyword approach |
| **SDXL-Base-1.0** | ~9min | ⭐⭐⭐⭐⭐ | Adds camera settings, photorealistic focus |
...
See the agent enhance your prompt automatically! 🐱✨
