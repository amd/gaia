...
<Steps>
  ...
  <Step title="Pull models (first time)">
    ```bash
    lemonade-server pull SDXL-Turbo  # Image generation
    lemonade-server pull Qwen3-4B-GGUF  # Prompt enhancement
    lemonade-server pull Qwen3-VL-4B-Instruct-GGUF  # Vision + stories
    ```
  </Step>
  ...
  <Step title="Generate image with story">
    ...
    The agent will:
    1. Enhance your prompt with quality keywords
    2. Generate the image (~17s)
    3. Analyze the image with VLM
    4. Create a creative story (~17s)
...
graph LR
    A["You Type:<br/><b>'robot kitten'</b>"] --> B["🧠 LLM<br/><small>Enhances</small>"]

    B --> C["Enhanced:<br/><i>'glowing eyes,<br/>soft lighting,<br/>detailed, 4K'</i>"]

    C --> D["🖼️ SD<br/><small>Generates</small>"]

    D --> E["Image"]

    E --> F["👁️ VLM<br/><small>Analyzes</small>"]

    F --> G["📄 Description"]

    G --> H["📖 VLM<br/><small>Creates</small>"]
    ...
    style D fill:#C8171E,stroke:#C8171E,stroke-width:3px,color:#fff
    style E fill:#ED1C24,stroke:#C8171E,stroke-width:3px,color:#fff
    style F fill:#F4484D,stroke:#ED1C24,stroke-width:3px,color:#fff
    style G fill:#6c757d,stroke:#495057,stroke-width:2px,color:#fff
    style H fill:#F4484D,stroke:#ED1C24,stroke-width:3px,color:#fff
    style I fill:#ED1C24,stroke:#C8171E,stroke-width:3px,color:#fff

    linkStyle 0,1,2,3,4,5,6,7 stroke:#ED1C24,stroke-width:3px
```

**The pipeline:**
1. **LLM analyzes** input → adds lighting, style, quality keywords
2. **SD generates** image with enhanced prompt
3. **VLM analyzes** the generated image in detail
4. **VLM creates** a creative 2-3 paragraph story

Everything runs locally on Ryzen AI hardware.

---

## Example Output

What you see when you run `gaia sd "cute robot kitten"`:
...
gaia sd "cute robot kitten with a story"
...
```bash
gaia sd "create a cute robot kitten and tell me a story about it"
```
