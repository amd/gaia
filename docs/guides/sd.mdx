...
The `gaia sd` command demonstrates GAIA's multi-modal agent capabilities: an LLM enhances your prompt, Stable Diffusion generates the image, then a vision model (VLM) analyzes it and creates a creative story.

**Key features:**
- **100% Local** - LLM, SD, and VLM all run on Ryzen AI
- **Multi-modal pipeline** - Text (LLM) → Image (SD) → Vision analysis (VLM) → Story (VLM)
- **Easy to use** - Just type a simple description, get image + story
- **Demonstrates SDToolsMixin** - Shows how to build multi-modal agents

**Example:**
- You type: "robot kitten"
- LLM adds: "glowing LED eyes, soft lighting, detailed metallic texture, 4K"
- SD generates: Image
- VLM analyzes: "A cute mechanical kitten with silver body..."
- VLM creates: Story about Whiskers the robot kitten
- Result: Image + creative narrative
