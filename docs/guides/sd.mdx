...
# Photorealistic with auto-settings (uses SDXL-Base-1.0 defaults)
result = mixin._generate_image(
    prompt="a mountain landscape, photorealistic, natural lighting, 4k"
)
print(f"Image saved to: {result['image_path']}")

# Fast generation with specific model
result = mixin._generate_image(
    prompt="a cyberpunk city at night, neon lights, stylized",
    model="SDXL-Turbo"
)
```
...
```python
# Photorealistic with auto-settings (recommended)
result = agent._generate_image(
    prompt="a dragon on a cliff at sunset, photorealistic, detailed",
    model="SDXL-Base-1.0"  # Auto: 1024x1024, 20 steps, CFG 7.5
)
...
<Note>
**Recommended:** Use `SDXL-Base-1.0` for photorealistic images (default). SDXL-Turbo produces stylized/artistic results.
</Note>

<Note>
**Performance:** SDXL-Base-1.0 at 1024x1024 takes ~8-10 minutes per image (20 steps). Use SDXL-Turbo for faster results (~15-60 seconds).
</Note>
