...
<Steps>
  <Step title="Initialize GAIA (first time only)">
    ...
    This installs Lemonade Server and downloads required models.

    Or if you already have GAIA installed:
    ```bash
    # Pull SD model if not already downloaded
  ...
  <Step title="Generate image with story">
    ...
    The agent will:
    1. Load models automatically (Qwen3-4B for enhancement, Qwen3-VL-4B for vision)
    2. Enhance your prompt with quality keywords
    3. Generate the image (~17s)
    4. Analyze the image with VLM
    5. Create a creative story (~17s)
...
```mermaid
%%{init: {'theme':'base', 'themeVariables': { 'primaryColor':'#ED1C24', 'fontSize':'16px'}}}%%
graph LR
    A["'robot kitten'"] --> B["🧠 LLM"]
    B --> C["🖼️ Image"]
    C --> D["👁️ VLM"]
    D --> E["📖 Story"]
...
**Simple flow:**
1. **LLM** adds quality keywords
2. **SD** generates image (~17s)
3. **VLM** analyzes what's in the image
4. **VLM** creates a creative story (~17s)

All running locally on Ryzen AI.
