...
<Tip>
  **New to building agents?** Start with [Custom Agents](/sdk/core/agent-system) to
...
bundling a copy of its own — see [TUI architecture](/reference/tui)
...
| | Agent package | Skill |
|---|---|---|
| Manifest | `gaia-agent.yaml` | the YAML front matter of `SKILL.md` ([Skill Format](/spec/agent-skills)) |
...
- [Build a custom agent](/sdk/core/agent-system) — focuses on the agent class itself.
- [Agent System](/sdk/core/agent-system) and [Tools](/sdk/core/tools) — the building blocks.
- [Plugin registry](/spec/plugin-registry) — how installed agents are discovered.
