...
([#2467](https://github.com/amd/gaia/issues/2467)). This whole guide is about
agents; if what you have is a `SKILL.md` folder, the differences are:
...
- **`community` and `verified` need a cleared security audit.** The publish call
  refuses those tiers unless a passing audit report rides along
  ([#2468](https://github.com/amd/gaia/issues/2468)); an `experimental` skill may
  publish without one and is recorded as `unaudited` rather than stamped as
  passing.
