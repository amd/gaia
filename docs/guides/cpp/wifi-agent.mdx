...
<Info>
  **Source Code:** [`cpp/examples/wifi_agent.cpp`](https://github.com/amd/gaia/blob/main/cpp/examples/wifi_agent.cpp) — single-file, self-contained agent (~320 lines).
</Info>
...
The entire agent is a single `.cpp` file with three sections. Let's walk through each one.
...
The agent registers 9 tools in two categories:
...
| `restart_wifi_adapter` | `Disable-NetAdapter` + `Enable-NetAdapter` | `adapter_name` | Adapter stuck in bad state |
...
flowchart TD
    ...
    A -->|No / Disconnected| F1["FIX: restart_wifi_adapter"]
...
public:
    WiFiTroubleshooterAgent() : Agent(makeConfig()) {
...
int main() {
    try {
        std::cout << "\n=== Wi-Fi Troubleshooter | GAIA C++ | Local inference ==="
                  << std::endl;

        WiFiTroubleshooterAgent agent;  // constructor registers tools + connects to LLM
...
Try:
  ...
  'My Wi-Fi is connected but I can't browse the web.'
  'Check if DNS is working and fix it if not.'

You: Run a full network diagnostic.

  [step 1/20] Calling check_adapter ...
  [step 2/20] Calling check_ip_config ...
  [step 3/20] Calling ping_host (gateway: 192.168.1.1) ...
  [step 4/20] Calling test_dns_resolution ...
  [step 5/20] Calling test_internet ...

Agent: ## Network Diagnostic Report

**1. Wi-Fi Adapter:** Connected to "HomeNetwork" (802.11ac, signal 92%)
**2. IP Configuration:** 192.168.1.105 via DHCP (lease valid)
**3. Gateway:** 192.168.1.1 — ping succeeded (3ms)
**4. DNS Resolution:** google.com -> 142.250.80.46 — OK
**5. Internet:** TCP connection to 8.8.8.8:443 — succeeded

**Status: RESOLVED** — All diagnostics passed. Your network connection is healthy.
```
