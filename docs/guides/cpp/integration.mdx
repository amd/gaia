...
<Tabs>
  <Tab title="FetchContent (Recommended)">
    ...
    <Note>
    `SOURCE_SUBDIR cpp` tells CMake to use only the `cpp/` subdirectory of the GAIA repository. This skips the Python package entirely — your build tree contains only the C++ library and its dependencies.
    </Note>
...
<Tabs>
  ...
  <Tab title="Lemonade (default)">
    [Lemonade Server](https://github.com/amd/lemonade) is the default backend, optimized for AMD Ryzen AI NPU and GPU acceleration.
