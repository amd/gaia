...
This is where the agent fundamentally differs from the [Wi-Fi Troubleshooter](/guides/cpp/wifi-agent). Instead of relying on PowerShell for everything, the Process Analyst uses native Win32 APIs for its hot path — process enumeration and memory measurement.
