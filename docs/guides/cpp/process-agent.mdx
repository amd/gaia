...
description: "Build a system analyst agent in C++ that explains running processes, detects suspicious files, and lets you stop, restart, or quarantine items — all with local LLM inference"
icon: "microchip"
---
...
The System Analyst is an AI agent that makes sense of what's running on your PC. On startup it scans every process and service, classifies them by resource use and behavior, and shows you a plain-English summary. You can ask it to explain any task — what it is, what it does, and whether there's reason to be concerned — or take action directly.
...
<Steps>
  ...
  <Step title="Run the agent">
    ...
    The agent auto-analyzes on startup. After the analysis completes, you'll see the action menu.

    Try these inputs:
    ```
    1           (Explain all items)
    1 A3        (Explain task A3)
    2 A1        (Stop task A1)
    4 C1        (Quarantine suspicious item C1)
    5           (Reanalyze)
    6           (Toggle background monitor — default 2 min interval)
    6 5         (Toggle monitor with 5-minute interval)
    What is svchost.exe?   (free-form question)
    ```
...
The `runShell()` function wraps PowerShell execution, identical in pattern to the [Wi-Fi Troubleshooter](/guides/cpp/wifi-agent):
...
This is where the agent fundamentally differs from the [Wi-Fi Troubleshooter](/guides/cpp/wifi-agent). Instead of relying on PowerShell for everything, the System Analyst uses native Win32 APIs for its hot path — process enumeration and memory measurement.

Here's the core of `getTopProcesses()`, which combines snapshot enumeration, process grouping, file version info, and factual flag computation:

```cpp
static gaia::json getTopProcesses(int topN = 20) {
    HANDLE snapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    // ... enumerate all processes, measure memory via K32GetProcessMemoryInfo ...
    ...
    // ... group processes, resolve parent names via pid-to-name map ...

    // Sort by total memory descending, enrich with version info
    for (int i = 0; i < limit; ++i) {
        const auto* g = sorted[i];

        // File version info (company, description) via GetFileVersionInfoW
        ...
        if (company.empty()) flags.push_back("unknown_company");
        if (desc.empty())    flags.push_back("unknown_description");
        if (lowerPath.find("\\Temp\\") != std::string::npos ||
...
Tools are registered with the same pattern as the [Wi-Fi Troubleshooter](/guides/cpp/wifi-agent): a name, description, callback, and typed parameter list. The framework automatically includes these in the system prompt sent to the LLM.

Here's `system_snapshot`, which demonstrates the hybrid Win32 + PowerShell approach:
...
toolRegistry().registerTool(
    ...
    [](const gaia::json& /*args*/) -> gaia::json {
        ...
        result["disk"]          = getDiskUsageInfo();     // Win32: GetDiskFreeSpaceExW
        result["memory"]        = getMemoryInfo();        // Win32: GlobalMemoryStatusEx
        result["top_processes"] = getTopProcesses(30);    // Win32: CreateToolhelp32Snapshot

        // PowerShell: CPU load, uptime, services, connections
        std::string psCmd = "$o=@{}; $c=Get-CimInstance Win32_Processor; ...";
        // ... merge PowerShell results into result ...
...
toolRegistry().registerTool(
    ...
    [](const gaia::json& args) -> gaia::json {
        ...
        // Auto-append .exe if missing
        if (lower.substr(lower.size() - 4) != ".exe") target += ".exe";

        KillResult kr = killProcessesByName(target);
        return {
            {"tool", "kill_process"}, {"process", name},
...
**Why `command` + `output` keys?** Every tool returns both a `command` string and an `output` string. The `ProcessConsole` (and its base `CleanConsole`) uses these for its TUI display — the `command` appears in the tool header, and `output` appears in the preview box. This consistent contract means the output handler doesn't need to understand tool-specific JSON structures.
...
Select the most significant resource consumers. Consider:
- Memory usage relative to total system RAM
- Instance count (many instances = noteworthy)
- Whether the resource usage is expected for that application type
Tag each: [NORMAL] expected, [HIGH] unexpectedly high, [SUSPICIOUS]

### B. Services (problematic)
Report services with Status not "OK" or Memory > 200 MB.
If none: "All services running normally"

### C. Suspicious Items
The flags array provides factual signals — use as INPUT, not verdicts:
- unknown_company + unknown_description = STRONG signal
- temp_path = STRONG signal for non-installers
- EXCEPTIONS: svchost.exe, csrss.exe, smss.exe may show empty company — NORMAL
```
...
Users reference items by group letter + number (e.g., "Explain A3", "Stop B1").

- **Quarantine**: Before calling quarantine_item, you MUST:
  1. Call explain_item to get full details
  2. Present confirmation in Key: Value format
  3. Ask: Kill [name] and move it to quarantine? (yes / no)
  4. Wait for response. If NO, abort. If YES, call quarantine_item.
```

**Why A/B/C labels?** They create a shared vocabulary between the LLM and the user. "Explain A3" unambiguously maps to the third process in the resource consumers section — no re-scanning required.

**Why explicit quarantine protocol?** Quarantine is irreversible — it kills the process AND moves the executable file. The multi-step protocol (explain, confirm, wait for yes/no) prevents accidental data loss from an LLM hallucination.
...
3. **Item reference** — `A4: cpptools-srv.exe` — bold cyan tag, bold white name
4. **Key: Value** — first colon within 20 chars with alpha key — bold white key, normal value
5. **Numbered item** — `1. chrome.exe ...` — normal rendering with markdown bold support
...
if (line.size() >= 4 && line[0] >= 'A' && line[0] <= 'C' &&
    ...
    if (colon != std::string::npos && colon <= 4) {
        ...
        std::cout << color::BOLD << color::CYAN << tag << color::RESET
                  << "  " << color::BOLD << color::WHITE << rest << color::RESET;
    }
}
```
...
**Alert delivery.** Pending alerts are displayed before the next menu prompt, so you see them naturally between actions. For CRITICAL and NEW_SUSPICIOUS alerts, the agent also sends a Windows balloon notification via `Shell_NotifyIconW` so you get notified even if the terminal is in the background.
...
Then update the system prompt's `## AVAILABLE TOOLS` section to reference the new tool so the LLM knows when to use it. Optionally, add an `ActionEntry` to `kActions[]` if you want a new numbered menu item (e.g., `[7] Startup Items`).

The framework handles everything else — the new tool appears in the LLM's system prompt automatically via the tool registry.
...
<CardGroup cols={2}>
  ...
  <Card title="Wi-Fi Troubleshooter Agent" icon="wifi" href="/guides/cpp/wifi-agent">
    Compare single-phase diagnostic flow with the two-phase workflow used here
  </Card>
</CardGroup>
