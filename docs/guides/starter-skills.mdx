...
print(agent.query("What's the current state of NPU support in llama.cpp?"))
```
...
- **Demonstrates** — multi-step planning, web search + fetch, optional RAG over
  what it fetched.
- **Consumes** — `search_web`, `fetch_page`, `index_document`, `query_documents`.
...
| `email-triage` (Gmail triage over MCP) | There is no `gmail` connector id — Gmail is a *scope* of the `google` OAuth connector, not an MCP server with tools. Email triage ships instead as a full agent: see the [Email guide](/guides/email). `github-triage` covers the same connector-bridged primitive. |
