...
<Note>
  **Memory is opt-in (beta) and disabled by default.** This feature is still under active development -- every new GAIA install ships with memory off, so you'll see standard chat behavior until you turn it on. With memory disabled, prompt construction and session reset do no memory work. The SDK `memory_store` accessor returns `None` for a disabled session; callers can use `memory_unavailable_message()` to explain why. See [Enable Memory](#enable-memory) below before trying any examples on this page.
</Note>
...
After each conversation turn (with memory enabled), the agent automatically decides what's worth remembering -- without you saying "remember." This isn't pattern matching; the LLM sees your existing memory, the new conversation, and what the tools returned during the turn, and decides whether to **ADD** new knowledge, **UPDATE** a changed fact (old version preserved with lineage), or **DELETE** something you've contradicted.

It extracts information useful in future conversations -- facts, preferences, project details, people, deadlines -- and skips greetings, confirmations, ephemeral details, and narration of the current session (where you're working, what was just changed). For example, "we standardized on Python 3.12 and I'm using a Ryzen 9 9950X" stores two facts but ignores subjective asides like "it's much faster."

Only what you said or what a tool showed is stored. A claim that appears only in the agent's own answer is not, because the answer can be wrong, and a permission refusal is not, because permissions change.

<Tip>
  Intelligent extraction kicks in for messages of 5+ words. Short messages like "yes" or "thanks" are skipped. The LLM's explicit memory tools (`remember`, `update_memory`, `forget`) still handle anything the auto-extraction misses.
</Tip>
...
`/memory` works the same way regardless of how the flagship agent got onto your
machine -- installed standalone, or hub-installed via `gaia init` and run as a
daemon-supervised sidecar. The one thing that can still block it is an
out-of-date sidecar: a peer built before this contract has no memory route at
all, and `/memory` says so and names the fix rather than failing silently --
`gaia hub uninstall gaia` then `gaia hub install gaia` picks up a build that has
it.
...
- **Automatic error learning** -- a tool call that ran and failed is stored as an error pattern; the next session's system prompt includes a "Known errors to avoid" list so the agent doesn't repeat it. Calls the permission layer refused — including ones you declined at the confirmation prompt — are not stored (they describe your current permissions, which can change) and never clear what is already stored; an error is dropped as soon as the same operation (the same program or path) actually runs and succeeds.
- **Lessons from fixes** -- when a call fails and a changed call of the same kind then works in the same turn (for example `pytest` fails until the agent adds `env TOYBOX_CLOCK=frozen`), GAIA keeps both calls as a lesson for that project. Later sessions anywhere in the same project see it in their system prompt with the date it was learned; another project never does. Scope is the project root -- the repository you are working in, or `GAIA_PROJECT_ROOT` when you set it -- so approving a file elsewhere mid-session doesn't move your lessons. It gains confidence each session the fix works again, is dropped if the fix itself fails, and a newer fix for the same operation replaces the older one rather than piling up variants.
