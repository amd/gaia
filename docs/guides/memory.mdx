...
<Steps>
  ...
  <Step title="Teach it about you">
    ...
    Agent: Stored as a fact and a preference.
    ```
  </Step>
  ...
  <Step title="Add a time-sensitive item">
    ...
    Agent: Stored as a reminder with a due date, linked to person:sarah.
    ```
  </Step>
...
Recall also supports **temporal filtering** with `time_from` and `time_to`. The agent converts natural language dates to time ranges automatically:

```
You: "What did we discuss about deployment last week?"
Agent: [computes last week's date range, searches within that window]

You: "What errors have I seen today?"
Agent: [filters to today's date, category=error]

You: "What meetings did I have in March?"
Agent: [recalls notes with domain=meeting from March 1-31]
```
...
Agent: Stored as a note with domain=journal, dated 2026-04-01.
...
Agent: Stored as a fact with domain=research.
...
Agent: Stored as a reminder with due_at=Friday.
...
Agent: "Sure, here's the refactored version. By the way,
        your expense report is due tomorrow."
```

Recurring reminders work too -- the agent advances the date each time:

```
You: "Remind me to do a weekly review every Friday at 5pm"
Agent: Stored with due_at=next Friday 5pm.
       After each reminder, the agent advances due_at to the following Friday.
```
...
Agent: Stored as a fact linked to person:sarah_chen, email marked sensitive.

— Another conversation —

You: "Sarah prefers morning meetings and likes detailed agendas"
Agent: Added to person:sarah_chen profile.
...
Agent: [auto-stores error pattern]
...
- **0.4** -- LLM-extracted (automatically captured from conversation)
- **0.4** -- system discovery (found during bootstrap scan)
- **0.5** -- tool-stored (agent explicitly remembered it)
- **0.5** -- consolidation (distilled from old conversation sessions)
