...
- **Lessons from fixes** -- when a call fails and a changed call of the same kind then works in the same turn (for example `pytest` fails until the agent adds `env TOYBOX_CLOCK=frozen`), GAIA keeps both calls as a lesson for that workspace. Later sessions in the same folder see it in their system prompt with the date it was learned. It gains confidence each session the fix works again and is dropped if the fix itself fails. Lessons never show up in other folders.
