...
2. **Recall naturally** -- each turn, the few stored memories most relevant to your message are added to the agent's context automatically, each with the date it was learned. For anything more, the LLM searches with its own tools: hybrid semantic+keyword search finds memories by meaning, not just exact words.
