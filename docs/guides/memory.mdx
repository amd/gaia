...
- **Automatic error learning** -- a tool call that ran and failed is stored as an error pattern; the next session's system prompt includes a "Known errors to avoid" list so the agent doesn't repeat it. Calls the permission layer refused are not stored (they describe your current permissions, which can change), and an error is dropped as soon as the same operation (the same program or path) succeeds.
