...
In a default session the active context is `global`, and the system prompt includes memories from every context. An agent that sets a specific context sees that context plus `global`, and new memories are stored under the active context:

```python
agent.set_memory_context("personal")   # or init_memory(context="personal")
```

<Tip>
  `gaia chat` runs in the default `global` context. The model can't choose a context when it stores a memory, so a memory can't be filed somewhere no later session reads it.
</Tip>
