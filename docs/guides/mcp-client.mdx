...
Try it now - this connects to multiple MCP servers and starts an interactive session:
...
```
Connected to MCP servers: memory, thinking, time
...
Three MCP servers connected, tools auto-discovered, and the agent uses them naturally.
...
graph TB
    User("User Query")
    YourAgent("YourAgent<br/>(Agent + MCPClientMixin)")
    ...
    Transport --> Custom("Your Server")

    style User fill:#2d2d2d,stroke:#ED1C24,stroke-width:2px,color:#fff
    style YourAgent fill:#ED1C24,stroke:#F4484D,stroke-width:3px,color:#fff
    ...
    style FS fill:#2d2d2d,stroke:#ED1C24,stroke-width:2px,color:#fff
    style GH fill:#2d2d2d,stroke:#ED1C24,stroke-width:2px,color:#fff
    style DB fill:#2d2d2d,stroke:#ED1C24,stroke-width:2px,color:#fff
...
<Steps>
  ...
  <Step title="Verify with a query">
    ...
    ```
    ```
    Connected to MCP servers: memory
    Try: 'What time is it in Tokyo?' | Type 'quit' to exit.
