...
Build a working MCP-powered agent in 2 minutes.

<Steps>
  <Step title="Create the Agent">
    Create a file `time_agent.py` with this complete example:
...
Use the get_current_time tool to answer time-related questions."""
  ...
  <Step title="Run Your Agent">
    ...
    **Expected output:**
    ...
    Result: The current time in Tokyo is [current time]
    ```
  </Step>
</Steps>
...
- The `get_current_time` tool was automatically discovered and registered
- The LLM decided to call the tool based on your query
- The tool result was formatted into a natural language response

<Tip>
  For production use, configure servers once using `gaia mcp add` and load them from config. See [`examples/mcp_config_based_agent.py`](https://github.com/amd/gaia/blob/main/examples/mcp_config_based_agent.py) for the recommended pattern.
</Tip>
