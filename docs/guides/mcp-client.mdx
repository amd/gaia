...
MCP is an open protocol that standardizes how AI applications connect to external tools and data sources. Many services provide MCP servers (filesystem, GitHub, Slack, databases, etc.), and GAIA can connect to all of them.
...
<Steps>
  <Step title="Add an MCP Server">
    Connect to an MCP server using the CLI. This saves the server configuration to `~/.gaia/mcp_servers.json` for reuse:
    ...
    The command above:
    - Creates a server named "time"
    - Uses `uvx` to run the `mcp-server-time` package
    - Saves the configuration for later use
  </Step>

  <Step title="Use in Your Agent">
    Add the MCPClientMixin to your agent:
  ...
  <Step title="Run Your Agent">
    The agent now has access to all time server tools:
...
<Note>
**License**: This guide is part of the GAIA project and is subject to the [project's license](https://github.com/amd/gaia/blob/main/LICENSE).
</Note>
