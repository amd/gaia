...
<Info>
  **Example:** [`examples/mcp_config_based_agent.py`](https://github.com/amd/gaia/blob/main/examples/mcp_config_based_agent.py)
</Info>
...
2. Generate a new token with `repo` scope (or fine-grained permissions as needed)

**Step 2:** Pass the token via the `env` field

The new config format supports environment variables natively:
...
<Note>
The `env` field passes environment variables directly to the subprocess without shell expansion, making it secure and portable across platforms.
</Note>
