...
<Warning>
  **Hardware Requirements:** The Agent UI requires one of the following to run large language models locally:
  ...
  If your device is not supported, the CLI will display an error at launch and the in-app banner will appear with a link to [request support for your device](https://github.com/amd/gaia/issues/new?template=feature_request.md&title=[Feature]%20Support%20Agent%20UI%20on%20additional%20devices&labels=enhancement,agent-ui).

  To bypass the check (e.g. when connecting to a remote Lemonade server):
  ```bash
  gaia --ui --base-url https://your-server:8000/api/v1   # remote server — check skipped
  gaia --ui --skip-device-check                          # force bypass
  set GAIA_SKIP_DEVICE_CHECK=1 && gaia --ui              # env var bypass
  ```
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Unsupported device error at launch">
    The Agent UI requires an AMD Ryzen AI Max (Strix Halo) or an AMD Radeon GPU with ≥ 24 GB VRAM to run large language models locally.

    **Bypass options:**

    ```bash
    # Connect to a remote Lemonade server (inference runs there, not locally)
    gaia --ui --base-url https://your-server:8000/api/v1

    # Force bypass (you accept responsibility for memory requirements)
