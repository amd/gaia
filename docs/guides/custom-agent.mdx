...
| `wait` | `gaia.agents.tools.wait_tools.WaitToolsMixin` | `self.register_wait_tools()` |
...
Bundles are validated on import. The receiving end enforces conservative limits (max 1000 entries, 500 MB uncompressed, 50 MB per file, 100 MB upload via the UI) and rejects symlinks, device or other special files, and path-traversal entries.
...
<AccordionGroup>
  ...
  <Accordion title="Research Agent — RAG + file search">
    ...
    it up itself (see [the RAG tools mixin contract](/spec/rag-tools-mixin)
    for what each attribute is for). Pass `documents` pointing at wherever your
    documents actually live; the example never widens access beyond that.
    ...
    class ResearchAgent(Agent, RAGToolsMixin, FileSearchToolsMixin):
        ...
        def __init__(self, documents: Optional[List[str]] = None, **kwargs):
            # RAGToolsMixin reads this state off the host agent; set it here,
            # before super().__init__() runs _register_tools(). **kwargs
            # forwards everything else Agent.__init__ accepts (model_id, etc).
            self.rag = RAGSDK(RAGConfig(allowed_paths=documents))
  ...
  <Accordion title="Code Review Agent — shell + file I/O">
    ...
    class CodeReviewAgent(Agent, FileIOToolsMixin, ShellToolsMixin):
        ...
        def __init__(self, documents: Optional[List[str]] = None, **kwargs):
            # FileIOToolsMixin reads this state off the host agent; set it
            # here, before super().__init__() runs _register_tools(). **kwargs
            # forwards everything else Agent.__init__ accepts (model_id, etc).
            self.path_validator = PathValidator(documents)
            super().__init__(**kwargs)
...
### Why a custom RAG agent can't see the Document Library

A custom agent's `documents` list only ever contains what the *caller* passes
