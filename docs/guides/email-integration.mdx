...
The frozen sidecar is ideal for production but opaque to debug: if you hit a
triage/draft bug, there's no source to edit — you'd file an issue and wait for a
new binary. To iterate on the agent, run its **Python source** and drive it with
the **same npm client**. The frozen binary is that source frozen (PyInstaller
freezes a thin re-export of `gaia_agent_email.server`), so the `/v1/email/*`
contract is identical — **only the base URL differs from production.**
...
spawns nothing, and returns a bound client (no `child`, nothing to `shutdown()` —
you own the `serve` process):
...
// Edit the Python under gaia_agent_email/, save → it auto-reloads → re-run. Seconds.
```

`npx @amd-gaia/agent-email dev` launches the `serve` process for you
(`--python <path>` to use a specific venv). Auto-reload restarts the server, so
in-process `/v1/email/agent/*` sessions reset on each reload — irrelevant to the
stateless `triage`/`draft`/`send` calls (each request is self-contained). Switch
back to production by pointing at `startSidecar` (frozen binary) instead of
