...
<Note>
  **Migration note:** triage no longer auto-downloads the model on its first call.
  On a fresh server, provision once — `POST /v1/email/init` (or pull the model on
  your Lemonade) — before the first triage request; until then triage returns
  **502** with the fix in the error. The target server's catalog must list the
  model that `GET /v1/email/init` reports as `model.id` (`user.`-prefixed
  registrations match tolerantly).
</Note>
