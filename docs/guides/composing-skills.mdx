...
<Info>
  **Prerequisites:** you have an agent package with a `gaia-agent.yaml` — see
  [Hub publishing](/guides/hub-publishing). Background on the format itself is in
...
- [Skill Format](/spec/agent-skills) — the full field grammar
- [Hub publishing](/guides/hub-publishing) — building the agent the skills attach to
