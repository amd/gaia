...
The agent can run compact SLMs **before** the main LLM for two triage tasks,
...
<Warning>
**Experimental — off by default.** Enable it with `GAIA_EMAIL_USE_SLM=true` (or
`use_slm=True` on the config).
</Warning>
...
## Mailbox selection in the flagship agent

The flagship `gaia` agent reads mail through the same five tools regardless of which
...
account was connected earlier for the Email Triage Agent, it already carries the broader
`gmail.modify` scope; the flagship reuses that rather than asking you to reconnect, but
it does need its own entry in the grant ledger:
