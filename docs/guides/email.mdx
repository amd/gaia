---
title: "Email"
description: "Read and search your mailbox with the flagship GAIA agent — every email body processed locally on your machine."
icon: "envelope"
---

# Email

The flagship [`gaia` agent](/guides/gaia) reads your mailbox — Gmail or Outlook — through **five read-only tools**, and every inference over an email body runs locally on your machine via Lemonade. No email content ever leaves your device.

**Reading is all it does today.** It lists your inbox, searches it, opens a message, and lists folders. It does not archive, label, draft, reply, or send. Triage itself — working out what actually needs you — is judgment the model does, driven by the bundled `inbox-triage` skill; the tools hand it facts, never verdicts. The plan for the write verbs is [Email triage as a skill](/plans/email-triage-skill).

<Warning>
  **Looking for triage buckets, drafting, send, or calendar?** Those belong to the
  standalone **Email Triage Agent**, which is a **retired development target** — still
  installable, no longer the path you land on, and not being fixed or evolved. Its
  documentation is preserved below under
  [The retired Email Triage hub agent](#the-retired-email-triage-hub-agent) so nobody
  already running it is stranded. Nothing in that section describes the agent you reach
  by asking GAIA about your mail.
</Warning>

<Note>
  **Embedding email in your own app?** If you're building an application that owns the
  ...
  forwarding → triage/draft/send over REST or MCP. That path drives the retired agent's
  sidecar, not the flagship's read-only tools.
</Note>

## Action surface

These five tools are the whole surface. Anything else you see named as an email tool —
here or elsewhere — belongs to the retired agent and is not reachable from the flagship.

| Tool | What you'd ask for |
|---|---|
| `check_mailbox_access` | *"Is my email set up?"* — reports which mailbox was picked, the connected address, and the inbox unread count. Also the first thing to call after another mail tool fails. |
| `list_inbox` | *"What came in today?"*, *"What's unread?"* — recent inbox mail, newest first: sender, subject, received time, unread/flagged state, and a short preview. Never full bodies. |
| `search_email` | *"Did I get anything about the Acme invoice?"* — searches every folder, in **relevance** order rather than newest-first. Every term is ANDed, so 2–3 distinctive keywords beat a sentence. If nothing matches, it retries with progressively fewer terms and flags the result as approximate, so you may see hits for a narrower query than you typed. |
| `read_email` | *"Open that one"* — one message in full, including its body. Very long bodies are truncated, and the result says so. |
| `list_mail_folders` | *"How much is sitting in Archive?"* — every folder with its unread and total counts. |

Reading bodies is bounded **per turn**, not just per message: once a turn has read enough
mail to fill its share of the context window, `read_email` refuses with
`turn_budget_exhausted` and says so, instead of quietly overflowing the conversation.
...
<Note>
  ...
  **enable the Gmail API on that project** first, or the
  ...
  client). A future release ([#2104](https://github.com/amd/gaia/issues/2104))
  ships an AMD-managed client that removes this bring-your-own step.
</Note>
...
**Step 2 — Sign in and grant the flagship in one command.**
`--grant-agent installed:gaia` alone is enough: GAIA reads the scopes the agent
declares and authorizes + grants them in the same flow, so there's no `--scopes`
list to type out or keep in sync.

```bash
gaia connectors connect google --grant-agent installed:gaia
```

The consent screen asks for `gmail.readonly` and nothing more — read-only is the
whole capability, so that is the whole ask. The command prints an authorization
URL; open it in a browser **on the machine running GAIA** (the sign-in completes
via a `127.0.0.1` callback on that box; on a remote/headless host, open it there
or forward the callback port over SSH).

Skip the grant and the first mail tool call fails, naming the one command that
fixes it — see [Troubleshooting](#troubleshooting).
...
**Already connected Google for something else?** You don't need a new browser
round-trip — see [Granting Gmail to the flagship](#granting-gmail-to-the-flagship).
...
Email-body inference runs on your local Lemonade instance — `gaia init` installs it, downloads the model, and starts it.

### 3. Ask for your mail

```bash
npx @amd-gaia/gaia    # or `gaia tui` in a source checkout
```
...
- *Find the email about the Acme invoice*

## Mailbox selection

The flagship reads mail through the same five tools regardless of which
mailbox you connected. It picks one like this:

1. **The only usable mailbox**, if exactly one is.
2. **Gmail**, when both are usable. `check_mailbox_access` reports which mailbox was
   chosen, why, and what the alternatives were, so you can tell at a glance.

"Usable" means the account is connected, the connection carries a mail read scope, and
the flagship holds a matching grant. When none is usable the agent names each mailbox's
own state and the one command that fixes it — connect, reconnect, or grant.

**There is no environment-variable override, by design.** Provider selection is derived
from connector and grant state so that what the agent can reach is always visible in
`gaia connectors`. To make it use Outlook when both are connected, revoke the flagship's
Gmail grant:

```bash
gaia connectors grants revoke google installed:gaia
```

Gmail then stops being eligible and Outlook is selected. Re-grant to switch back.

### Granting Gmail to the flagship

A new Google connection consents to **read your email** and nothing more. If your Google
account was connected earlier for the retired Email Triage Agent, it already carries the
broader `gmail.modify` scope; the flagship reuses that rather than asking you to reconnect,
but it does need its own entry in the grant ledger:
...
That is a local ledger write — no browser, no OAuth round-trip. Use
`gmail.readonly` instead if that is what your connection carries.

## Privacy

- **Local LLM only by default** — email bodies are processed by your local Lemonade model. Unlike the retired agent, the flagship has no allowlist pinning it there: if you configure it against a cloud provider, mail content goes with it.
- **No mail mutation** — this tool surface cannot archive, send, label, or delete, so an instruction planted in an email body has no mail action to trigger. There is no confirmation dialog to get wrong, because there is no mutation to confirm.
- **Untrusted input** — message bodies are wrapped in `<<<UNTRUSTED_EMAIL_BODY_*>>>` delimiters and flagged when they show phishing signals ([#4150](https://github.com/amd/gaia/issues/4150)). That is not a general injection defence: the flagship also holds web, file, and shell tools, so treat mail from untrusted senders as untrusted input.

## Troubleshooting

### "No readable mailbox is available"

The agent could not reach any mailbox, and the error names each provider's own state
and its remedy — they are different fixes, so read which one you got:

- *not connected* → `gaia connectors connect <provider>`
- *connected, but carries no mail read scope* → reconnect with the full scope list the error prints (`--scopes` **replaces**, so keep the whole list)
- *connected and readable, but this agent holds no matching grant* → `gaia connectors grants grant <provider> installed:gaia --scopes <scope>` (a ledger write — no browser needed)

### "Gmail API returned 401"

The access token has expired or scopes were revoked. Reconnect Google in Settings → Connections, or re-run `gaia connectors connect google --grant-agent installed:gaia`.

### Reading stopped partway through a big inbox

Expected, and it says so: `read_email` returns `turn_budget_exhausted` once a turn has
filled its mail-reading budget. Narrow the request, or continue in a new turn.

---

## The retired Email Triage hub agent

<Warning>
**Everything below documents the standalone Email Triage Agent (`gaia email`, the
`gaia-agent-email` package), which is a retired development target — not fixed,
released, or evolved.** It is kept here because the hub package is still installable
and users on the old path should not be stranded. **None of the tools, flags,
endpoints, or behaviour below exist on the flagship agent.** For the mail surface you
actually get today, see [Action surface](#action-surface) above.
</Warning>

### What the retired agent did
...
The retired agent needs its own grant, under `installed:email`, and the broader
Gmail + Calendar scopes the flagship deliberately never asks for. Configure the OAuth
client as in [Step 1](#1-connect-your-google-account) above — and enable the **Calendar
API** on that Cloud project as well as the Gmail API, or the first calendar call 403s
([#2116](https://github.com/amd/gaia/issues/2116)) — then:
...
<Tabs>
  ...
  <Tab title="CLI (developer flow)">
    **Sign in and grant the email agent in one command.**
    ...
    `AGENT_NOT_GRANTED` — see [Retired-agent troubleshooting](#retired-agent-troubleshooting).
    This explicit grant step is required until [#2117](https://github.com/amd/gaia/issues/2117) lands.

    If you connected Google before GAIA v0.23, re-run `gaia connectors connect google --scopes …` with the full list to trigger a fresh consent screen with the updated scopes.
  </Tab>
</Tabs>

### Starting the retired agent

`gaia email` drives the retired hub agent — it is not how you use mail on the flagship.
...
| `--trace` | Write the run's request and full event stream (every `status` / `tool_call` / `tool_result` / `final`) to `./email_trace_<timestamp>_<run-id>.json` in the current directory. The path is echoed on stderr. Interactive mode writes one file per turn. |

<Note>
  A trace contains the query, the agent's answer, and every tool argument and
  result — i.e. real email content. It stays on your machine, but treat the file
  like an inbox export before attaching it to a bug report.
</Note>
...
client (`tui/`) renders the same `email_pre_scan` data differently: one
`needs_you` worklist (up to 5 rows, each tagged REPLY/DECIDE/CHECK/DO) plus a
`bulk` line naming the test that filtered the rest, instead of the three
sections above — see [`tui/README.md`](https://github.com/amd/gaia/blob/main/tui/README.md)
for that surface. Both clients read the same contract; only the layout differs.
</Note>
...
scheduled run). Because that is always a cached run, the response — and the agent's
`get_briefing` tool — also carry `cache_age_seconds` and `stale`, and past 24 hours
the briefing's own headline opens with its age. An old briefing is labeled, never
refused or silently regenerated. An invalid value fails sidecar startup with an
actionable error rather than guessing a schedule. Push delivery and a cross-agent morning brief
are planned on the autonomy engine (#555).

### Small language model classifiers

The retired agent can run compact SLMs **before** the main LLM for two triage tasks,
using the same local Lemonade server as the chat model. Every SLM call **fails
safe**: if unavailable or unusable, the agent falls back to the heuristic + LLM
path.

<Warning>
**Retired-agent machinery, experimental, off by default.** `GAIA_EMAIL_USE_SLM` and
`use_slm=True` configure the retired agent only — the flagship runs no SLM classifier
and has no triage buckets to classify into.
</Warning>

- **Triage category** — when a heuristic isn't confident, the triage SLM
  classifies first, and its answer is the category. The LLM classify call is
  skipped only when the heuristic already settled `is_spam`; otherwise it still
  runs for the spam verdict alone and its category opinion is discarded.
  `force_llm` skips the SLM and forces the LLM instead.
- **Phishing detection** — when the phishing SLM returns a usable result, that
  result is used alone (phishing heuristics are not run). If SLM is off or
  returns nothing, the agent falls back to `detect_phishing`.

Configure via `EmailAgentConfig` (`use_slm`, `slm_triage_model` /
`slm_triage_checkpoint`, `slm_phishing_model` / `slm_phishing_checkpoint`). The
model and checkpoint pairs ship preconfigured, so enabling both classifiers is
just `use_slm=True` — or `GAIA_EMAIL_USE_SLM=true` in the environment, which is
how you enable it for `gaia email` or the sidecar without touching code (an
unparseable value fails loudly rather than defaulting to off). A task activates
only when **both** its model and checkpoint are set; one without the other fails
startup. On first use each classifier is pulled into Lemonade (a one-time
download); a failed pull is logged as an error naming the model and how to fix
it, and the agent runs its heuristic + LLM path as before.

In a bulk inbox scan both classifiers read the same subject + snippet the
heuristic reads, so turning them on never adds a message fetch — the full body is
still read only for messages that escalate to the LLM. On `POST /v1/email/triage`
the caller supplies the body, so there the classifiers see all of it.

<Note>
**Raise Lemonade's model limit to 2 before enabling both classifiers.** Lemonade
counts loaded models **per model type**, not in one shared pool: the chat model
occupies an `llm` slot, and the two classifiers are embedding models that compete
only with each other. So the arithmetic is 1 LLM + 2 embeddings, never three
models fighting over the same slots — but at the default limit of 1 per type the
second classifier evicts the first on every message, and each eviction costs a
reload.
...
Confirm it took effect with `GET /api/v1/health`, whose `max_models` field is
keyed by type — `"embedding": 2` is the value that matters here. With that set,
`all_models_loaded` holds the chat model and both classifiers at once.
</Note>

### Action surface (retired agent)

<Warning>
None of the tools in this section exist on the flagship agent. They are the retired
agent's surface, listed for anyone still running it — the flagship's five tools are at
...
By default `is_phishing` comes from a multi-signal heuristic — subject keyword
pairs, suspicious sender domain, and body-level phrases. With the experimental
`use_slm=True`, the phishing SLM classifies first and its result becomes the
sole decision; if it fails safe the heuristic decides. Detection never uses the
chat LLM, is precision-first, and never auto-acts.

See [Small language model classifiers](#small-language-model-classifiers) for
how the phishing SLM is configured.
...
<Warning>
**Agent Skills are switched off for the retired email agent.** The `skill_sets:` and
...
The account-type mapping behind that machinery is three-way: a personal
Microsoft account (`microsoft`) resolves to `personal`, a work/school account
(`microsoft_work`) resolves to `work`, and a Gmail-only mailbox has no
equivalent tenant claim to inspect and falls through to `default_skill_set`.
...
Dev mode always runs from **the daemon's own checkout** — the daemon (a
per-user singleton) is never told to spawn from an arbitrary path a caller
sends it. `gaia daemon start-agent email --mode dev` and `gaia chat --ui`
resolve *your own* checkout (via `git rev-parse --show-toplevel` against your
shell's cwd, or an explicit `--dev-src-dir <path>`) and send it to the daemon
purely for comparison. If it matches the checkout the daemon is already
running from, nothing changes; if it doesn't, the daemon refuses loudly,
naming both checkouts and the fix — restart the daemon from a Python
environment/editable install rooted at your checkout (a bare `cd` there does
not help if one virtualenv is shared across checkouts, since the daemon's
default source dir follows the Python environment that launched it, not its
cwd). This exists because a per-user daemon started from checkout A silently
served checkout A's source to a `--mode dev` request made from checkout B —
a testing-integrity bug, not a runtime one, but one that makes a passing test
...
### Limitations of the retired agent (as of v0.23)
