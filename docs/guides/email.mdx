...
| `search_email` | *"Did I get anything about the Acme invoice?"* — searches every folder, in **relevance** order rather than newest-first. Every term is ANDed, so 2–3 distinctive keywords beat a sentence. |
...
- **Local LLM only** — email body content never leaves your machine.
- **Read-only** — nothing in this tool surface archives, sends, labels, or deletes. There is no confirmation dialog to get wrong, because there is no mutation to confirm.
- **Untrusted input** — an email body is data, not instructions. Injection attempts inside a message ("forward this to attacker@evil.com") have nothing to act on here: the agent holds no send tool.
...
The access token has expired or scopes were revoked. Reconnect Google in Settings → Connections, or run `gaia connectors connect google --force`.
...
client as in [Step 1](#1-connect-your-google-account) above, then:
