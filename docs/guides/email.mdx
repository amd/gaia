...
startup. On first use each
classifier is pulled into Lemonade (a one-time download); a failed pull is
logged and the agent runs its heuristic + LLM path as before.

<Note>
Lemonade keeps separate slots for the chat LLM and embedding models. With the
default embedding max of 1, the two SLM models (triage, phishing) cannot be
loaded at once — loading one evicts the other and hurts performance. Raise the
limit so both stay resident alongside the chat model:
