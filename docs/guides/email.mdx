...
2. **Mailbox account type** — a personal Microsoft account (`microsoft`)
   activates `personal`, a work/school account (`microsoft_work`) activates
   `work`. The kind is derived from the id_token `tid` claim when the mailbox
   is connected and stored with the connection, so nothing is re-derived per
   run. Pin it with `GAIA_EMAIL_ACCOUNT_TYPE=work`.
