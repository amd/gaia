...
1. **`GAIA_MAIL_PROVIDER`** — set it to `google` or `microsoft` to force the choice.
   If that mailbox is not usable, the agent says so rather than quietly using the other.
2. **The only usable mailbox**, if exactly one is.
3. **Gmail**, when both are usable. `check_mailbox_access` reports which mailbox was
