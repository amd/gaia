...
<Info>
  **Prerequisites:** Complete the [Setup](/setup) guide and have the [Agent UI](/guides/chat-ui) backend running.
</Info>
...
<Steps>
  ...
  <Step title="Add the MCP server to Claude Code">
    ...
    claude mcp add gaia-agent-ui -s project -- uv run python -m gaia.mcp.servers.chat_ui_mcp --stdio

    # Or user-scoped (available in all projects)
    claude mcp add gaia-agent-ui -s user -- uv run python -m gaia.mcp.servers.chat_ui_mcp --stdio
    ```
...
# Custom port
uv run python -m gaia.mcp.servers.chat_ui_mcp --port 9000
```
