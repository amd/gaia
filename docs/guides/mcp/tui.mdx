...
`gaia-tui` boots straight into the flagship. Chat now has an in-session `/agents`
command that lists installed agents and switches to one, but there is no
dedicated control-API tool for it -- drive it the same way a human would, with
`tui_send_text("/agents")` and `tui_send_keys`. `tui_status` reports `view` as
one of:
...
<Warning>
  ...
  `gaia-tui`; a different agent at launch is `gaia-tui chat --agent <id>`;
  switching agents inside a live session is chat's `/agents`, driven through
  `tui_send_text`/`tui_send_keys` like any other command; and installing
