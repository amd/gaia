...
matching local HTTP server and port — and only while the process that file
records is still running. A killed or crashed embedded server cannot clear its
own state file, so that file is ignored once its process is gone and GAIA falls
back to `LEMONADE_BASE_URL` or the default port.
