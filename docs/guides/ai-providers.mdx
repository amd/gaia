...
description: "Run GAIA locally or connect Fireworks AI, QwenCloud, and AMD LLM Gateway through Lemonade."
---

GAIA's terminal UI offers **Local**, **Fireworks AI**, **QwenCloud**, and **AMD LLM Gateway**.
...
For **QwenCloud**, paste a **pay-as-you-go** API key (it starts with `sk-`) from the
[QwenCloud console](https://home.qwencloud.com/api-keys). GAIA routes requests
through Lemonade to QwenCloud's OpenAI-compatible endpoint,
`https://dashscope-intl.aliyuncs.com/compatible-mode/v1`. The endpoint is fixed,
like Fireworks'. **Qwen3.8 Max** (`qwencloud.qwen3.8-max`) is listed first when your
account exposes it. Faster models such as `qwencloud.qwen3.8-flash` are in the
same list. Token Plan and Coding Plan keys (`sk-sp-`) only work with the plan's own
endpoint and a registered client. The setup screen refuses a pasted one, and an
authentication failure names the key type if one reaches Lemonade through
`LEMONADE_QWENCLOUD_API_KEY`. QwenCloud chat sends conversation history to
QwenCloud and may incur usage charges. QwenCloud bills by input-length tier, so
GAIA ships no built-in rate for it. The session readout shows tokens, and
`/cost help` explains how to set your own rate.
...
- Leave the key field blank to use a credential already configured in Lemonade.
  `LEMONADE_FIREWORKS_API_KEY`, `LEMONADE_QWENCLOUD_API_KEY`, and
  `LEMONADE_AMD_API_KEY`, set in the **Lemonade server process**, take precedence
  over pasted keys. A conflict is reported
  explicitly.
...
for model availability and deployment requirements, and
[QwenCloud models](https://www.qwencloud.com/models) for the Qwen catalog.
