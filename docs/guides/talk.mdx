...
Talk mode leans on two native libraries: PortAudio for the microphone, and espeak-ng, which Kokoro TTS falls back on for words outside its pronunciation dictionary. Install them before the talk extras.
...
<Tabs>
  ...
  <Tab title="Windows">
    PortAudio ships inside the `sounddevice` wheel, so the microphone needs nothing extra. espeak-ng does not:
...
<Note>
  Transcribing an **existing** recording is no longer part of talk mode — point the GAIA agent at the file instead. See [Transcription & Meeting Notes](/guides/transcription).
</Note>
...
<Tabs>
  ...
  <Tab title="Model & Backend">
    ...
    gaia talk --use-claude                            # claude-sonnet-5 by default
    gaia talk --use-claude --claude-model claude-opus-5
    ```

    <Note>Defaults: `Gemma-4-E4B-it-GGUF`, 512 tokens, local Lemonade. `--use-claude` needs `ANTHROPIC_API_KEY`. `--use-chatgpt` was removed — configure the model in Lemonade and point `--base-url` at it.</Note>
  </Tab>
...
Talk mode's ASR covers **live microphone input only**. To transcribe a file you already have, see [Transcription & Meeting Notes](/guides/transcription).
