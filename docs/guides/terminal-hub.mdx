...
<Note>
  `gaia tui` is the Go terminal binary. Building from source it is
  `tui/bin/gaia-tui` (`cd tui && make build`) — never `gaia`, which is the Python
  CLI's name and has entirely different subcommands. It accepts a leading `tui`
  word and drops it, so `./tui/bin/gaia-tui` and `./tui/bin/gaia-tui tui` open the
  same session. See the
...
Each release publishes a native installer per platform. They put two binaries on
disk and on your PATH — `gaia-tui` (this hub) and `gaia-agent` (the flagship agent
the hub runs) — so there is nothing to unzip and nothing to `chmod`.

| Platform | File | Installs to |
| --- | --- | --- |
| Windows x64 | `gaia-<version>-win-x64-setup.exe` | `%LOCALAPPDATA%\Programs\GAIA` |
| macOS Apple silicon | `gaia-<version>-darwin-arm64.pkg` | `/usr/local/bin` |
| macOS Intel | `gaia-<version>-darwin-x64.pkg` | `/usr/local/bin` |
| Linux x64 (Debian/Ubuntu) | `gaia_<version>_amd64.deb` | `/usr/bin` |
| Linux x64 (Fedora/RHEL) | `gaia-<version>.x86_64.rpm` | `/usr/bin` |
...
already-open one keeps the PATH it started with.
...
`pkgutil --forget` drops only the installation receipt and deletes nothing, which
is why the `rm` comes first. `~/.gaia` is untouched either way.
...
unidentified developer; open it from Finder's right-click menu to proceed.
