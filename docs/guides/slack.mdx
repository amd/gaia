...
<Warning>
  **This bridge drives the real agent, not a chat-only session.** Unlike the
  ...
  not ask.** Whoever is on the allowlist can read the files the agent can read — your home
  directory, by default — and your indexed documents.
</Warning>
...
It also **serves exactly one member**. The bridge drives a single agent with a single
conversation, so a second member would share the first's history — and could ask what they
read — along with every "Always allow" they granted.
...
| `files:read` | Download a document or image you share, so the agent can open it |
...
- **Share a file** and it is saved to `~/.gaia/slack/inbox/`, and the agent is told where,
  so it can open or index it before answering.
- **Ask for a file back** and a file the agent writes — with `write_file`, `edit_file`, or
  another file-writing tool — under an upload root (your home directory by default) is
  uploaded to the thread once the write succeeds.
...
- **If the agent asks you a question mid-answer**, the question is posted and that answer
  ends there — the bridge cannot pause for a reply. Answer with a normal message and the
  agent picks up from the same conversation.
...
A prompt nobody answers within **ten minutes** is denied, and so is one Slack refuses to
post. The agent would otherwise wait forever, and it is the only agent this bridge has — so
one unanswered prompt would block every later message.
...
| `gaia slack start --allowed-users <id>` | Run the bridge (blocks until Ctrl-C) |
| `gaia slack stop` | Stop a bridge started with `--background` |
| `gaia slack status` | Show detection, configuration, and liveness |
| `gaia slack decline [--never]` | Record that you don't want Slack set up — `--never` stops the TUI offering it again |
