---
title: "v0.23.0"
description: "The Agent UI gets in-app install, auto-update, and a first-run wizard so a new user never touches a terminal; the email agent gains a full-autonomy engine that files low-signal mail on its own while always confirming anything destructive; Outlook now covers work/school accounts with device-code sign-in; and every agent runs as its own daemon-supervised process instead of living inside the UI."
---
