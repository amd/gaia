...
Every release PR must update **5 files**. Using `v0.17.0` as an example:
...
#### 4. `hub/components/{terminal-hub,agent-ui}/gaia-agent.yaml` — bump `version`

`release_components.yml` gates a tag against both manifests — publishing under a stale `version` fails the release, since R2 paths are immutable.

#### 5. `src/gaia/apps/webui/package.json` and `package-lock.json` — bump the UI version

Run `node installer/version/bump-ui-version.mjs` after `src/gaia/version.py` is updated; it reads the new version from there and rewrites both files.
