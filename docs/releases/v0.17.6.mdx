...
- **Windows installer failed at `ensure-uv` for users without `uv` on PATH** (PR [#968](https://github.com/amd/gaia/pull/968), closes [#966](https://github.com/amd/gaia/issues/966)) — No `uv` binary was ever bundled for `win-x64`, so the installer hard-failed on a clean machine. Windows builds now ship the pinned `uv` binary in `vendor/uv/win-x64/`, `backend-installer.cjs` resolves and invokes the packaged binary, and a packaged Windows rescue installer is included for recovery from a broken state.
