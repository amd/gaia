...
Now let's build this agent incrementally using **a single file** that grows with each step.

<Note>
**Building Strategy:** You'll create ONE file called `hardware_advisor.py` and progressively add tools to it. Each step builds on the previous one.
</Note>

---

### Step 1: Create the Basic Agent Skeleton

Start by creating the file with the basic agent structure (no tools yet).
...
<Tabs>
  <Tab title="Implementation">
    ...
    Always check real hardware first - never guess specifications."""

        def _register_tools(self):
            # No tools yet - we'll add them in the next steps
            pass
  ...
  <Tab title="What You Have">
    ...
    - ✗ No tools yet (agent cannot check hardware)
    - ✗ Cannot run yet - tools needed
    </Check>
  </Tab>
</Tabs>

<Warning>
**Don't run this yet!** This is just the skeleton. The agent has no tools, so it can only give generic advice. Continue to Step 2 to add hardware detection.
</Warning>

---

### Step 2: Add Hardware Detection Tool

Now add the `get_hardware_info` tool to detect system specifications.

<Tabs>
  <Tab title="Implementation">
    **Replace the `_register_tools()` method** in `hardware_advisor.py` with:
  ...
  <Tab title="Run">
    ...
    **Then ask:** "How much RAM do I have?"
  </Tab>
  ...
  <Tab title="What You Have">
    ...
    - ✓ get_hardware_info tool
    - ✓ Can detect RAM, CPU, NPU
    - ✗ No model catalog access yet
    </Check>
  </Tab>
</Tabs>
...
### Step 3: Add Model Catalog Tool

Add the `list_available_models` tool to query the model catalog.
...
<Tabs>
    ...
    ```python
        ...
        def list_available_models() -> Dict[str, Any]:
            ...
            try:
                ...
                return {"success": True, "models": enriched_models, "count": len(enriched_models)}
            except Exception as e:
                return {"success": False, "error": str(e)}
    ```
  </Tab>
  ...
  <Tab title="Run">
    ...
    **Then ask:** "What models are available?"
  </Tab>
  ...
  <Tab title="What You Have">
    <Check>
    - ✓ Hardware detection (get_hardware_info)
    - ✓ Model catalog access (list_available_models)
    - ✗ Cannot recommend models yet
    </Check>
  </Tab>
</Tabs>
...
### Step 4: Add Smart Recommendations Tool

Add the final `recommend_models` tool to complete the agent.
...
<Tabs>
    ...
    ```python
        ...
        def recommend_models(ram_gb: float, gpu_memory_mb: int = 0) -> Dict[str, Any]:
            ...
            try:
                ...
                max_model_size_gb = ram_gb * 0.7  # 70% rule
  ...
  <Tab title="Run">
    ...
    **Then ask:** "What size LLM can I run?"
  </Tab>
  ...
  <Tab title="Output Example">
    ...
    [Tool: recommend_models]
    {"max_model_size_gb": 22.4, "recommendations": [...]}
  ...
  <Tab title="What You Have">
    ...
    - ✓ Hardware detection (get_hardware_info)
    - ✓ Model catalog access (list_available_models)
    - ✓ Smart recommendations (recommend_models)
    - ✓ **Complete Hardware Advisor Agent!**
    </Check>
  </Tab>
</Tabs>

<Info>
**Next:** Your agent is now functionally complete! For the production-ready version with GPU detection and an interactive CLI, see [`examples/hardware_advisor_agent.py`](https://github.com/amd/gaia/blob/main/examples/hardware_advisor_agent.py).
</Info>
