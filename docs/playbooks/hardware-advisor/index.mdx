...
### Step 1: Agent Skeleton (Not Runnable Yet)

Start by creating the file with a minimal agent structure—just the class and a basic system prompt.
...
### Step 2: GPU & Hardware Detection (Runnable)

Add the `_get_gpu_info()` helper method and the full `get_hardware_info()` tool with GPU object, NPU object, and OS field.
...
### Step 3: Model Catalog (Runnable)

Add the `list_available_models()` tool with `labels` field and `message` in the response.
...
### Step 4: Smart Recommendations (Runnable)

Add the complete `recommend_models()` tool with `estimated_runtime_gb`, `fits_in_ram`, `fits_in_gpu`, and `constraints` object.
#### Implementation
...
### Step 5: Interactive CLI (Runnable)

Replace the simple `__main__` block with a full interactive CLI function.
#### Implementation
...
Your `hardware_advisor.py` should now match `examples/hardware_advisor_agent.py` exactly (except for the copyright header).
#### Verify Structure

**Your file should have:**
1. ✓ Public imports: `from gaia import Agent, tool`
2. ✓ `__init__` with `max_steps = 50`
3. ✓ Full 8-line system prompt
4. ✓ `_get_gpu_info()` helper (70 lines)
5. ✓ `_register_tools()` with `client = self.client` and `agent = self`
6. ✓ Full `get_hardware_info()` with GPU object, NPU object, OS field
7. ✓ Full `list_available_models()` with `labels` and `message`
8. ✓ Full `recommend_models()` with `estimated_runtime_gb`, `fits_in_ram`, `fits_in_gpu`, `constraints`
9. ✓ Full `main()` function with interactive CLI
10. ✓ `if __name__ == "__main__": main()`
#### Compare Files

**Run diff to verify:**
```bash
diff hardware_advisor.py examples/hardware_advisor_agent.py
```

**Expected output:** Only copyright header difference (lines 1-18)
#### Test Both
...
Both should produce identical interactive sessions.
#### Complete!
...
<Info>
**Congratulations!** You've built a production-ready hardware advisor agent that matches the official example. Your implementation includes GPU detection, smart memory calculations, and an interactive CLI—all running locally on AMD hardware.
</Info>
