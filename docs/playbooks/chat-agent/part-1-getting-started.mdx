  ...
  <Step title="Set up your project">
    ...
    <Tabs>
      <Tab title="Core + ChatAgent Source">
        ...
        <Info>
        Create your agent scripts in this folder. The standalone `gaia-agent-chat` wheel provides `ChatAgent`; its PyPI publishing is paused, so install it from the repository subdirectory. GAIA has no `agent-chat` extra. The `rag` extra adds document-processing dependencies (faiss, PyMuPDF).
        </Info>
      </Tab>
