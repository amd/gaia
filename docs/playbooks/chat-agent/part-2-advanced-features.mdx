...
save_current_session()
  → Serialize to JSON
  → Write atomically to ~/.gaia/sessions/{id}.json (temp file + rename)
  → Include metadata (timestamp, version)
```

Sessions live under `~/.gaia/sessions` (or `$GAIA_CONFIG_DIR/sessions`) no matter which directory you start from. A session id must be 1–128 characters from `A-Z a-z 0-9 . _ -`; anything else raises `ValueError`.

**Load process:**
```python
load_session(session_id)
  → Read JSON from disk (SessionCorruptError if the file is truncated or invalid;
...
<Tabs>
  ...
  <Tab title="Trade-offs">
    **`use_llm_chunking`:**
    - `True` - Semantic boundaries, more accurate, slower; indexing fails with an error if the LLM is unreachable or can't produce split points
