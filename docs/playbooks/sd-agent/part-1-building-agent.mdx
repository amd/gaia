...
```python
    ...
    def _register_tools(self):
        ...
        def create_story_about_image(image_path: str, story_style: str = "adventure") -> dict:
            """
            Create a story about an image with metadata tracking.

            Wraps VLMToolsMixin's generic create_story_from_image() to add:
            - Defaults to adventure style (but accepts other styles)
            - Adds custom metadata for tracking
            - Could extend with: DB saving, markdown formatting, etc.

            Args:
                image_path: Path to the image file
                story_style: Story style (default: adventure). Options: whimsical, dramatic, adventure, educational
            """
            # Call VLM mixin's internal method with specified or default style
            result = self._create_story_from_image(image_path, story_style=story_style)
