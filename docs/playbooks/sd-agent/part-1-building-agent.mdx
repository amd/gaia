...
|-------|------|----------|----------|---------|
| **Qwen3-8B-GGUF** | Reasoning engine | `POST /v1/chat/completions` | NPU/GPU | 8K tokens |
| **SDXL-Turbo** | Image generation | `POST /images/generations` | GPU | N/A |
| **Qwen3-VL-4B-Instruct-GGUF** | Vision analysis | `POST /v1/chat/completions` (with images) | NPU/GPU | 8K tokens |
...
**Model formats:** LLMs use GGUF quantization (Qwen3-8B, Qwen3-VL-4B), SD uses safetensors (SDXL-Turbo). All optimized for AMD hardware acceleration (NPU/GPU). Total memory: ~15GB loaded.
...
<Steps>
  ...
  <Step title="Initialize SD profile">
    ...
    All models are optimized for AMD hardware (NPU/GPU acceleration).
...
def __init__(self, output_dir="./generated_images"):
    # Initialize Agent base class first
