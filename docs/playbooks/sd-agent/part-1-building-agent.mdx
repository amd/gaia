...
```python
    ...
    def _register_tools(self):
        # Add custom tools that build on mixin capabilities
        # This demonstrates the wrapper pattern: custom tools using mixin tools
        from gaia.agents.base.tools import tool

        @tool(name="create_story_about_image")
        def create_story_about_image(image_path: str) -> dict:
            """
            Create a story about an image with custom formatting.

            This wraps VLMToolsMixin's create_story_from_image with custom behavior.
            """
            # Use VLM mixin's tool to get the story
            result = self._create_story_from_image(image_path, story_style="adventure")

            # Add custom formatting or metadata
    ...
    def _get_system_prompt(self) -> str:
        # Extend SD mixin prompts with custom instructions
        # The base SD prompt teaches proper image generation
        # We add instructions about our custom story tool
        return """
Custom tool available:
- create_story_about_image: Creates adventure-style stories with custom metadata

Use generate_image from SD mixin for images.
Use analyze_image from VLM mixin for descriptions.
Use create_story_about_image (custom) for adventure stories."""
```

**What's happening with system prompts:**

The Agent base class automatically composes the final system prompt:
1. Calls `_get_mixin_prompts()` → Gets `[sd_prompt]` from SDToolsMixin
2. Calls `_get_system_prompt()` → Your custom additions (we just return the SD prompt here)
3. Combines them into the final prompt sent to the LLM

You can also extend instead of replace:
```python
def _get_system_prompt(self) -> str:
    return ""  # Use only SD mixin prompt (automatic)

# OR add custom instructions:
def _get_system_prompt(self) -> str:
    return "Additional: Always save metadata to logs"  # Appended to SD prompt
```
