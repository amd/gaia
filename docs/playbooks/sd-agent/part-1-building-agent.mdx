...
def create_story_about_image(image_path: str) -> dict:
    ...
    # 1. Use mixin client directly
    path = Path(image_path)
    if not path.exists():
        return {"status": "error", "error": f"Image not found: {image_path}"}

    image_bytes = path.read_bytes()
    prompt = "Create a short adventure story (2-3 paragraphs) that is exciting with action and discovery. Bring the image to life with narrative."
    story = self.vlm_client.extract_from_image(image_bytes, prompt=prompt)
...
**Pattern:**
1. Use mixin clients directly (`self.vlm_client`, `self.sd_client`) to access core capabilities
2. Add custom behavior: metadata, logging, database operations, formatting
3. Return the modified result

**Why use clients?** Mixins provide clients (`self.vlm_client`) that expose the underlying APIs. This lets you build custom tools without duplicating mixin tool logic.
...
def create_and_catalog_image(prompt: str, tags: list[str]) -> dict:
    ...
    # 1. Use SD client to generate
    img_data = self.sd_client.generate_image(prompt=prompt, model=self.sd_default_model)
    if not img_data or "image" not in img_data:
        return {"status": "error", "error": "Image generation failed"}

    # Save image
    image_path = self.sd_output_dir / f"{prompt[:30].replace(' ', '_')}.png"
    image_path.write_bytes(img_data["image"])

    # 2. Use VLM client to analyze
    analysis_prompt = "Describe this image in detail, focusing on composition, subjects, and visual elements."
    description = self.vlm_client.extract_from_image(img_data["image"], prompt=analysis_prompt)

    # 3. Custom: Save to database
    self.execute_sql(
        "INSERT INTO images (path, prompt, tags, description) VALUES (?, ?, ?, ?)",
        (str(image_path), prompt, ",".join(tags), description)
    )
...
This pattern shows the power of composition: mixin clients + custom orchestration.
...
🎉 **Congratulations!** You've built your first multi-modal agent.
