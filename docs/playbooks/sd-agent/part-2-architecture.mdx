...
1. **User input** → Agent receives text query or task
2. **LLM reasoning** → Agent's LLM decides what to do (which tools to call, what parameters to use)
3. **Tool execution** → Tools run (generate image, analyze image, etc.)
4. **Tool results** → Results fed back to LLM
5. **LLM response** → Agent synthesizes final answer
...
graph TB
    User("👤 User: 'robot kitten + story'")
    Agent("ImageStoryAgent<br/>(Agent + SDToolsMixin + VLMToolsMixin)")
    LLM("🧠 LLM Reasoning<br/>(Qwen3-8B)")

    User --> Agent
    Agent --> LLM

    LLM --> SDMixin("SDToolsMixin")
    SDMixin --> GenImg("generate_image()<br/>→ SDXL-Turbo via Lemonade")
    GenImg --> ImgSaved("🖼️ Image saved to disk")

    LLM --> VLMMixin("VLMToolsMixin")
    VLMMixin --> CreateStory("create_story_from_image()<br/>→ Qwen3-VL-4B via Lemonade")
    CreateStory --> StoryResult("📖 Story generated")

    ImgSaved --> Result("✅ Final Result")
    StoryResult --> Result
    Result --> User
...
- `get_vlm_system_prompt()` - Returns VLM usage guidelines
...
├── Defines create_story_from_image() with @tool decorator
├── Defines answer_question_about_image() with @tool decorator
└── Tools auto-registered via @tool decorator

_register_tools() (Your agent)
└── Define custom tools with @tool (optional)
...
def _get_mixin_prompts(self) -> list[str]:
    ...
    # Modify SD prompt to use serious tone
...
See what prompt the LLM receives:

```python
# View final composed prompt
print(agent.system_prompt)

# View mixin prompts separately
print(agent._get_mixin_prompts())

# View agent custom separately
print(agent._get_system_prompt())

# Enable prompt display on startup
agent = ImageStoryAgent()
agent.show_prompts = True  # Prints system prompt during initialization
```
...
<Card title="Part 3: Advanced Patterns & Variations →" icon="wand-magic-sparkles" href="/playbooks/sd-agent/part-3-variations">
