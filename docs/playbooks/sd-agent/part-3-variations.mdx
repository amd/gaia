...
<AccordionGroup>
  ...
  <Accordion title="Agent repeats same action multiple times">
    ...
    **Solution:** Set `max_steps` lower in Agent init:
    ```python
    super().__init__(max_steps=5)  # Default is 20 for base Agent, 10 for SDAgent
    ```
...
<Steps>
  ...
  <Step title="Tool composition through custom tools">
    How to write domain-specific tools that use mixin clients directly (`self.vlm_client`, `self.sd_client`) to build specialized features. This keeps mixins reusable while enabling custom workflows.
  </Step>
