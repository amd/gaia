...
both and every value both state occurs at a shared source offset. The retained
extraction may fill fields the other left `not stated`, but not the reverse at
the same time, and a final sentence period is the only tolerated value
difference. Two quotes that do not overlap are never merged into one entry.
Conflicting evidence is retried once per page, then fails visibly.
