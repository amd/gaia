...
inventory to the final answer. A final model summary cannot drop retained entries. Repeated
names need distinctive quotations; ambiguous or conflicting evidence fails
explicitly. Reading every page without extracting an inventory is insufficient.
...
source text visible, aligns to whole lines or sentences, and preserves verbatim
field values. Quotes and values are matched ignoring case and spacing (captions
use non-breaking and doubled spaces), but a match must fall on whole words and the
stored quote is always the source's own text at its exact offsets.

An item is located by its identity value (a field named for a `name`, `id` or
`title`, such as `exercise_name`; else the first requested field), not by its
whole quote, so neighbouring items' quotes may overlap. Two extractions are one
item when their identity values sit at the same source span; other values must
sit at one location, one inside the other (a page boundary may cut a description
short, never a name), and the merged entry keeps the fuller value of each field.
When a quote names its item twice, an omission pass takes the occurrence no
earlier item claimed. Two items from one reply are never merged. Conflicting
evidence is retried once per page, naming the conflicting quote, then fails
visibly.
