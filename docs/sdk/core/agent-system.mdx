...
are escaped). Saved results must be read back and contain all retained entries
before completion, and hand edits to a file it wrote this turn are refused. Every requested source
...
paraphrase; the verbatim quote is their evidence. An item with no stated field is
rejected.
