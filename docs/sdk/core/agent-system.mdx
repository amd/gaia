...
`"delegate"` in `KNOWN_TOOLS`). The flagship agent ships it off; turn it on with
`GaiaAgentConfig(delegate_enabled=True)` or `GAIA_DELEGATE=1`, and size the
child's budget with `delegate_max_steps` (default 40) or
`GAIA_DELEGATE_MAX_STEPS`. When on, it is always offered, whatever the dynamic
tool selection picks for the turn.
...
- `steps`, `tool_calls`, `tokens` (`input`, `output`, `cached`), `hit_step_limit`.
