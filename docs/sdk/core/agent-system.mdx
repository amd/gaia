...
a save from an earlier session is not a claim about this turn.
...
path. Active extraction skills can activate it. Sources are the files a request
reads from; a path after "to" or "into" is an output, not a source.
...
when an identity field (a field named with `name`, `id` or `title`, else any
field) states the same value at a shared source offset, and every other value
