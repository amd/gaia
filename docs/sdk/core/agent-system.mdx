...
**What the user gets:** at `max_consecutive_repeats` identical calls the model gets one correction in place of the call, and a repeat after that ends the turn. If the calls that ran worked, the agent makes one more model call with no tool calls allowed and answers from their results; if they errored or were refused, the answer names that failure.
...
or a `run_python` / `execute_python_file` output with a test runner's summary
(pytest, unittest, jest, vitest, mocha, `go test`, `cargo test`). Other
unrecognized runners remain `unverified`.
