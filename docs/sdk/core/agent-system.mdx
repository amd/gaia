...
one reply are never merged, and when a quote names its item twice an omission
pass takes the occurrence no earlier item claimed. Evidence too ambiguous to
merge keeps both entries, and the rendered inventory marks the later one
"(may repeat item N)" for review. Only an unusable reply (not JSON, a quote not
