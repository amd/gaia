...
Local thresholds/targets are 30,000/20,000 characters for NPU or an unspecified
device and 60,000/40,000 for GPU/CPU; the Claude path uses its existing larger
window. These are per-result character budgets, not measured remaining tokens.
The omitted middle is not yet recoverable through a continuation tool. Existing
structured-result exceptions and tool-specific caps remain separate.
