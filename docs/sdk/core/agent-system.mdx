...
Shown holds the closing part when the result marks it a lead (a run's summary
line), then the first part, then the other parts marked as leads or named by
the call's arguments, then the rest in order, while they fit. Text with no structure
...
def tool(func):
    ...
    #    Each argument carries its own line so the model reads it at the slot it
    #    fills; the Args: block is then dropped from `description` (see below).
    ...
    _TOOL_REGISTRY[func.__name__] = {
        "name": func.__name__,
        "description": schema_description(func.__doc__),  # docstring, minus Args:
        "parameters": params,
        "function": func,
        "atomic": False,
    }
