...
a transcript or screenshot written elsewhere cannot satisfy it. Explicit save
requests are checked even when the answer silently omits the save. Negated
instructions and how-to questions do not create save obligations.

Text deliverables must be read back after their latest mutation. A read before
...
turn; a general statement that everything was cleaned up is not proof.
...
returns `status: "incomplete"` with `completion_gaps`. A blocked save cannot be
...
checks the source fingerprint again before completion, and renders the accumulated
inventory directly. A final model summary cannot drop retained entries. Repeated
...
source items. General summaries, advice about extraction, and negated enumeration
requests do not activate this path. Active extraction skills can activate it.
...
when an identity field (`name`, `id` or `title`, else any field) is stated by
both and every value both state sits at one source location, one inside the
other (as when a page boundary cut a value short). The merged entry's quote spans
both quotes and keeps the fuller value of each field. Two items from one reply,
or quotes that do not overlap, are never merged. Conflicting evidence is retried
once per page, naming the conflicting quote, then fails visibly.
