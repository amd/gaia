...
result the parent sees stays under 4,000 characters:

- `result` — the child's answer; a long one is elided and kept whole in the transcript.
- `evidence` — `files_changed` (from `git status` before and after, or a
  modification-time scan outside a repository), `commands_run` (up to 20, with
  the total), and `tests` (the child's test, lint and build runs).
- `steps`, `tool_calls`, `tokens` (`input`, `output`, `cached`), `hit_step_limit`.
- `transcript` — an artifact handle for the child's full conversation, readable
