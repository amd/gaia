...
def process_query(self, user_input, max_steps=None):  # defaults to self.max_steps (20)
    ...
    # 9. Max steps reached: one more call with no tools, so the agent says what
    #    it found, what it couldn't finish and why, and what to try next
    answer = self.llm.generate(prompt + STEP_LIMIT_INSTRUCTION, tools=None)
