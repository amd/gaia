...
`extract_document_items(file_path)`. It reads a permission-checked snapshot and
processes overlapping 4,000-character chunks sequentially, with an additional
omission check per chunk. Each call has isolated context and no tools. Requested
fields written as `fields: name, reps, cue` must appear on every extracted entry;
missing source facts are marked as not stated.
...
quotes, with a 30-minute extraction deadline (plus the enclosing tool timeout).
...
turn cap only for extraction answers (maximum 256,000 characters, explicit error
above that bound). Follow-ups can retrieve those observations through existing
...
when every value both state sits at the same source offset; a fuller extraction
may fill fields the other left `not stated`, and a final sentence period is the
only tolerated value difference. Conflicting evidence fails visibly.
