...
the NPU budget. A cloud model reached through Lemonade uses 8,000/6,000: its
limit is the cost of re-sending every result on every later step, not its
context window. The Claude path uses its existing larger window. These are
