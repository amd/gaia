...
changed an output invalidates earlier evidence. A file that a successful
executor modified during the turn, such as `ls > listing.txt`, counts as written
once it is read back. Cached transcripts are not new writes.
