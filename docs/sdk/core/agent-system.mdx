...
a save from an earlier session is not a claim about this turn. An answer's save
claims are checked only when the user asked for a save or a file-writing tool
ran, so "downloads are saved to ~/Downloads" stays an answer.
...
changed an output invalidates earlier evidence. Paths computed dynamically and
not already tracked require an explicit destination in the request or a file
write tool that returns its destination. Cached transcripts are not new writes.
