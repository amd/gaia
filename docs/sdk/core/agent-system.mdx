...
- `evidence` — `files_changed` (from `git status` before and after, or a
  modification-time scan outside a repository), `commands_run` (up to 12,
  with the total, minus the shell tool's `cd <workdir> &&` prefix), and
  `tests` (the child's test, lint and build runs).
