...
- `index` — one `{label, offset, length}` per part left out, in document order.
  `read_tool_output(artifact, offset, length)` returns exactly that part.
...
`archived_field`, or JSON (one record per line) for a structured result. Pass
an `index` entry's `offset` and `length` as `offset` and `limit` to read exactly
that part; an entry longer than a page returns `remaining` and continues at
`next_offset`. Small results remain unchanged. Structured excerpts retain whole
records.
