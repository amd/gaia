...
<Warning>
  ...
  Neither opt-in widens `run_shell_command`: a command off its read-only list
  (`npm test`, `git commit`) is refused under either one, where an interactive
  run would have asked. Only the TUI's full access runs those unasked.
</Warning>
