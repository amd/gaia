...
`description`), and a `{type, required, description}` entry per parameter. The
type comes from the annotation (`str`, `int`, `float`, `bool`, `tuple`, `set`,
`dict`, plus generics like `List[str]` and `Optional[...]`); an unannotated or
unrecognised parameter falls back to `"unknown"`. The per-parameter
`description` is parsed from the docstring's Google-style `Args:` block, so the
model reads the constraint at the slot it is filling — write one line per
argument. Parameter **default values** are still not stored; `required` is
derived from whether a default exists.
</Note>
...
def tool(func=None, *, atomic: bool = False, display_label: str | None = None,
    ...
    def decorator(f):
        ...
        _TOOL_REGISTRY[f.__name__] = {
            "name": f.__name__,
            "description": f.__doc__ or "",  # whole docstring
            "parameters": params,
            "function": f,
            "atomic": atomic,
...
- The full docstring is stored once as `description`, **and** each argument's
  line from the `Args:` block is stored on that argument's entry. Wrapped
  continuation lines are joined into one string. An argument with no `Args:`
  entry simply has no `description` key.
