...
{
    "name": "get_weather",
    "description": "Get current weather for a city.\n\nUse this tool when ...",  # docstring, minus Args:
    "parameters": {
    ...
    "preflight": None,      # optional check run before the confirmation prompt
}
```

<Note>
**What's actually captured:** The function name, the docstring **without its
`Args:` block** (as `description`), and a `{type, required, description}` entry
per parameter. The type comes from the annotation (`str`, `int`, `float`,
`bool`, `tuple`, `set`, `dict`, plus generics like `List[str]` and
`Optional[...]`); an unannotated or unrecognised parameter falls back to
`"unknown"`. The per-parameter `description` is parsed from the docstring's
Google-style `Args:` block, so the model reads the constraint at the slot it is
filling — write one line per argument, and don't repeat it in the prose.
Parameter **default values** are still not stored; `required` is derived from
...
billed on each step of each turn. Two caps, defined in
`src/gaia/agents/base/tools.py`:

| Field | Budget |
|-------|--------|
| `description` (the docstring, minus `Args:`) | 400 characters |
| Each parameter description (one `Args:` entry) | 160 characters |

`python util/lint.py --tool-descriptions` fails the build when a tool exceeds
either, naming the tool and its length; `python util/tool_schema_tokens.py`
shows where the characters go.

Keep what the model needs to call the tool correctly — what it does, when to
use it instead of a neighbouring tool, what it refuses, what it returns.
Rationale, history, and worked examples belong in a code comment, not the
schema.

<Warning>
**What the check does not cover.** It measures the flagship agent's tool
surface only: MCP tools are excluded, and a hub agent outside the flagship is
unchecked — so a packaged agent can still ship a multi-thousand-character
description without failing lint. The flagship skeleton also picks up whatever
the host environment offers, so the tool count it sees can differ between a dev
box and CI. Treat a green run as "the flagship is in budget", not "every tool
in the repo is".
</Warning>
...
def tool_name(param: type) -> return_type:
    ...
    Args:                                            ← Reaches the model per-arg
...
When you decorate a function with `@tool`, Python introspection extracts the
schema (see `src/gaia/agents/base/tools.py` for the authoritative
implementation):
...
def tool(func=None, *, atomic: bool = False, display_label: str | None = None,
         timeout: float | None = None, preflight=None):
    """Decorator that registers a function in the global tool registry."""

    def decorator(f):
        sig = inspect.signature(f)
        # Google-style "Args:" block -> {arg_name: description}
        arg_descriptions = _parse_arg_descriptions(f.__doc__)
        params = {}

        for name, param in sig.parameters.items():
            info = {
                # Unwraps Optional[X] / X | None and generics (List[str] -> array)
                "type": _infer_param_type(param.annotation),
                "required": param.default is inspect.Parameter.empty,
            }
            if arg_descriptions.get(name):
                info["description"] = arg_descriptions[name]
            params[name] = info

        _TOOL_REGISTRY[f.__name__] = {
            "name": f.__name__,
            # Cleaned docstring, minus the Args: block (it ships per-parameter)
...
- `description` is the docstring with its leading indentation and its `Args:`
  block removed — each argument's line is stored on that argument's entry
  instead, so the same text is not billed twice per call. Wrapped continuation
  lines are joined into one string. An argument with no `Args:` entry simply
  has no `description` key. Sections after `Args:` (`Returns:`, `Raises:`, …)
  stay in `description`.
...
- Generic annotations are specialised: `list[str]` / `List[str]` → `array`,
  `Dict[str, Any]` → `object`, and `Optional[X]` / `X | None` resolves to `X`'s
  type. An unannotated parameter (or a union of two real types) stays
  `"unknown"`.
...
- `@tool(preflight=...)` takes a function of the call's arguments that returns
  the refusal the call has already earned, or `None`. The agent runs it before
  the confirmation prompt, so nobody is asked to approve a call that is going to
  be refused. The tool body must enforce the same rule itself; the file-editing
  tools use this for their read-before-edit check.
- Unsupported decorator keywords raise `TypeError` at decoration time and
  identify the invalid argument, tool name, and accepted keyword set.
