...
8. `start_skill_synthesis()` — distill repeated workflows into procedural memory, on a background thread
...
Skill synthesis is the one step that does **not** run inline: `start_skill_synthesis()` hands it to a background thread, because a distillation call is seconds of model time and would otherwise land in front of the user's first answer. Each pass reads only the history no earlier pass consumed (a watermark in the `meta` table plus per-episode marks in `synthesis_marks`), and spends at most `max_distill_calls_per_pass` model calls — 2 by default — leaving the rest for the next session. `wait_for_skill_synthesis(timeout)` joins the pass; `_synthesize_skills(force=True)` or `MemoryStore.reset_synthesis_progress()` re-opens history it already consumed.

A stored procedure's `success_count` is its **lineage's** track record, not one pass's. Each pass adds only the episodes the matched row was not built from, so a later batch can still supersede a stale procedure, and re-reading history — a forced pass, or the first pass after a reset — counts each session once.
...
Preferences:
  - tone: professional but friendly (learned 2026-02-11)
  - code_style: black formatter, 88 char lines (learned 2026-01-20, last confirmed 2026-03-02)

Known facts:
  - Project uses React 19 with app router (confidence: 0.82, learned 2026-03-01)
  - User's name is Alex, role is tech lead (confidence: 0.95, learned 2026-01-20)

Skills:
  - Deploy workflow: test → build → push → verify (confidence: 0.88, learned 2026-02-02)
  - Docker compose: always use --build flag on first run (confidence: 0.72, learned 2026-02-15)
  - Git bisect: use binary search for regression hunting (confidence: 0.65, learned 2026-03-10)

Known errors to avoid:
  - execute_code: "import torch" fails -- torch not installed on this machine (learned 2026-03-12)
  - pip install: always use --index-url for PyTorch packages (learned 2026-03-12)
```

Each personal item carries the absolute date it was learned and, if it was later stored again or updated, the date it was last confirmed. The dates are absolute rather than "3 days ago" so the prompt stays byte-stable for the session; the per-turn context carries the current time.
...
Returns the **per-turn dynamic context** that is prepended to the user message each turn. Contains the current time, upcoming/overdue items, and the stored memories most relevant to the user's message.
...
Stored memories that may bear on this message:
  - [note] staging deploys need VPN on (learned 2026-02-03, last confirmed 2026-03-20)
```

Always returns at least the current time.

**Relevant memories** come from a vector search on the user's message: at most `TURN_RECALL_TOP_K` (3) items of category fact, preference, note, skill or error, each with cosine similarity of at least `TURN_RECALL_MIN_SIMILARITY` (0.5). Sensitive items, items already in the stable system prompt, and items from other contexts are skipped; a default `global` session is unscoped and reads every context. Credential-shaped content is masked. Surfacing a memory here does not bump its confidence or use count. If the embedding call fails, the section is left out and a warning is logged. Returns an empty string only if `init_memory()` has not been called.
...
Sends the conversation turn, this turn's tool record (each call, its arguments, whether it succeeded, failed or was refused, and a short result), and existing memory to the LLM, which returns a JSON array of operations:

| Operation | Description | Required fields |
|-----------|-------------|-----------------|
| `add` | New knowledge not already in memory | `op`, `category`, `content`, `grounded`, optional: `entity`, `domain`, `confidence` (default 0.4) |
| `update` | Modify existing item (correction, enrichment, supersession) | `op`, `knowledge_id`, `content`, `grounded`, optional: `entity`, `domain`, `category` (an update naming `system`/`profile`/`permission` is dropped) |
| `delete` | Remove item contradicted or invalidated | `op`, `knowledge_id`, `reason`, `grounded` |
...
`grounded` must be `"user"` (the user said it) or `"tool"` (a tool result shows it). An op grounded in anything else, or citing a tool when no tool succeeded this turn, is dropped: the assistant's answer alone is not evidence.
...
Wraps every non-memory tool call to auto-log it to `tool_history`. If a tool runs and fails, the error is automatically stored as knowledge (`category="error"`) for future avoidance. A call refused before it ran (`executed: False` on the result, which includes a declined confirmation prompt) is neither stored nor treated as evidence the operation works — only a call that actually ran and succeeded clears errors stored for the same operation (the tool plus the program or path it acted on). Memory tools (remember, recall, etc.) are excluded from logging to avoid noise and recursion.

**Hook 3: `_after_process_query()` callback**

Called after `process_query()` completes. Stores both conversation turns (user + assistant) in the conversations table and runs Mem0-style LLM extraction (ADD/UPDATE/DELETE/NOOP operations against existing memory). For turns ≥ 5 words, the extraction pipeline fetches top-10 relevant existing items via `_hybrid_search()`, then asks the LLM, with the turn's tool record, to decide what operations to perform — no regex heuristic fallback.
...
<Card title="search_past_conversations" icon="clock-rotate-left">
  Search conversation history by keywords, time range, or both. Returns matching turns with timestamps and session IDs. An empty result carries `past_conversation_turns` (turns stored by earlier sessions) and a `message` that says plainly when there are none to search.
</Card>
...
| `_extract_via_llm(user_input, assistant_response, existing_items, tool_record)` | Mem0-style extraction: ADD/UPDATE/DELETE/NOOP |
