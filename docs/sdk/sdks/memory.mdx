...
| **Medium** | 8–20 words, or contains "how", "why", "explain" | 5 |
| **Complex** | > 20 words, or contains "compare", "across", "all", "history", "everything" | 10 |
...
def search(
    self,
    query: str,                     # Search query (used for both FTS5 and vector search)
    category: str = None,           # Filter by category
...
Uses hybrid search combining vector similarity (FAISS) and keyword matching (FTS5 BM25) via Reciprocal Rank Fusion (RRF), followed by cross-encoder reranking. FTS5 uses AND semantics by default with automatic OR fallback. Bumps confidence +0.02 on each recalled item. Filters on `superseded_by IS NULL` to return only current/active items.

The `time_from` and `time_to` parameters add temporal filtering on `created_at`, narrowing results before the vector/BM25 ranking pipeline.
...
Only provided fields are changed. Sets `updated_at` to the current time. The `superseded_by` parameter is used by the LLM extraction pipeline to mark old items as replaced by newer versions while preserving fact lineage.
...
def get_items_with_embeddings(
    ...
    # Used to build/rebuild the FAISS index.
...
def get_entities(self, limit: int = 100) -> List[Dict]
    ...
    # Returns: [{"entity": "person:sarah_chen", "count": 5}, ...]
...
| `search(query, category, ...)` | Hybrid search: vector + BM25 + RRF + cross-encoder (v2: adds `time_from`/`time_to`, `superseded_by IS NULL` filter) |
