...
| `embedding_model` | `Optional[str]` | Default memory embedder | Override the embedding model; its live response determines the vector dimension. |
...
These examples need the local embedding backend initialized by `gaia init`.
`init_memory()` can leave `memory_store` as `None` when memory is unavailable;
check it before relying on persistence. `Agent` accepts neither `name` nor
`system_prompt` constructor keywords: use class metadata and `_get_system_prompt()`.
Do not call its abstract `_register_tools()` via `super()`.
...
class RememberBot(MemoryMixin, Agent):
    """An agent with persistent memory tools."""
    ...
    def _get_system_prompt(self):
        return "Use memory tools to remember and recall useful preferences."
...
# The agent can save this with remember when memory is available.
# Verify with recall; this short prompt does not trigger automatic extraction.
```
...
class WorkPersonalAgent(MemoryMixin, Agent):
    def __init__(self, context="work", **kwargs):
...
# A remember tool call uses the active work context; verify its result.

agent.set_memory_context("personal")
agent.process_query("Remember: dentist appointment Thursday at 2pm")
# A remember tool call now uses the personal context; verify its result.
