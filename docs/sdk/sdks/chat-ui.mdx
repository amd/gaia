---
title: "Chat UI SDK"
description: "Python backend SDK for the GAIA Chat desktop application - FastAPI server, SQLite database, and Pydantic models"
icon: "desktop"
---

<Info>
  **Source Code:** [`src/gaia/chat/ui/`](https://github.com/amd/gaia/blob/main/src/gaia/chat/ui/)
</Info>

<Note>
**Import:**
```python
from gaia.chat.ui.server import create_app, DEFAULT_PORT
from gaia.chat.ui.database import ChatDatabase, DEFAULT_DB_PATH
from gaia.chat.ui.models import SystemStatus, ChatRequest, SessionResponse, DocumentResponse
```
</Note>

**See also:** [User Guide](/guides/chat-ui) | [Chat SDK](/sdk/sdks/chat) | [API Specification](/spec/chat-ui-server)

---

## Overview

The Chat UI SDK is the Python backend that powers GAIA Chat Desktop. It provides:
...
```bash
python -m gaia.chat.ui.server --port 4200
```
...
The persistence layer for all Chat UI data. Uses SQLite with WAL mode for concurrent read access.
...
All request and response bodies use Pydantic models from `gaia.chat.ui.models`.
...
<AccordionGroup>
  <Accordion title="GET /api/system/status">
    Check system readiness for the chat UI.
...
The Chat UI uses SQLite with four tables:
...
The Chat UI server delegates to the core [Chat SDK](/sdk/sdks/chat) for LLM communication:

```python
from gaia.chat.sdk import ChatSDK, ChatConfig

# The server creates a ChatSDK instance per request
config = ChatConfig(
...
GAIA Chat is also available as an npm package for quick installation:

```bash
npm install -g @amd/gaia-chat
```

This provides the `gaia-chat` CLI command:

```bash
gaia-chat              # Start Python backend + open browser
gaia-chat --serve      # Serve frontend only (Node.js static server)
gaia-chat --port 8080  # Custom port
gaia-chat --version    # Show version
```
...
| Path | Description |
|------|-------------|
| `bin/gaia-chat.mjs` | CLI entry point (Node.js) |
| `dist/` | Pre-built frontend (React SPA) |

### Release Management

The package version is managed via `version.txt` in the webui directory:

```bash
# Check version consistency
node scripts/bump-chat-version.mjs --check

# Sync package.json to version.txt
node scripts/bump-chat-version.mjs

# Full release (sync, commit, tag, push)
node scripts/release-chat.mjs
```

Tags matching `chat-v*` trigger the automated npm publish workflow.

---

## Related

- **[User Guide](/guides/chat-ui)** -- Desktop app usage, features, and troubleshooting
- **[Chat SDK](/sdk/sdks/chat)** -- Core conversation SDK with memory and RAG
