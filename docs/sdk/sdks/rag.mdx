...
flowchart TD
    ...
    style J fill:#A87B2D,stroke:#A87B2D,stroke-width:2px,color:#fff
    style K fill:#EFC480,stroke:#E2A33E,stroke-width:2px,color:#1a1a1a
    style L fill:#28a745,stroke:#1e7e34,stroke-width:2px,color:#fff

    linkStyle 0,1,2,3,4,5 stroke:#E2A33E,stroke-width:2px
```
...
3. Generates embeddings for all chunks. A short, empty, or dimensionally inconsistent batch is retried under the existing two-retry policy, then fails indexing if still invalid with an actionable error; partial vectors are never paired with different chunks.
...
| `500` | ~2-3 paragraphs | General documents, manuals (default) |
...
config = RAGConfig(
    ...
    embedding_model="user.embeddinggemma-300m-GGUF",  # Embedding model

    # === Storage ===
    # cache_dir=...,           # Where to store the index (default: ~/.gaia/cache/rag)
