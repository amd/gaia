...
<Note>
The default embedder is now `user.embeddinggemma-300m-GGUF` (EmbeddingGemma 300M), replacing `nomic-embed-text-v2-moe-GGUF` because the current llama.cpp server cannot load the nomic MOE embedder. Both are 768-dim, so retrieval behavior is unchanged. If you have an index built with the old model, it is re-embedded automatically on the next run — the changed model name invalidates the cached vectors under `cache_dir`.
</Note>
