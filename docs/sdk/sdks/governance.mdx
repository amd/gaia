...
```python
# pip install "acgs-lite>=2.12.0,<3.0"   (or: pip install "amd-gaia[acgs]")
...
is not treated as policy. `[acgs]` installs published `acgs-lite` 2.12.0+,
which ships `acgs_lite.integrations.gaia`. A missing or too-old wheel
fails closed with an install hint — the stub is never substituted silently.
