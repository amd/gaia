...
| `REVIEW` | A checkpoint is opened and your `governance_reviewer` callback is called. APPROVE → tool runs; REJECT → tool is refused. Either way a receipt is written. |

If `REVIEW` fires and no reviewer is set, the mixin **fails closed** — the tool is
denied without executing.
...
GAIA's default `AgentConsole.confirm_tool_execution` is **not** consulted
automatically because its default implementation auto-approves. Pass an explicit
`governance_reviewer` that actually blocks.
