...
`default()` is the supported path today (in-repo **stub**, risk tags only).

`from_acgs_lite()` is the constitution-backed swap this layer was written
for. It wraps the inner engine in a GAIA-owned risk-tag floor (`blocked` /
`review` cannot be loosened; `GAIA_AUTO_APPROVE_TOOLS` is not policy) and
fail-closes if the adapter module is missing. It is **not a supported
install path yet**: published acgs-lite 2.11.0 does not ship
`acgs_lite.integrations.gaia`, and there is no `amd-gaia[acgs]` extra
until a wheel that includes that module is on PyPI. Keep using `default()`
until then.
