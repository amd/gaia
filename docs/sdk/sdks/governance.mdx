...
`default()` is the in-repo **stub** (risk tags only). For a constitution-backed
engine, swap in ACGS-lite — the Protocol this layer was written for:

```python
# pip install "acgs-lite>=2.11.0,<3.0"   (or: pip install "amd-gaia[acgs]")
# from_acgs_lite() also needs acgs_lite.integrations.gaia in that wheel.
agent = MyAgent(governance_adapter=GaiaGovernanceAdapter.from_acgs_lite())
```

`from_acgs_lite()` wraps the inner engine in a GAIA-owned risk-tag floor:
`blocked` / `review` tags cannot be loosened, and `GAIA_AUTO_APPROVE_TOOLS`
is not treated as policy. `[acgs]` installs the published `acgs-lite`
distribution (not a #2240 unpublished wheel). A missing package, or a
wheel that does not ship `acgs_lite.integrations.gaia`, fails closed
...
([`gaia.ui.sse_handler`](https://github.com/amd/gaia/blob/main/src/gaia/ui/sse_handler.py))
...
<Note>
Two different classes share the name `SSEOutputHandler`. The Agent UI one above
blocks on a real user decision. The API one
([`gaia.api.sse_handler`](https://github.com/amd/gaia/blob/main/src/gaia/api/sse_handler.py))
...
closed there. See [API Server](/sdk/infrastructure/api-server).
</Note>
...
|---|---|---|
| `PolicyEngine` | `RuleBasedPolicyEngine` (stub) | `GaiaGovernanceAdapter.from_acgs_lite()`, LLM judge, OPA |
