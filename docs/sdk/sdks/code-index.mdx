...
For an editable repository checkout, use `uv pip install -e ".[rag]"` instead.
On Linux/WSL, add `--extra-index-url https://download.pytorch.org/whl/cpu`
to the GAIA installation command to avoid CUDA packages.
...
State is initialized lazily. Call `_init_code_index_state` to set an explicit
starting repository. Its optional `ceiling_paths` lists the roots within which
the agent may select repositories; by default the only root is `repo_path`.
For a custom agent working across sibling repositories, supply their common
workspace root (or each allowed repository) as the ceiling before registering
tools. Indexing a second repository does not narrow that original boundary.
Search and clear operate on the currently selected repository, not a combined
multi-repository index. A standalone `CodeIndexSDK` instance stays tied to its
configured repository.

`CodeIndexToolsMixin` is also registered in `KNOWN_TOOLS["code_index"]` ([`src/gaia/agents/registry.py`](https://github.com/amd/gaia/blob/main/src/gaia/agents/registry.py)) for dynamic composition when scaffolding agents — see the [custom-agent guide](/sdk/core/custom-agents).

## Privacy and safety

- Embeddings go to the configured Lemonade server. Use a local server to keep source chunks on this machine. Cloud chat can still receive retrieved code as conversation context.
- Sensitive filenames are skipped during discovery: `.env`, `.env.*`, SSH private keys, `*.pem`, `*.key`, `*.pfx`, `*.p12`, `*.jks`, `*.keystore`.
- The SDK uses `gaia.security.PathValidator` to keep file reads scoped to the configured `repo_path`, and the `index_codebase` tool checks requested repositories against the mixin's configured `ceiling_paths`.
