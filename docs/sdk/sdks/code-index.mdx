...
<Note>
  **Looking for usage docs?** See the [Code Index guide](/guides/code-index) for CLI walkthroughs and architecture overview.
</Note>
...
`code_index` is registered in `KNOWN_TOOLS` ([`src/gaia/agents/registry.py`](https://github.com/amd/gaia/blob/main/src/gaia/agents/registry.py)) so the BuilderAgent template ([`src/gaia/agents/builder/template.py`](https://github.com/amd/gaia/blob/main/src/gaia/agents/builder/template.py)) can scaffold the import and base class for new agents that opt into it. See the [Custom Agents guide](/guides/custom-agent) for the Python-agent shape.
