...
**See also:** [User Guide](/guides/chat) · [Agent harness](/sdk/core/agent-system)

---

`AgentSDK` is the conversation/model client used by the [agent harness](/sdk/core/agent-system).
It does not itself dispatch your `@tool` functions. Use `Agent` for a tool-using
loop, or `AgentSDK` when your application owns the model conversation directly.
All examples below require a configured backend unless they only inspect configuration.

`send()` and `send_stream()` manage bounded in-memory chat history;
`max_history_length` counts question/answer pairs. `send_messages()` and
`send_messages_stream()` accept caller-owned message lists instead. Neither is a
persistent memory database. Use [Memory SDK](/sdk/sdks/memory) for durable memory.
...
class AgentResponse:
    ...
    usage: Optional[Dict[str, Any]] = None  # Provider-reported token usage
```
...
`send(..., no_history=True)` bypasses stored chat history for that request.
It does not disable enabled RAG. `get_history()` returns separate user/assistant
entries; use `conversation_pairs` when counting completed exchanges.
...
Install `amd-gaia[rag]`, initialize Lemonade and embeddings with `gaia init`,
and replace the sample PDF paths with files you own. Check indexing results
before asking a document question; a failed index is not evidence that the
model read the document.
...
if not chat.enable_rag(documents=["manual.pdf", "guide.pdf"]):
    raise RuntimeError("One or more documents could not be indexed")
...
# Add more documents
if not chat.add_document("troubleshooting.pdf"):
    raise RuntimeError("Could not index troubleshooting.pdf")
...
<CardGroup cols={2}>
  <Card title="Messaging" icon="message">
    - `send(message: str, *, no_history=False, **kwargs) -> AgentResponse`
    - `send_stream(message: str)`
    - `send_messages(messages, system_prompt=None, tools=None, **kwargs) -> AgentResponse`
    - `send_messages_stream(messages, system_prompt=None, tools=None, **kwargs)`
  </Card>
  ...
  <Card title="History" icon="clock-rotate-left">
    ...
    - `conversation_pairs`, `history_length` (properties)
  </Card>
...
- **[Agent harness](/sdk/core/agent-system)** — Tool-using execution loop
