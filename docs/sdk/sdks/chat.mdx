...
The SDK applies the system prompt once per request. `set_system_prompt()` and
`update_config(system_prompt=...)` take effect on the next request; structured
message calls can override it for one request without changing the stored prompt.

Custom prompts use the native system-message channel for every provider.
...
class AgentResponse:
    ...
    usage: Optional[Dict[str, Any]] = None  # per-call token usage, when the provider reports it
    reasoning: Optional[str] = None  # the model's reasoning, kept apart from `text`
```
