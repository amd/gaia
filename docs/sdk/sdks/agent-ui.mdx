...
**See also:** [User Guide](/guides/agent-ui) | [Chat SDK](/sdk/sdks/chat) | [API Specification](/spec/agent-ui-server)

---

## Overview

The Agent UI SDK is the Python backend that powers GAIA Chat Desktop. It provides:
...
<AccordionGroup>
  <Accordion title="POST /api/chat/send">
    ...
    **Streaming response** (SSE events):
    ...
    When `stream: true`, the server returns a `text/event-stream` response. Each line follows the SSE format `data: <JSON>`. The `SSEOutputHandler` (`src/gaia/ui/sse_handler.py`) bridges agent console events to the following typed events:
...
The Agent UI server delegates to the core [Chat SDK](/sdk/sdks/chat) for LLM communication:

```python
from gaia.chat.sdk import AgentSDK, AgentConfig

# The server creates an AgentSDK instance per request
config = AgentConfig(
    model=session.get("model", "Qwen3.5-35B-A3B-GGUF"),
