...
Restoration budgets complete turns against the device profile (32K NPU, 64K
GPU/CPU), reserving the current prompt, native tool schemas, output allowance and
2K estimated tokens of headroom. History uses at most half the profile window.
