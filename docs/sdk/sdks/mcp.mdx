...
<AccordionGroup>
  ...
  <Accordion title="Config file not found">
    `load_mcp_servers_from_config()` returns 0 servers. GAIA looks for a local config in two places (in order):

    1. Current working directory (`./mcp_servers.json`)
    2. The directory containing the script being run (i.e., `__main__.__file__` parent)

    Then merges in the global config:

    3. `~/.gaia/mcp_servers.json`
    ...
    If running a script from a different directory, place `mcp_servers.json`
