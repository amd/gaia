...
A configuration file lets you manage MCP servers without changing agent code. GAIA searches for `mcp_servers.json` in two locations: first `./mcp_servers.json` (project-specific), then `~/.gaia/mcp_servers.json` (global default).
...
<AccordionGroup>
  ...
  <Accordion title="Config file not found">
    `load_mcp_servers_from_config()` returns 0 servers. Verify the file exists:
    ```bash
    cat ./mcp_servers.json        # Project config
