...
A watcher calls Python callbacks; it does not automatically create agent tools,
extract structured data, or create database tables. Supply those behaviors in
your host and stop the watcher during shutdown. The
[complete example below](#complete-example) shows an `Agent` that registers its
watching tools explicitly.
