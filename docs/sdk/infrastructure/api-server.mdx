...
# Now accessible via the OpenAI SDK — once a model is registered in
# AGENT_MODELS (empty by default, see below):
...
is empty by default — the server starts and serves an empty `/v1/models` list
until you add an entry. To expose a model, append an entry to `AGENT_MODELS`:
