...
auto-approving them**. The refusal is not silent — streaming clients get a
`tool_confirm_denied` event and non-streaming responses are prefixed with the
...
which shows a permission prompt.

<Warning>
For a trusted, single-user, localhost-only server you can opt out:

```bash
GAIA_API_ALLOW_UNCONFIRMED_TOOLS=1 gaia api start
```
...
`gaia api start` prints a warning when it is set, and the server logs one on
every auto-approved call. Never set it on a shared or network-exposed host.
</Warning>
