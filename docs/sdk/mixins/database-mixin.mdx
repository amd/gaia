...
`DatabaseAgent` registers its database tools. Adding another mixin to the class
does not initialize or register that mixin's capabilities automatically.
Set its required host state and call its registration method from
`_register_tools()`. RAG and file-watching integrations have additional lifecycle
requirements; see [tool mixins](/sdk/mixins/tool-mixins) and
[the complete watcher example](/sdk/utils/file-watcher#complete-example).
