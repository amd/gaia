...
GAIA's `gaia.connections` package implements OAuth 2.0 PKCE
...
`gaia.connections.store` module writes to:
...
`gaia.connections.list_connections()` returns only metadata
...
schema is exercised in `tests/unit/connections/test_secret_hygiene.py`
...
1. The named agent must have a per-agent grant covering the requested
   scopes (in `~/.gaia/connections/grants.json`).
...
- The agent's `REQUIRED_CONNECTIONS` declaration before granting (visible
  in the consent dialog).
- The CLI grants list (`gaia connections grants list google`) at any time.
- Periodic revocation of unused grants
  (`gaia connections grants revoke <provider> <agent_id>`).
...
`~/.gaia/connections/grants.json`.** Grants are per-agent scope
...
`gaia connections status`) each maintain their own in-memory access-
...
| Forged agent identity in tool body | `_agent_context` is private (not in `gaia.connections.__init__`)                       |
...
- [ ] Production users do not see `keyrings.alt` plaintext fallback —
  if they do, the connections module raises `ConnectionsError` with
  this page in the message.
- [ ] Periodically review `~/.gaia/connections/grants.json` for
  unexpected grants. The CLI command
  `gaia connections grants list google` enumerates them.
