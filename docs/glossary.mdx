...
### ChatAgent
The base class GaiaAgent extends, providing conversation history, RAG for document Q&A, voice (Talk mode), and interactive commands like `/clear`, `/history`, and `/stats`. Retired as a selectable agent — its `chat`/`doc`/`file` ids stay resolvable for stored sessions but no longer appear in the picker. See also: GaiaAgent, AgentSDK, AgentSession.
