...
### Context Size
The number of tokens allocated for LLM processing. GAIA selects a context size from its device profile when loading a model. Larger windows allow more input but require more memory. Not the same as Context Window (the model's inherent limit). See also: Context Window, Max Tokens.
...
### Lemonade Server
AMD's optimized LLM serving platform providing hardware-accelerated inference on Ryzen AI processors. Supports NPU/iGPU hybrid mode, model management, and an OpenAI-compatible API. Start with `gaia init`. Required for most GAIA operations. See also: NPU, Hybrid Mode, LEMONADE_BASE_URL.
