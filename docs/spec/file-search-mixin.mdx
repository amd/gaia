...
`write_file` and `edit_file` share the [read-before-edit guard](/spec/file-io-tools-mixin#read-before-edit):
an existing file the agent has not read, or one whose mtime or size changed
since it did, is refused before anything is written or backed up. Successful
writes become the baseline for later edits.
