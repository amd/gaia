...
or creating a backup. Overwriting an existing file is refused, and the file left
unchanged, when its backup cannot be made. Successful writes become the baseline
for later edits.
