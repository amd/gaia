...
Existing scope regression tests are in `tests/unit/test_file_search_scope.py`.
...
Call `register_file_search_tools()` from your agent's registration hook. The
signatures above describe registered harness tools, not Python instance methods
such as `agent.search_file()` or `agent.read_file()`. Use the normal agent
query/dispatch path; see [Testing agents](/sdk/testing).

The packaged [GAIA agent](/guides/gaia) already includes file search. For example:

```text
Find Python files containing TODO under ~/projects/my-app/src.
```

Naming the directory limits the search to that tree. Inspect the returned
paths and matches before changing files. For a custom host, review
[tool composition](/sdk/mixins/tool-mixins) and the
[source implementation](https://github.com/amd/gaia/blob/main/src/gaia/agents/tools/file_tools.py).
