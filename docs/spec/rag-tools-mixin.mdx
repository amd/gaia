...
below before `_register_tools()` runs, or every tool call raises an
`AttributeError` the moment it touches `self.rag`:
...
This is enforced by convention today, not by the mixin — a host class that
forgets one of these fails silently in the specific sense that the tool call
still returns a response, just a "temporarily unavailable" fallback instead
of an error a developer would notice. [#3316](https://github.com/amd/gaia/issues/3316)
tracks making `_register_tools()` fail loudly at registration time instead of
relying on documentation to catch this.
