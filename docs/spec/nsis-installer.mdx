...
| Step | Action | Details |
|------|--------|---------|
| **LAUNCH** | User runs `setup.exe` | Starts NSIS installer with MUI2 interface |
| **CHECK** | Check existing install | Prompts to remove previous installation if found |
| **SELECT** | Select components | GAIA (required), RAUX UI (optional) |
| **DOWNLOAD** | Download/Extract | Embedded Python 3.10.9, docopt.py workaround |
| **PREREQS** | Check prerequisites | Lemonade Server (prompt), FFmpeg (via winget) |
| **INSTALL** | Install GAIA | Runs `install.bat` to configure Python and pip install |
| **CONFIGURE** | Configure system | Update PATH, create desktop shortcuts, launch RAUX setup |
| **FINISH** | Finish page | Options to launch GAIA CLI or RAUX UI |
...
| Section | Contents |
|---------|----------|
| **HEADER** | `!define` parameters, `Var` declarations, MUI2 settings |
| **PAGES** | Welcome, License, Directory, RAUX Options (custom), Install, Finish (custom) |
| **INSTALL** | Cleanup existing, Create directories, Download Python, Configure Python, Install dependencies, Install GAIA |
| **FUNCTIONS** | `.onInit`, `RAUXOptionsPage`, `CustomFinishPage`, `run_raux_installer`, `check_raux_install`, `uninstall_raux`, `RunGAIACLI`, `RunRAUX` |
