...
flowchart TD
    A(["NSIS INSTALLER"]) --> B(["PYTHON EMBED"])
    A --> C(["GAIA PACKAGE"])
    C --> D(["LAUNCH SCRIPTS"])
    C --> E(["ELECTRON APPS"])
...
|-----------|------|---------|
| **NSIS INSTALLER** | `Installer.nsi` | Main installer script, orchestrates all components |
| **PYTHON EMBED** | `install.bat` | Downloads and configures embedded Python 3.10.9 |
| **GAIA PACKAGE** | File Layout | GAIA source code and dependencies |
| **LAUNCH SCRIPTS** | `launch_gaia.bat` | CLI launcher for embedded Python environment |
| **ELECTRON APPS** | `src/gaia/apps/` | Desktop UI apps (Jira, Chat, Docker) |
...
%%{init: {'theme':'base', 'themeVariables': { 'primaryColor':'#ED1C24', 'primaryTextColor':'#fff', 'primaryBorderColor':'#C8171E', 'lineColor':'#F4484D', 'secondaryColor':'#2d2d2d', 'tertiaryColor':'#f5f5f5', 'fontFamily': 'system-ui, -apple-system, sans-serif'}}}%%
flowchart TD
    A[/"INSTALLER"/] --> B(["$LOCALAPPDATA/GAIA/"])
    B --> C(["bin/"])
    B --> D(["python/"])
    B --> E(["src/"])
    B --> F(["installer/"])
...
| Directory | Contents |
|-----------|----------|
| **bin/** | `launch_gaia.bat` - CLI launcher |
| **python/** | Embedded Python 3.10.9, `Scripts/` (pip.exe, gaia.exe), `Lib/site-packages/` |
| **src/gaia/** | GAIA source: `apps/` (jira, chat, docker webui), `electron/` (package.json, main.js) |
| **installer/** | `install.bat`, `launch_gaia.bat`, `installer_utils.py` |
