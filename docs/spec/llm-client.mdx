...
- **Two providers**: Lemonade (local AMD-optimized, plus models configured behind Lemonade such as on-prem gateways) and Claude
...
- **Backward-compatible** `use_claude` flag

**Provider Capabilities:**

| Method | Lemonade | Claude |
|--------|:--------:|:------:|
| `generate()` | ✓ | ✓ |
| `chat()` | ✓ | ✓ |
| `embed()` | ✓ | ✗ |
...
Cloud models served through Lemonade have their own capabilities; selecting
Lemonade does not imply that every remote model supports every operation.
</Note>
...
1. **Factory Pattern**
   ...
   - Explicit provider selection via `provider` parameter ("lemonade", "claude")
   - Backward-compatible `use_claude` flag
   - Auto-detection of provider from flags when `provider` not specified
   - Default to Lemonade provider when no flags set
   - Retired names (`"openai"`, `"litellm"`) and a true `use_openai` flag raise
     `ValueError` with migration guidance before any provider is imported
...
3. **Provider Implementations**
   ...
   - **ClaudeProvider**: `generate`, `chat`, `vision`, `get_performance_stats` (last-call usage)
...
def create_client(
    ...
    **kwargs,
    ...
    Args:
        provider: Explicit provider name ("lemonade" or "claude").
                  If not specified, auto-detected from the use_claude flag.
        use_claude: If True, use Claude provider (ignored if provider is specified)
        use_openai: Retained only to reject legacy callers; True always raises
    ...
    Raises:
        ValueError: If provider is unknown, is a retired name ("openai",
                    "litellm"), or use_openai is True
    ...
    Note:
        ...
        An explicit supported provider takes precedence over use_claude.
    """
```
...
class LLMClient(ABC):
    ...
    def get_performance_stats(self) -> dict:
        ...
        Note:
            Supported by: Lemonade, Claude (last-call token usage)
        """
...
**Partial support** - `generate`, `chat`, `vision`, `get_performance_stats`.
...
class ClaudeProvider(LLMClient):
    ...
    def __init__(
        ...
        model: str = DEFAULT_CLAUDE_MODEL,  # "claude-sonnet-5"
        system_prompt: Optional[str] = None,
        max_retries: int = 3,
        timeout: float = 300.0,
        **_kwargs,
        ...
        Args:
            ...
            model: Model name (default: DEFAULT_CLAUDE_MODEL)
            system_prompt: Default system prompt for chat
    ...
    # Supports: generate, chat, vision, get_performance_stats
    # Raises NotSupportedError: embed, load_model, unload_model
```
...
def create_client(provider=None, use_claude=False, use_openai=False, **kwargs):
    # Retired selections fail before any provider import
...
```python
# ClaudeProvider overrides vision but not embed
class ClaudeProvider(LLMClient):
    def vision(self, images: list[bytes], prompt: str, **kwargs) -> str:
        # Sends the image as a base64 content block
        ...

    # embed() inherited - raises NotSupportedError
```
...
class LemonadeProvider(LLMClient):
    ...
    def generate(self, prompt, model=None, stream=False, **kwargs):
        # Routed through chat (completions endpoint not available in Lemonade v9.1+)
        return self.chat([{"role": "user", "content": prompt}], model=model, stream=stream, **kwargs)
```
...
```python
class ClaudeProvider(LLMClient):
    def __init__(self, api_key=None, model=DEFAULT_CLAUDE_MODEL, **kwargs):
        import anthropic
...
**File:** `tests/unit/test_removed_llm_providers.py` — retired names and flags are
rejected across the factory, SDK, base agent and CLI, before provider import or
...
[project]
dependencies = [
    "openai>=1.0.0",      # OpenAI-compatible HTTP client used by LemonadeClient
...
# This works (Claude supports vision)
description = client.vision([image_bytes], "Describe this image")

# This raises NotSupportedError (Claude doesn't support embeddings)
try:
    embeddings = client.embed(["Hello world"])
except NotSupportedError as e:
    print(f"Feature not available: {e}")
    # Output: "Claude does not support embed"
```
...
### Example 6: Embeddings (Lemonade only)
...
<Warning>
The Lemonade provider sends inference over **OpenAI-compatible endpoints**, but
before a local-model request it also calls Lemonade's model-lifecycle API (for
example `/load`). A generic OpenAI-compatible server is therefore not a drop-in
replacement: configure the third-party or on-prem model in a Lemonade server and
point GAIA at that server. See [gateway migration](/sdk/sdks/llm#gateway-migration).
</Warning>
...
GAIA sends inference requests in the OpenAI-compatible shapes below.
...
Point GAIA at the Lemonade server that fronts your model:

<Tabs>
  <Tab title="Environment Variable">
    ```bash Linux
    export LEMONADE_BASE_URL="http://your-lemonade-server:8080"
    ```

    ```powershell Windows (PowerShell)
    $env:LEMONADE_BASE_URL="http://your-lemonade-server:8080"
    ```

    ```cmd Windows (CMD)
    set LEMONADE_BASE_URL=http://your-lemonade-server:8080
    ```
  </Tab>
  ...
  <Tab title="Direct Initialization">
    ...
    client = create_client(base_url="http://your-lemonade-server:8080")
...
- If your server uses `/v1` instead, provide the full path: `http://localhost:8080/v1`
</Note>
...
# Connect to the Lemonade server that fronts your model
client = create_client(
    base_url="http://your-lemonade-server:8080",
...
<AccordionGroup>
  <Accordion title="Connection Errors" icon="plug">
    **Problem:** Requests fail because the server can't be reached (connection refused, DNS, or TLS)

    **Solutions:**
    1. Verify the server is running:
       ```bash
       curl http://your-lemonade-server:port/api/v1/models
       ```
    2. Check firewall settings
    3. Ensure correct base URL format
    4. Test with explicit base URL:
       ```python
       client = create_client(base_url="http://localhost:8080/api/v1")
       ```
  </Accordion>

  <Accordion title="404 Endpoint Errors" icon="magnifying-glass">
    **Problem:** Requests fail with HTTP 404

    **Solutions:**
    1. Verify API path structure: `/v1` vs `/api/v1`
    2. Confirm `base_url` points at the Lemonade server, not the raw upstream endpoint
    3. Use chat method explicitly if needed:
       ```python
  ...
  <Accordion title="Model Not Found" icon="robot">
    ...
    **Solutions:**
    1. Specify model explicitly:
       ...
       ```
    2. List the models the server exposes:
       ```bash
       curl http://your-lemonade-server:port/api/v1/models
       ```
    3. Use the exact model ID from that catalog
  </Accordion>
  ...
  <Accordion title="Streaming Issues" icon="signal-stream">
    ...
    **Solutions:**
    1. Verify the server supports Server-Sent Events (SSE)
...
**Features:**
- Factory-based client creation with `create_client()`
- Two providers: Lemonade (local), Claude
