...
<Warning>
  **CLI update (#977):** `gaia mcp add` / `gaia mcp remove` referenced in this spec
  were removed. MCP servers are now configured through the connectors framework
  (`gaia connectors configure <id> --set KEY=VALUE`; the entry is persisted to
  `~/.gaia/mcp_servers.json` with secrets in the OS keyring). `gaia mcp list` still
  lists configured servers. Sections below using `gaia mcp add/remove` are pending a
  rewrite — see issue #1339.
</Warning>

<Note>
🔧 **You are viewing:** API Specification - Complete technical reference

**See also:** [MCP Client](/sdk/sdks/mcp) · [API Specification](/spec/mcp-client)
</Note>
...
flowchart TB
    ...
    subgraph ExtServers ["External MCP Servers"]
        Time(["Time"])
        GitHub(["GitHub"])
        FS(["Filesystem"])
        More(["..."])
    ...
    style Mixin fill:#E2A33E,stroke:#A87B2D,stroke-width:3px,color:#1a1a1a
    style Manager fill:#EFC480,stroke:#E2A33E,stroke-width:2px,color:#1a1a1a
    style Client fill:#A87B2D,stroke:#A87B2D,stroke-width:2px,color:#fff
    ...
    style GA fill:none,stroke:#E2A33E,stroke-width:2px,stroke-dasharray:5,color:#E2A33E
...
- [MCP Client](/sdk/sdks/mcp) - SDK reference for connecting to MCP servers
