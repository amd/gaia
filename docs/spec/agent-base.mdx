...
<Warning>
The direct OpenAI and LiteLLM provider adapters have been removed. Any
`use_chatgpt` field shown below is a migration-error shim: setting it to `True`
raises before inference. Use the [Lemonade gateway path](/sdk/sdks/llm#gateway-migration).
</Warning>
...
def __init__(
    ...
    Args:
        ...
        use_chatgpt: Removed; True raises migration guidance
        ...
        max_consecutive_repeats: Max consecutive identical tool calls before stopping (default: 4; at least 2, or ValueError)
        ...
        resend_reasoning_across_requests: Send the model's stored reasoning from earlier
            user requests back to it. Within one request, each step's reasoning is always
            sent back on its assistant message as `reasoning_content` (default: False)
        max_output_tokens: Output-token cap per LLM reply, thinking included (default: None —
            32768 for a Lemonade cloud model such as fireworks.*, 8192 otherwise)
    """
```
...
def process_query(
    ...
    Returns:
        dict: {
            ...
            "timing_summary": dict,  # Where the time went (see Step Timing)
            "max_steps_reached": bool,  # Hit the step limit before answering
...
Each step's `stats` record in `conversation` carries a timing breakdown next to
`performance_stats`. The times come from the agent's own clock, and the token
counts come from each response's own usage, so the figures hold for cloud models
too.
...
```python
{
    "type": "stats",
    "step": 3,
    "performance_stats": {...},    # unchanged: what the backend reported
    "step_seconds": 41.2,          # this step's wall time
    "llm_seconds": 39.8,           # model requests, retries and continuations included
    "tool_seconds": 0.9,           # sum of this step's tool calls
    "overhead_seconds": 0.5,       # the rest; the three add up to step_seconds exactly
    "ttft_seconds": 1.3,           # first request's time to first token; None if not streamed
    "finish_reason": "length",     # as the provider returned it; None if not reported
    "reasoning_tokens": 1830,      # None when the provider does not report them
    "output_tok_per_s": 48.6,      # completion tokens / generation seconds
    "llm_calls": 2,                # more than 1 means the step retried or continued
    "tools": [{"name": "run_shell_command", "seconds": 0.9}],
}
```

For a streamed request, `output_tok_per_s` counts the tokens after the first over
the time after the first token: pure decode. A non-streamed request has no
first-token mark, so it divides all its tokens by the whole request, which makes
the figure a lower bound.

`timing_summary` in the result totals the run: `wall_seconds`, `llm_seconds`,
`tool_seconds`, `overhead_seconds`, `llm_calls`, `output_tokens`, and
`slowest_steps`, the three longest steps, each with its tokens, finish reason, and
which part (`llm`, `tools`, or `overhead`) dominated it.
