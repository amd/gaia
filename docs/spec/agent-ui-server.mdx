...
| `POST /api/documents/upload` | `multipart/form-data` file blob | Drag-and-drop in browser mode / modern Electron | File is persisted under `$GAIA_HOME/documents/<id>_<name>.<ext>` (default `~/.gaia/documents/`) and owned by the server |
...
When a server-owned document is deleted (`DELETE /api/documents/{id}`), the on-disk file under `$GAIA_HOME/documents/` is also unlinked as a best-effort cleanup. User-owned files from `upload-path` are left untouched.
