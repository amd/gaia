...
def tool(
    ...
    *,
    ...
    preflight: Callable[[dict], dict | None] | None = None,
) -> Callable:
    """
    ...
    Args:
        ...
        atomic: If True, marks this tool as suitable for direct execution
                without multi-step planning. Use for simple, self-contained
                operations (default: False)
        display_label: Optional user-facing label for UI progress strips
        timeout: Per-tool execution limit in seconds, or ``None`` for the
                 global default
        preflight: Called with the call's arguments before the confirmation
                   prompt; returns the refusal the call has already earned,
                   or ``None``. The tool body must enforce the same rule,
                   since state can change while a prompt waits
...
Unsupported keyword arguments raise ``TypeError`` when the function is
decorated. The error names the tool, the invalid argument, and the accepted
arguments (``atomic``, ``display_label``, ``timeout``, and ``preflight``).
...
# Registry structure:
{
    "tool_name": {
        ...
        "atomic": bool,           # True if marked as atomic (direct execution)
        "display_label": str | None,
        "timeout": float | None,
        "preflight": Callable | None  # checked before the confirmation prompt
    }
}
```
...
def get_system_info() -> dict:
    ...
    This is a simple, self-contained operation suitable for direct
    execution without multi-step planning.

    Returns:
        Dictionary with system info (platform, CPU, memory)
    """
    import platform
    import psutil
...
# Registry entry includes atomic flag:
{
    "get_system_info": {
        "name": "get_system_info",
        "description": "Get current system information...",
        "parameters": {},
        "function": <function get_system_info>,
        "atomic": True  # Marked as atomic
    }
}
```
...
_TOOL_REGISTRY[tool_name] = {
    ...
    "parameters": params,
    "function": f,
    "atomic": atomic  # Defaults to False
}
```
