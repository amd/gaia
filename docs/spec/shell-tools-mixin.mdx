...
Still refused anywhere on a line: redirection (`>`, `>>`, `<`), command
substitution (backticks, `$()`), a background `&`, an inline `VAR=value`
assignment, and newlines — each changes what a command *is*, rather than which
commands run. Use `write_file` / `edit_file` to write a file.
...
compound line is never handed over as one string.
