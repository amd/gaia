...
command substitution (backticks, `$()`), a background `&`, an inline
`VAR=value` assignment, and newlines — each changes what a command *is*,
rather than which commands run. Use `write_file` / `edit_file` to write a file.
...
travels in it, spelled `2>nul`, because there the shell owns the pipeline.
