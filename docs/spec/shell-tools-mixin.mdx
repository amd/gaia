...
| `cd <dir> && a` | `cd` moves the rest of its line, and — if it actually ran — the **next separate call** too |
...
Commands share one session per agent, so a `cd` is still in effect on the next
separate call — `cd build` in one turn, then `make` in the next, without
repeating the directory. Only the working directory persists; nothing is handed
to a real shell, so `export` and `source` have nothing to persist *from*.

A `cd` moves the session only when it actually ran: one that `&&`/`||` skipped,
or that a timeout cut the line before reaching, leaves the session where it was.
A `cd` into a directory the path policy refuses is not absorbed either —
otherwise persistence would be a way to reach a forbidden directory and then
read files there by bare name, which the per-argument check never sees.

An explicit `working_directory` argument is a **one-shot override**: the command
runs there, and the session's own directory is untouched.

| Tool | Behaviour |
|------|-----------|
| `get_shell_state` | Reports the session's current working directory |
| `reset_shell_session` | Returns the session to the directory the task started in |
...
class ShellToolsMixin:
    ...
    - get_shell_state: Report the session's current working directory
    - reset_shell_session: Return the session to its starting directory
    """
