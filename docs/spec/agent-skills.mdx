...
<Steps>
<Step title="Metadata (one line in the prompt)">
`name` + `description` of every installed skill are listed in the system prompt,
one line each — about a line per skill for the whole library, instead of any skill
body sitting in the prompt on every turn. See [Skill catalogue](#skill-catalogue).
</Step>
<Step title="Body (on trigger)">
When a skill is selected — by the model calling `load_skill`, or by the user
asking for it — the `SKILL.md` Markdown body is
...
Once a skill is in scope, it is triggered by the model calling `load_skill` —
because the skill catalogue below shows the work matches it, or because the user
named it. A user-facing `/web-research` slash
...
A user should never have to know a skill's name. The flagship lists every
installed skill in its system prompt, one line each (name and `description`), and
the model loads a skill when the work matches it — at the start of a task or
partway through, for example after a command the skill grants was refused.

<Note>
**Write the `description` for this.** The model decides from it. Say what the
skill does and when to use it, naming the objects, formats, and services a user
would mention — `github-triage` names the "unread notification inbox", the "issue
backlog", and the `gh` CLI.

Use the user's plainest word, not the precise one. `inbox-triage` originally said
only "mailbox" and "email" — a request phrased as *"anything urgent in my mail?"*
shares no word with either, so name the casual synonyms too (mail, unread, deal
with, rundown).
</Note>

The list depends only on what is installed, so it is identical on every turn and
stays in the cached head of the prompt. It costs about one line per skill (roughly
1,000 tokens for the starter pack) plus a ~96-token sourcing rule: facts about live
external state must come from a tool call, never from memory. A description is
cut at 480 characters in the list, so put the "use when" triggers early. A skill
already under LOADED SKILLS stays listed, and the model is told not to load it
again — dropping it would change the block from turn to turn. Read-only
`.claude/skills` imports are loadable by name but not listed — they are another
host's marketplace, written for working *on* a repo rather than answering a
user's question.

A CLI granted through `shell:execute:<binary>` is refused until its skill is
loaded, and the refusal names the skill to load.

Tunable: `GAIA_SKILL_DISCOVERY` (`0` hides the list).
