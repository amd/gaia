...
|---|---|---|
| **Load** | one skill clears the confidence bar, and either beats the runner-up by a margin or is the one the request actually named | it is loaded and the reply opens by saying so |
...
user's behalf. Identity is the one exception: when equal-scoring candidates
differ in how much of the request is the skill's **own name**, the named one
wins. *"Triage my inbox"* scores identically for `inbox-triage` and
`github-triage` (whose description mentions a notification *inbox*), but only one
...
Use the user's plainest word, not the precise one. `inbox-triage` said "mailbox"
and "email", so *"anything urgent in my mail?"* matched it at zero — "mail" is
not "mailbox" to a lexical index, and the only skill claiming "urgent" was the
GitHub one.
</Note>
