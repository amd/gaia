...
| `shell` | `execute` | **Bridged when scoped** — `shell:execute:<binary>` grants that one CLI, restricted to the read-only subcommands in `BINARY_POLICIES`. An unscoped `shell:execute` is still refused. |
...
A skill declaring any **unbridged** local-capability permission raises
...
| Local-capability permissions | `filesystem` / `database` / `desktop` / `env` refuse the load outright. `shell:execute:<binary>` is bridged: it grants that one CLI, limited to the read-only subcommands in `BINARY_POLICIES`; unscoped `shell:execute` is still refused | Shipped |
