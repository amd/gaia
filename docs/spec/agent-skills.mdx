...
**Also shipped (hub side):** the pre-publish security audit — `gaia skill audit`
plus the hub's audit gate on the skill catalog lane
([#2468](https://github.com/amd/gaia/issues/2468)).

**Still proposed:** the permission sandbox and tier-ceiling enforcement
([#1019](https://github.com/amd/gaia/issues/1019)), the marketplace **client** —
`install`/`remove`/`search`/`publish` and the lock file, so nothing calls those
hub endpoints yet ([#2467](https://github.com/amd/gaia/issues/2467)) — and the
`/api/skills` router and Agent UI panel.
...
- A SemVer `version` is **required** to publish, and `0.0.0` — the format's
  ...
  release.
- **Skills enter the catalog by pull request.** There is no self-serve publish
  API — the Worker authenticates against a maintainer-held `PUBLISH_TOKENS`
...
- The **pre-publish security audit**
  ...
  claim. `community` and `verified` are refused without a cleared report;
  `BLOCK` is rejected and `REVIEW` is held out of the catalog. An
  `experimental` skill may publish without one and is recorded as `unaudited`
  rather than stamped as having passed — the catalog never implies an audit that
  did not happen.
  ...
  not clear" enforced rather than recorded. Because the report is
  publisher-supplied and unsigned, a recorded verdict is marked
  `publisher-asserted`: it means self-consistent, not vouched for. See
