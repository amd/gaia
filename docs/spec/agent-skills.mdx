...
the `skills/` R2 namespace, server-side validation of the SKILL.md grammar, and
the pre-publish audit gate ([#2467](https://github.com/amd/gaia/issues/2467)).

**Also shipped (client side, [#2467](https://github.com/amd/gaia/issues/2467)):**
the marketplace verbs `search`/`install`/`remove`/`publish`, bundle signing +
install-time tier ceilings, `skill-lock.json`, and the local resolver that turns
a declared `skills:` / `skill_sets:` block into loaded skills at construction.
...
| Install | `gaia agent install <id>` | `gaia skill install <name>` (**PROPOSED** — the verb does not exist yet) |
...
engine. Binding it to the skill, version, and audited bytes is
[#2468](https://github.com/amd/gaia/issues/2468); making it unforgeable needs a
signing key the publisher cannot mint
([#1710](https://github.com/amd/gaia/issues/1710)). Until then a recorded tier
means *consistent*, not *vouched for*.
</Note>

**Verification.** Every published artifact's Worker-computed SHA-256 is recorded
in the per-skill `manifest.json`, the same way the agent lane does it, so an
installer can verify the bytes it downloaded. The installer that will check it is
part of the marketplace client and is still proposed.
