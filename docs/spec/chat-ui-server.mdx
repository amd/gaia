...
- **CORS:** Allows all origins (`*`). The standalone server (`python -m gaia.chat.ui.server`) binds to `localhost` by default. Note: `gaia chat --ui` binds to `0.0.0.0` for Electron/browser access
