...
1. **POST /v1/chat/completions**
   ...
   - Message history: prior turns are loaded into the agent's conversation,
     and `system` / `developer` messages are added to its system prompt
   - `temperature`, `top_p`, and `max_tokens` apply to every model call the
     agent makes for the request
...
1. **Message Handling**
   - OpenAI message format; `content` is a string or an array of content parts
   - Text parts are joined with newlines; any other part type (`image_url`,
     `input_audio`, `file`) is rejected with 400
   - `system` and `developer` messages are appended to the agent's system prompt
   - The last message must be from the user (400 otherwise); earlier
     user/assistant/tool messages become the agent's conversation history
...
1. **SSE Streaming**
   ...
   - A client disconnect stops the agent (its cancel event and the SSE
     handler's `cancelled` flag are set), so it stops running tools and
     generating
   - If the agent raises mid-stream, the stream ends with an
     `{"error": {...}}` chunk followed by `data: [DONE]`
...
class ContentPart(BaseModel):
    """One element of a content-part array. Only type "text" is served."""
    model_config = ConfigDict(extra="allow")
    type: str
    text: Optional[str] = None

class ChatMessage(BaseModel):
    """Chat message in OpenAI format. "developer" is treated as "system"."""
...
class ChatCompletionRequest(BaseModel):
    ...
    # Unset keeps the agent's own default. Set values apply to every model
    # call the agent makes while answering (max_tokens is per call).
...
```python
async def create_chat_completion(request: ChatCompletionRequest):
    # 1. Validate the model FIRST: an unknown model is a 404 even when the
    #    message array is also malformed.
    if not registry.model_exists(request.model):
        raise HTTPException(status_code=404, detail=f"Model '{request.model}' not found")

    # 2. Split the request: the final user message is the query, system and
    #    developer messages are caller system text, the rest is history.
    #    Raises 400 on a non-text content part or a non-user last message.
    user_message, history, system_text = _split_conversation(request.messages)

    # 3. Resolve a fresh agent and load the conversation into it. get_agent
    #    raises ValueError for an unregistered model, re-raised as a 404.
    agent = registry.get_agent(request.model)
    agent.conversation_history = history
    if system_text:
        agent.set_caller_system_prompt(system_text)
    if history or system_text:
        agent.mark_external_content()  # caller-supplied, not typed this turn
    if request.temperature is not None:
        agent.chat.config.temperature = request.temperature
    # ...same for top_p and max_tokens
