...
`write_file`, `write_python_file`, `write_markdown_file`, `update_gaia_md`,
`replace_function`, `edit_file`, and `edit_python_file` refuse a file whose
mtime or size changed since the agent read it, and refuse one it has never read
at all. Rejection leaves the file and its backups untouched. A successful write
records the file again, so the agent's own change does not block its next edit.
Creating a new file needs no read.

See [Read before edit](#read-before-edit) for what counts as a read, the exact
refusal shape, and how the rule interacts with confirmation prompts.

### No backup, no write

Every tool that modifies an existing file backs it up first. If that backup
cannot be made — a full disk, an unwritable cache directory — `write_file`,
`write_python_file`, `write_markdown_file`, `replace_function`, `edit_file`, and
`edit_python_file` refuse the change and leave the file untouched. The error
names the cause and the directory to free or make writable.

The refusal carries `executed: False`, because nothing ran. A tool result
without it is treated as having executed and failed, which is how a one-off
full disk would otherwise be stored as a durable "writing this file is broken"
lesson and replayed into later prompts long after the disk was freed.
...
Read any file and intelligently analyze based on file type. A successful read,
a paged one included, is what lets the write and edit tools change the file —
see [Read before edit](#read-before-edit).
...
- `backup` (bool, optional): Create backup (default: True). The copy goes to
  `~/.gaia/cache/backups/<original absolute path>/<name>.<timestamp>.bak`, never
  beside the file, with or without a `PathValidator`. The backups directory is
  owner-only, and the five newest backups of each file are kept. If the
  backup cannot be made (a full disk, an unwritable cache), the edit is
...
Every tool here that changes an existing file — `edit_file`, `edit_python_file`,
`replace_function`, and `write_file`, `write_python_file`, `write_markdown_file`
and `update_gaia_md` when the target already exists — refuses the file unless
the agent has read it. An edit made from a grep snippet or a guess is made
blind; this makes the agent look at what it is about to change.

- **What counts as a read:** `read_file` returning the file, a paged
  (`offset`/`limit`) read included. So does the agent creating the file, or
  editing it successfully. A shell `cat` does not.
- **`filesystem_tools`' `read_file` unlocks only its text views** — the plain,
  paged, CSV and JSON ones. Its PDF, image, binary and `metadata` views show a
  summary rather than the file, so they unlock nothing. The `file_io` and
  `file_search` reads have no such split.
- **Changed on disk since the read** — a different mtime or size, a bare
  `touch` included — is refused too, until the agent reads the file again.
- **Creating a file needs no read.**
- **The record belongs to one agent instance** and lasts as long as it does.
  Another agent's reads unlock nothing.

The refusal happens before anything is written, and before the confirmation
prompt, so nobody is asked to approve an edit that is already going to be
refused. It applies with confirmations bypassed too, because it is about the
edit being correct, not permitted. A path outside the allowed paths gets the
usual scope refusal instead, so the check never reveals whether a file exists
there.
...
{
    "status": "error",
    "error": str,           # names the file and says to read it
    "error_type": "not_read" | "changed_since_read",
    "file_path": str,
    "executed": False       # stopped before it ran
}
```
...
so the two cannot drift apart. Two rules:
...
The C++ `file_edit` tool (`cpp/src/file_tools.cpp`) follows both rules and
rejects a stale write, but still lets an agent edit a file it never read.
