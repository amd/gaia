...
A host that forgets is refused rather than silently unconfined or crashed:

- **Write-capable tools** (`write_python_file`, `edit_python_file`,
  `write_markdown_file`, `write_file`, `edit_file`, `replace_function`) deny
  the operation outright — the target file is never created or modified —
  instead of proceeding without a confinement check.
- **Read tools** (`read_file`, `search_code`, `generate_diff`,
  `update_gaia_md`) raise a `MissingHostAttributeError` naming the host
  class, the missing attribute, and this section, instead of surfacing a
  bare, unhelpful `AttributeError`.

`self.path_validator` has no legitimate `None` value — unlike `RAGToolsMixin`'s
`self.rag`, there is no "intentionally disabled" state for path confinement.
