...
- `backup` (bool, optional): Create backup (default: True). With a
  `PathValidator` the copy goes to `~/.gaia/cache/backups/<original absolute
  path>/<name>.<timestamp>.bak`, never beside the file.
