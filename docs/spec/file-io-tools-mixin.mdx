...
Call `register_file_io_tools()` from your host's `_register_tools()` hook.
These are registered harness tools, not instance methods such as
`agent.read_file()`. Use the normal agent query/dispatch path; see
[Testing agents](/sdk/testing).

The mixin is not self-contained. Its host must supply a `path_validator`.
Python operations use the standard-library AST parser when optional host helpers
`_validate_python_syntax` and `_parse_python_code` are absent. The packaged
[ChatAgent](https://github.com/amd/gaia/blob/main/hub/agents/chat/python/gaia_agent_chat/agent.py)
provides a working host composition. Consult the
[implementation](https://github.com/amd/gaia/blob/main/src/gaia/agents/tools/file_io_tools.py)
before composing it onto a bare `Agent`; do not call the abstract
`Agent._register_tools()` via `super()`.

For a complete first custom agent, follow [Build a Python Agent](/sdk/core/custom-agents).
[#3316](https://github.com/amd/gaia/issues/3316) tracks enforcing mixin host
