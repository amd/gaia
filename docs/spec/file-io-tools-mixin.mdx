...
`write_markdown_file`, `update_gaia_md`, `replace_function`, `edit_file`, and
`edit_python_file` reject changes if the current file content differs from the
recorded generation.
...
excerpt must never be used as its complete replacement. Rejection leaves the file
and backups untouched and re-anchors the ledger
...
- `backup` (bool, optional): Create backup (default: True). The copy goes to
  ...
  beside the file. The backups directory is owner-only, and the five newest
  backups of each file are kept. If the backup cannot be made (a full disk, an
  unwritable cache), the edit is refused and the file is left unchanged.
