...
class RAGConfig:
    ...
    max_chunks: int = 5
    embedding_model: str = "user.embeddinggemma-300m-GGUF"
    cache_dir: str = field(default_factory=default_rag_cache_dir)  # ~/.gaia/cache/rag
...
class RAGSDK:
    ...
    def clear_cache(self):
        """Delete only files the cache wrote (hash-named *.json / *.json.sig
        and *_extracted.md). Raises ValueError if cache_dir is $HOME or ~/.gaia."""
        ...
