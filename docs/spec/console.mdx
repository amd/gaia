...
class OutputHandler(ABC):
    ...
    full_access: bool = False   # a person's on-screen full access (TUI only)
...
<Warning>
  ...
  cannot set it), and every approval taken that way is logged. Neither widens
  `run_shell_command`: a command off its read-only list is refused under them.
  `full_access` lifts that, and only an interactive host that shows it on every
  frame sets it (the TUI's `/full-access`, alongside `auto_approve_gated_tools`).
</Warning>
