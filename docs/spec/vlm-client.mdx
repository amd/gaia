...
VLMClient provides Vision-Language Model capabilities for extracting text from images using Lemonade server. It handles image-to-markdown conversion with on-demand model loading and downloading, and context manager support.
...
1. **Model Management**
   ...
   - Every request re-checks the model is loaded at its required context size
...
5. **Error Handling**
   ...
   - Loading failures (the real load error text, e.g. out of memory)
   - Extraction failures
   - Connection errors
...
1. **Performance**
   ...
   - Model loads use LemonadeClient's load timeout, not the extraction timeout
...
class VLMClient:
    ...
    def __init__(
        ...
        Note:
            ...
            - base_url defaults to http://localhost:13305/api/v1
    ...
    def extract_from_image(
        ...
        Note:
            - The request itself loads the VLM at its required context size
              (when auto_load=True); a load failure raises VLMExtractionError
...
There is no separate pre-load. `extract_from_image()` passes
`auto_download=self.auto_load` to `LemonadeClient.chat_completions()`, which
loads the model at its required context size (and downloads it if missing)
while holding the model-slot lease, then runs the request. A load failure
propagates and is re-raised as `VLMExtractionError` with the original error
text and the cause attached. Because every request re-checks the load, a model
evicted mid-batch is reloaded on the next page.
...
Output format: Clean markdown with the ACTUAL text from the image."""
    ...
    try:
        ...
        response = self.client.chat_completions(
            ...
            auto_download=self.auto_load,  # ctx-aware load under the lease
        )
...
def test_extract_from_image_load_failure():
    """A load failure surfaces its real error text."""
    with patch('gaia.llm.vlm_client.LemonadeClient') as mock_client:
        vlm = VLMClient()
        vlm.client.chat_completions = Mock(
            side_effect=LemonadeClientError("Failed to load model: out of memory")
        )

        with pytest.raises(VLMExtractionError, match="out of memory"):
            vlm.extract_from_image(b"fake_image_data")
...
def test_context_manager():
    """The context manager does not pre-load; the first request loads."""
    with patch('gaia.llm.vlm_client.LemonadeClient') as mock_client:
        vlm = VLMClient()
...
vlm = VLMClient(base_url="http://192.168.1.100:13305/api/v1")
