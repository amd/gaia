---
title: SD Agent
description: Intelligent Stable Diffusion assistant that optimizes prompts and parameters using AMD NPU-accelerated LLMs
---
...
- Web gallery UI for browsing, annotating, and rating
...
graph TB
    ...
    subgraph "User Interfaces"
        CLI["🖥️ CLI<br/>(gaia sd ...)"]
        GalleryUI["🌐 Gallery UI<br/>(Web/Electron)"]
        ChatInterface["💬 Chat Interface<br/>(Natural Language)"]
        TermDisplay["🖼️ Terminal Display<br/>(Sixel/iTerm2/Kitty)"]
    end

    subgraph "SD Optimization Agent"
        Agent["🤖 SDAgent<br/>(Agent + DatabaseMixin)"]

        subgraph "Core Components"
            PromptEnh["✨ Prompt Enhancer<br/>(LLM-powered)"]
            ParamOpt["⚙️ Parameter Optimizer<br/>(Model, Size, Steps, CFG)"]
            Analyzer["📊 Prompt Analyzer<br/>(Quality Scoring)"]
        end

        subgraph "Intelligence Layer"
            Search["🔍 NL Search<br/>(LLM → SQL)"]
            Learn["🧠 Preference Learner<br/>(Rating Analysis)"]
            Templates["📚 Template Library<br/>(DB-backed)"]
        end

        subgraph "Utilities"
            Export["📤 Export<br/>(JSON/CSV/ZIP)"]
            Cache["🗄️ Cache Manager"]
        end
    end

    subgraph "Storage Layer"
        DB[(💾 SQLite Database<br/>• generations<br/>• templates<br/>• prompt_versions<br/>• user preferences)]
        Files[("📁 Image Files<br/>.gaia/cache/sd/images/<br/>+ thumbnails")]
    end

    subgraph "AMD NPU-Accelerated Backend"
        LLM["🚀 LLM<br/>(Qwen3-4B-Instruct-2507-FLM)<br/>Lemonade Server / AMD NPU"]
        SD["🎨 Stable Diffusion<br/>(SD-Turbo / SDXL-Turbo)<br/>Lemonade Server"]
    end
    ...
    UserInput -->|Chat Message| ChatInterface
...
5. **Storage** → Agent saves metadata to SQLite + image file to disk
6. **Display** → Image shown in terminal or Gallery UI
7. **Feedback Loop** → User rates image, data used for future personalization
...
| **Optimization Scope** | Both prompts AND parameters | Holistic optimization for best results |
...
| **Database Schema** | generations, templates, prompt_versions tables | Structured storage for all generation data |
| **Image Files** | `.gaia/cache/sd/images/` | File system for actual image binaries |
| **Gallery UI** | Web-based gallery (Electron or browser) | Browse, search, annotate, rate generations |
...
DatabaseMixin
  └── SDAgent (Agent + DatabaseMixin)
      ...
      │
      └── Gallery:
          ├── GalleryServer               # Web UI server
          ├── GalleryAPI                  # REST API for UI
...
- [ ] Initialize SQLite database with schema (generations, templates, prompt_versions)
...
### Phase 2: Image Generation & Database Storage (Week 2)

**Goal:** Full generation pipeline with database persistence

- [ ] Extend `LemonadeClient` with `generate_image()` method
- [ ] Create `ImageGenerator` wrapper class
- [ ] Implement `generate_image` tool (full pipeline: enhance → optimize → generate → save)
- [ ] Save all generations to database with metadata
...
- [ ] Add `gaia sd generate` command
- [ ] Add `gaia sd history` command (list recent generations from DB)
- [ ] Integration tests with Lemonade Server

**Deliverable:** `gaia sd generate "mountain"` enhances, optimizes, generates, saves to DB, displays in terminal
...
### Phase 4: Gallery UI with Chat Interface (Week 4)

**Goal:** Standalone web UI for image creation and gallery management
...
- [ ] **Chat Interface**
  - Natural language image creation ("create a cyberpunk city")
  - Agent responses with optimization suggestions
...
**Deliverable:** Standalone UI at `http://localhost:5000` with chat-based image creation and searchable gallery
...
**CLI Enhancements:**
- [ ] Interactive mode (`gaia sd` with no args → conversational interface)
