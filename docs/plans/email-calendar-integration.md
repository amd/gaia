...
>
> **Strategic priority:** Tier 1 — Lead with. Email triage is the #1 use case
> by adoption in the OpenClaw ecosystem and the strongest proof point for
> local-first AI (always-on + sensitive data = very strong AMD advantage).

---

## 1. Strategic Context

From the OpenClaw strategy doc:

> "Clear 4,000+ emails overnight; daily morning briefs via Telegram;
> voice-to-journal knowledge capture" — Highest adoption category

> "Email triage and management — Always-on: Yes, Sensitive Data: Yes,
> AMD Local Advantage: Very strong (cost + privacy) — **Tier 1: Lead with**"

> Aspirational demo: "At CES 2027, show an Agent Computer that has autonomously
> managed a developer's email, code reviews, and infrastructure for 6 months —
> zero cloud costs, zero security incidents, complete auditable local history."

**Why this matters for AMD:** Email is the killer demo. Plug in Agent Computer,
wake up to a triaged inbox. Zero cloud cost, zero data exposure. Every competing
solution (ChatGPT, OpenClaw default config) sends your email content to cloud APIs.
...
| Provider | Phase 1 (Browser) | Phase 1.5 (MCP) | Phase 2 (Native) |
|----------|-------------------|------------------|-------------------|
| Gmail | Playwright → mail.google.com | `gmail-mcp-server` | IMAP + OAuth2 |
| Outlook/365 | Playwright → outlook.office.com | `outlook-mcp-server` | MS Graph API or IMAP + OAuth2 |
| Outlook Desktop | N/A | N/A | Exchange via `exchangelib` |
| Yahoo/Other | Playwright → provider URL | N/A | IMAP + app password |
...
| Provider | Phase 1 (Browser) | Phase 1.5 (MCP) |
|----------|-------------------|------------------|
| Google Calendar | Playwright → calendar.google.com | `@cocal/google-calendar-mcp` |
| Outlook Calendar | Playwright → outlook.office.com/calendar | `outlook-mcp-server` |
| Apple Calendar | N/A (no web UI) | N/A |
...
Outlook is a strategic priority because:
...
### Phase 1.5: Additional MCP Servers (v0.20.0, config only)
...
**Deliverable:** Power users who set up OAuth get faster, more reliable email/calendar.
...
**Strategic context:** Meeting transcription is a natural extension of the email +
