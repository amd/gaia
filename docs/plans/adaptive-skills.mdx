...
<Info>
  ...
  **In flight (do not treat as shipped):** the Phase 1 `gaia.skills` loader — `SkillManager`, `Agent.load_skill` / `unload_skill` / `get_skills_system_prompt` — lives on PR [#2669](https://github.com/amd/gaia/pull/2669), **open and unmerged**. `src/gaia/skills/` does not exist on `main`. Also in flight: the `Skill` → `DistilledProcedure` rename (part of [#2671](https://github.com/amd/gaia/issues/2671)). **This document uses the new name `DistilledProcedure` throughout** for the synthesis intermediate; on `main` that class is still `skill_synthesis.Skill` ([`:158`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/skill_synthesis.py)).
</Info>
...
mechanisms are adopted wholesale.
...
### Correction 3 — the KV-cache premise is already broken, at the front of the prompt
...
turns" ([`memory.py:2037-2040`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/memory.py)) —
...
([`preference_tools.py:64`](https://github.com/amd/gaia/blob/main/hub/agents/email/python/gaia_agent_email/tools/preference_tools.py)) —
...
the embedding model is unreachable, or after `clear_embeddings()` nulls every
procedure vector on an embedder change
([`memory_store.py:1500-1512`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/memory_store.py)),
...
[`:20-27`](https://github.com/amd/gaia/blob/main/hub/agents/email/python/gaia_agent_email/tools/preference_tools.py).
...
| `tool-hint` | 1 | `{tools: [name, …]}` | Contributed to the loader's `skill_tools` list |
...
sibling table, for the three reasons in [Correction 1](#correction-1-extend-procedures-is-wrong-one-database-is-right).
...
|---|---|---|---|
| `memory_store.py` | WAL + lock + migration harness ([`:373-470`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/memory_store.py)); the `procedures` accessors and their lineage semantics; `clear_embeddings` ([`:1500`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/memory_store.py)) | `_SCHEMA_SQL` + a v3→v4 marker bump; `clear_embeddings` should also null `skill_deltas.embedding` | `skill_deltas` table + its five accessors |
| `skill_synthesis.py` | `DistilledProcedure.parse` / `to_skill_md` ([`:180`, `:255`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/skill_synthesis.py)); `SynthesisConfig` + `load_synthesis_config` override pattern ([`:107`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/skill_synthesis.py)); `cluster_by_goal`'s deterministic seeding ([`:500-502`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/skill_synthesis.py)); `SIMILARITY_TAU` | A `DeltaConfig` sibling section in `memory_settings.json`; the ADD/UPDATE/NOOP *shape* re-applied to deltas keyed on `(base, anchor, kind)` instead of embedding cosine | The tier classifier; the delta reconciler; consolidation / umbrella merge |
| `procedural_memory.py` | The FAISS add/search/rebuild trio ([`:50`, `:118`, `:145`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/procedural_memory.py)); the change-detect-then-rebuild guard ([`:418-425`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/procedural_memory.py)); the off-state ladder in `recall_skill`'s docstring ([`:196-207`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/procedural_memory.py)) — the model for this doc's off-state table | Give the delta corpus its **own** index; do not add deltas to `_proc_faiss_index` | Session-boundary resolution instead of per-turn refresh; the structural-boundary fix for [#2676](https://github.com/amd/gaia/issues/2676) |
...
| **Embeddings cleared after an embedder change** ([`memory_store.py:1500`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/memory_store.py)) | Same as above: tier-3 dark until re-embedded; anchored tier-1/2 deltas keep applying. |
...
### Phase −1 — Prerequisite fixes on the shipped path (PROPOSED, unblocks everything)
...
| Embedding clear | `clear_embeddings` nulls procedure vectors | `memory_store.py:1500` | **Exists** — off-state input |
...
| Synthesis intermediate | `Skill` (4 fields) — **being renamed `DistilledProcedure`** | `skill_synthesis.py:158` | **Exists**; rename in flight ([#2671](https://github.com/amd/gaia/issues/2671)) |
...
| Recall→prompt | `_build_recalled_skills_prompt` (character-offset cut) | `procedural_memory.py:334`, `:353` | **Exists** — violates atomicity ([#2676](https://github.com/amd/gaia/issues/2676)) |
...
| Tier-0 precedent | `email_preferences` table + `set_priority_sender` | `hub/agents/email/python/gaia_agent_email/tools/preference_tools.py:64`, `:355` | **Exists** — the tier-0 worked example |
...
([Correction 3](#correction-3-the-kv-cache-premise-is-already-broken-at-the-front-of-the-prompt)).
