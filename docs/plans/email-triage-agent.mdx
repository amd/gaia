...
| **Slack brief delivery (webhook)** | POST formatted Block Kit message to user-configured `SLACK_WEBHOOK_URL` — see §12.18 | ~50 LOC Python + 1 config field |
...
with the installer; the Settings UI shows a one-click "Enable Gmail" / "Enable Outlook"
flow driven by §11 Auto-Discovery.
...
|----------|----|----|----|
| **P0 (must-ship)** | Master on/off toggle (§13.1), basic Email panel with Daily Brief placeholder (§12.3 stripped to email-only), Thread view with send-confirm modal (§12.4 core subset), "Connect" flow for one provider (§12.1 manual-email-entry variant), empty state (§12.12), observable kill switch (§13.6) | Auto-discovery across OS signals (§11.1 full), Speech-act badges + priority "why" tooltip (§12.4), Daily Brief calendar section (§12.3 C1 data sources), error/offline states (§12.12) | Split Inbox tabs (§12.5), Agent Inbox panel (§12.6), Inbox-Zero mode (§12.7), Activity Feed integration (§12.10), full Notifications (§12.14) |
| **P1 (ship if time)** | Search box (§12.9) using MCP search passthrough | Compose ghost-text (§12.8), keyboard shortcuts subset (§12.13: j/k/e/r/s/l) | Custom AI Labels management UI (§12.2), drag-to-train (§12.5), voice-first brief readout (§12.15), full keyboard shortcut set (§12.13) |
| **P2 (nice-to-have)** | — | Observability surfaces (§12.11), accessibility polish (§12.16) | Voice approval during triage review (§12.15), model-tier advanced overrides (§12.2), per-recipient profile browser (§12.2), mobile-ready data model (§12.17) |
...
### 12.18 Slack as an Output Channel
...
| 22 | **Slack webhook output channel** (§12.18 MVT tier) — block-kit formatter, config field, `gaia email slack-setup` | 0.5d | 0.2d | |
| 23 | Documentation: [`docs/guides/email.mdx`](../guides/email.mdx) + SDK cross-reference | 0.5d | 0.1d | |
...
| 29 | **Slack MCP bidirectional integration** (§12.18 C1 Polish tier) — pre-configured Slack MCP server, auto-registered tools (`send_slack_message`, `read_channel`, `search_slack`), DM-based query flow | 1d | 0.4d | ║ (with #17) |
| 30 | **Slack interactive approval flow** (§12.18 C2 tier) — Slack app + Events API + Block Kit approve/edit/reject buttons for drafts | 2d | 0.8d | |
| 31 | Documentation: [`docs/guides/email.mdx`](../guides/email.mdx) expanded, [`docs/sdk/sdks/email.mdx`](../sdk/sdks/email.mdx) | 1d | 0.2d | |
...
gaia email slack-setup                    # Configure Slack webhook URL (§12.18 MVT)
...
|-------|-------|--------------|
| [#737](https://github.com/amd/gaia/issues/737) | `[Connector Hub Phase 2] Token-auth connectors: Slack, GitHub, Notion` | **Directly covers our Slack auth story** — ships a Slack connector with vault-backed token storage, lifecycle (connect/test/disconnect/rotate), and per-agent enablement. If this lands, §12.18 C1 Polish (Slack MCP bidirectional) reduces to wiring an existing connector rather than writing fresh integration code. |
...
| [#690](https://github.com/amd/gaia/issues/690) | `Messaging security: restricted default tool set and input sanitization` | Applies to our Slack bidirectional path (§12.18 C1) — Slack DMs are untrusted input per [Security Model](security-model.mdx) §12. |
| [#689](https://github.com/amd/gaia/issues/689) | `Messaging adapter rate limiting infrastructure` | Applies to §12.18 C2 interactive approval flow (Slack rate limits). |
...
- [MITRE — Indirect Prompt Injection](https://atlas.mitre.org/techniques/AML.T0051.001)
