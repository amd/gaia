...
just consented to: there was no interpreter, test runner or package manager in
the allowlist, and nothing could write its output anywhere.

**What comes off, together, and only under bypass:**

| Gate | Default | Under bypass |
|------|---------|--------------|
| Shell operator block (`>`, `>>`, `<`, `&`, `` ` ``, `$()`, newline) | refused before parsing | parsed and executed |
...
| Shell rate limit (10/min, 3/10s) | paced — a command over the cap waits its turn | lifted |

Pipes and chaining (`\|`, `&&`, `\|\|`, `;`) were never blocked, or have since
stopped being: every step of a chain runs by default, judged on its own binary.
Bypass changes nothing there.

Redirection has one exception. A command that is *only* a skill-granted CLI —
`gh issue list > out.txt` with `shell:execute:gh` loaded — is handed its
arguments directly instead of to a shell, so the `>` would arrive as one more
word for `gh` to read and the file would never be written. That case is refused
with an explanation rather than run. Chain it with anything else and the shell
performs the redirect as usual.

**Blast radius: arbitrary code execution.** The developer set adds `node`,
`npm`, `make`, `cmake`, `go`, `cargo`, `sed`, `awk`, `curl`, `sleep`, `timeout`,
`export`, `cp` and `mv`. It also names `python`, `python3`, `pytest`, `gh` and
`git`, which are **consolidation rather than new reach** — those already have
paths today (`execute_python_file` for the `generic_file_ops` profiles, a
`shell:execute:<binary>` skill grant for `pytest`, `gh` and `git`). Listing them
here means bypass has one answer to "may this binary run" instead of three. The
per-skill grant path is untouched and still works with bypass off.

Naming `git` means its policy's outright refusals — push, reset, rebase, the
ones a grant cannot approve — come off under bypass with everything else. Same
reasoning as `rm` below, inverted: that refusal is a tripwire rather than a
boundary, since `python` and `make` in this same set reach git by shelling out.
Omitting it would have left bypass *stricter* than the default tier, where git's
read-only floor still answers `git status`.
...
**Stdio transport only.** The shell-guardrail lift exists on the sidecar's stdio
transport, which is one local parent process on a private pipe. It is not
reachable on the HTTP transport
(`hub/agents/gaia/python/gaia_agent/server.py`): that is a bound socket, and an
unguarded shell reachable over it is remote code execution rather than a relaxed
permission model. The HTTP handler pins `bypass_permissions = False`, and the
request model forbids unknown fields so a client cannot ask for it.

HTTP *does* have `POST /v1/gaia/sessions/{id}/bypass`, but it grants only the
other half of bypass — running gated tools unasked. Both halves ride one
`PermissionState`, so that class takes `lifts_shell_gates`, false for every HTTP
session: without it, `attach` would write the session's bypass onto each turn's
fresh handler and silently undo the pin above. Token auth and
