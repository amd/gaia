...
single plan. Security is GAIA's primary differentiator against competitors. While
OpenClaw has 42,000 exposed instances, 63+ CVEs, and 1,184 malicious skills on its
marketplace, GAIA enforces a defense-in-depth model: localhost-only communication,
...
| RAG cache | Pickle deserialization (RCE) | `pickle.load()` on cached data (`src/gaia/rag/sdk.py:1620`) | Migrate to safe serialization (#447) |
...
**Strategy doc requirement; prerequisites for #647 (skill marketplace).**

The SKILL.md format specification, permission model, security tiers, and sandboxing
architecture are defined in [skill-format.md](skill-format.md) — the canonical
...
- Three security tiers: AMD Verified (code-signed), Community Reviewed, Experimental
...
4. **Rate limiting:** Per-user, per-channel, and global rate limits (see
   `messaging-integrations-plan.md` Section 4.4 for configuration).
...
document indexes (line 1620). Pickle deserialization of untrusted data can lead to
...
|---------|----------|-------------|
| Shell command whitelist | `src/gaia/agents/chat/tools/shell_tools.py:268` | `ALLOWED_COMMANDS` set restricts CLI tool to read-only commands |
...
| Subprocess timeout | `src/gaia/mcp/external_services.py:67` | `timeout=30` on MCP subprocess calls |
| Tool registry validation | `src/gaia/agents/base/agent.py:1052` | Rejects unregistered tool names |
| Required argument checking | `src/gaia/agents/base/agent.py:1079` | Validates tool arguments before execution |
