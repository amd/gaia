...
**Companion plan:** [`installer.mdx`](./installer.mdx) covers the **Python CLI** install path. This document covers the **Electron desktop app** installer — which is the **primary install path for non-developer end users**.
</Note>
...
|---|---|---|
| **Windows NSIS** | Wizard checkbox: "Start GAIA when I sign in to Windows" → writes `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` pointing at `gaia-ui.exe --minimized` | **ON** (matches Slack/Discord/Zoom convention) |
...
| **Linux DEB** | `postinst` does NOT write autostart by default (Debian convention). Setting available in-app, writes `~/.config/autostart/gaia-ui.desktop`. | OFF (in-app setting) |
...
| **Linux DEB** | `apt remove gaia-ui` → tier 1; `apt purge gaia-ui` → tier 3. `prerm` script stops the GAIA Agent UI Electron process only (does NOT kill the user-launched `gaia chat --ui` backend, which the user may want to keep running). `postrm` handles purge cleanup. | Native `apt purge` |
...
**One implementation, four front-ends.** Lives at `src/gaia/installer/uninstall_command.py`. Already listed as a "future command" in the existing [`installer.mdx`](./installer.mdx) (lines 437–442); this plan implements it for real.
...
- NSIS installer template at `installer/nsis/installer.nsh` — custom pages for license acceptance and the auto-start checkbox
...
├── nsis/
│   ├── installer.nsh                  # Custom NSIS pages (license, auto-start checkbox)
...
|----------|--------|-------------|----------|---------------|-------------|------------|-------------|
| Windows x64 | `.exe` (NSIS) | Yes | Start Menu + Desktop | Yes (`.blockmap`) | Yes (SignPath) | Wizard checkbox (default ON) | ~85 MB |
...
**Action needed:** Submit application at [https://signpath.org/products/foundation](https://signpath.org/products/foundation).
...
- [ ] On Windows, opt into auto-start at sign-in via the wizard checkbox (default ON)
...
| **Auto-start ON default upsets some Windows users** | Low | Low | Wizard checkbox is highly visible; in-app setting easy to find; matches Slack/Discord/Zoom convention |
...
- Companion plan: [`installer.mdx`](./installer.mdx) — Python CLI install, `gaia init`, `gaia update`, and (post-Phase D) `gaia uninstall`
...
- SignPath OSS program: [https://signpath.org/products/foundation](https://signpath.org/products/foundation)
