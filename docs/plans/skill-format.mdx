...
<Info>
  ...
  **Still proposed (not written yet):** the `/api/skills` router and Agent UI panel, every `gaia skill install|remove|search|publish|update` verb — the CLI half of the marketplace, so nothing calls the hub endpoints above yet — `gaia skill migrate` (Phase 3), the permission sandbox for local-capability domains, and tier-ceiling enforcement ([#1019](https://github.com/amd/gaia/issues/1019)). Every such symbol is marked **PROPOSED** where it appears.
</Info>
...
The **server** half of the marketplace exists ahead of the CLI verbs above: the
Agent Hub Worker serves `POST /publish/skill` and a `skill` catalog lane
([#2467](https://github.com/amd/gaia/issues/2467)). Nothing in `gaia skill` calls
it yet — `publish` is still PROPOSED — so today the endpoint is reachable only by
a direct HTTP POST, and the hub has no published skills.
...
### Phase 4 — Marketplace + consumer wiring (PARTIAL; #647 / #2467 / #887 / #1451)

The **hub side shipped** with [#2467](https://github.com/amd/gaia/issues/2467): a
`skill` catalog lane, the `skills/` R2 namespace, `POST /publish/skill` with
server-side validation of this grammar, and the [#2468](https://github.com/amd/gaia/issues/2468)
audit gate. **Still PROPOSED:** every `gaia skill` verb that would use it
(`publish`/`install`/`search`/`remove`/`update`), the lock file, and install-time
tier enforcement; plus #887 emitting conformant `SKILL.md` and #1451 unioning
`tools_required`.
...
The enforcement and REST/UI layers are still greenfield. The marketplace is split:
its **hub-side** catalog lane and publish validation exist, while every `gaia
skill` verb that would talk to them does not.
...
| Hub catalog lane (server side) | `skill` catalog `type`, `POST /publish/skill`, `skills/` R2 prefix, SKILL.md publish validation, audit gate | `workers/agent-hub/src/` | **Exists** (#2467) — no client verb calls it yet |
