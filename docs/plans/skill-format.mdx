...
| `shell` | `execute`, `none` | One named CLI, scoped: `shell:execute:gh`. | **Bridged** — grants that binary; reads run, writes ask |
...
- `shell:execute:<binary>` has its own bridge — see
  ...
  three-tier subcommand table: reads run, a short list of writes asks the user
  per call, and every escalation class is refused outright.
...
1. **The binary must be policed.** `BINARY_POLICIES`
   ...
   maps each declarable binary to its subcommand table. A binary with no entry
   cannot be declared — the refusal names what *is* declarable.
2. **Subcommands are an allowlist, in three tiers.** Reads run with no prompt:
   for `gh`, `issue list|view|status`, `pr list|view|diff|checks|status`,
   `repo list|view`, `release list|view`, `run list|view`, `label list`,
   `search …`, `auth status`, and `gh api` for GET. A short list of writes is
   **confirmable** — `issue create|comment|edit`, `pr comment`,
   `label create|edit` — meaning the user is shown the exact command and answers
   yes / no / always-for-that-command-class. Everything absent is refused, so
   `gh auth token` (prints the credential), `gh alias` / `gh extension` /
   `gh config` / `gh codespace` (arbitrary code or a remote shell), and the
   irreversible verbs (`pr merge`, `issue close`, `label delete`, `repo delete`)
   are blocked without being enumerated — and are never offered as a prompt.
   Collapsing "refuse" into "ask" is what turns a gate into a habit; the tiers
   stay separate for that reason.
3. **Flags that escalate are refused, even on a confirmable write.** `gh api`
   rejects `-X`/`--method` with anything but `GET`, rejects `-f`/`-F`/`--field`/
   `--raw-field`/`--input` (a body implies a write), and rejects `graphql`. A
...
4. **The binary must be installed, unless something substitutes for it.**
   `load_skill` checks `PATH` and refuses with the install command when the
   binary is missing. A skill that loads without the tool it documents produces
   confident answers from no data — the failure this bridge exists to prevent.
   The exception is a policy that names a `substitute` (today only `pytest`,
   which usually lives in a virtualenv rather than on `PATH`). That skill loads
   without the grant, and its prompt says the command is unavailable and what to
   do instead — for `pytest`, call `pytest.main` from `execute_python_file`.
...
**The grant replaces the per-call prompt for READS only.**
...
to ask denies. So a *read* fully covered by an active grant skips the prompt: the
user already consented, in front matter, to one binary and a read-only table. A
confirmable write does not: the grant declares which writes may be *offered*, and
the user still approves each one. Everything else is unchanged —
`Agent._call_is_pre_authorized` asks the host's `skill_grant_covers_call` hook,
which answers False unless **every** segment of the command is a granted binary
running an allowed read. `pwd` still prompts. So does `gh issue list | head`:
consent was for `gh`, not for a
...
reachable today through the policed binary bridge and prompts at `community`;
