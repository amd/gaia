...
<Info>
  ...
  **Still proposed (not written yet):** the `/api/skills` router and Agent UI panel, `gaia skill update`, `gaia skill migrate` (Phase 3), and the permission sandbox for local-capability domains ([#1019](https://github.com/amd/gaia/issues/1019)). Every such symbol is marked **PROPOSED** where it appears.

  **Shipped since:** the marketplace verbs `gaia skill search|install|remove|publish` plus `keygen`/`trust`, install-time tier-ceiling enforcement, and the declarative `skills:` block ([#2467](https://github.com/amd/gaia/issues/2467)).
</Info>
...
**Still PROPOSED** — `update` (marketplace) and `migrate` (Phase 3):
...
<Note>
**The proposed verbs are absent, not stubbed.** `gaia skill migrate` is not a
...
| **Enforcement layers not yet shipped** | `/api/skills`, `migrate`, and the sandbox degrade to **absent**, not permissive: an unenforceable permission refuses the load rather than loading unguarded. Install-time tier ceilings now ship. |
| **Tier signal unavailable** | Falls to `experimental` — the most-restrictive tier — never to a more-permissive default. Imported skills are re-stamped `experimental` regardless of the tier they claim, and an unsigned or untrusted-key install is collapsed to `experimental` however high it claims. |
...
- **Still greenfield, still PROPOSED:** the local-capability permission sandbox
  ...
  `gaia skill update|migrate`. Do not build against these.
...
signing, install-time tier-ceiling enforcement, and `skill-lock.json`. An earlier
revision of this document said the #888 symbols were "verified absent on `main`";
that is no longer true of either set.
</Warning>
...
| **`gaia skill update\|migrate`** | — | **NOT FOUND** | **Greenfield — PROPOSED** (marketplace / Phase 3) |
...
| ☑ CLI: `gaia skill list`, `install`, `remove` | [CLI](#cli) | All three **shipped**, alongside `info`/`create`/`import`/`export`/`search`/`publish`/`keygen`/`trust` ([#2467](https://github.com/amd/gaia/issues/2467)). |
