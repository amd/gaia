...
└───────────────────────────────┬─────────────────────────────────────┘
  ...
  4. STORE    reconcile_and_store(candidate, store)            PROPOSED
     Mem0 ADD / UPDATE / NOOP; supersede when the LINEAGE's
     success_count dominates (Zep lineage) → row in procedures.
     The counts are the matched row's plus the episodes it was
     not built from, so an incremental window can still win and
     re-read history counts each session once.
                                 │
                                 ▼
...
`_synthesize_skills()` after consolidation, started through
`start_skill_synthesis()` on a background thread so a distillation call never
precedes the user's first answer. Each pass reads only history no earlier pass
consumed — the `skill_synthesis_watermark` in `meta` is the DETECT `since`, and
`synthesis_marks` records the episodes a pass handed to the distiller — and it
spends at most `max_distill_calls_per_pass` model calls, leaving the watermark at
the last fully consumed episode so the remainder is picked up next session.
...
| `max_clusters_per_pass` | `10` | Caps how many clusters one pass considers. |
| `max_distill_calls_per_pass` | `2` | Caps the LLM calls one pass actually spends. Each one holds the single local model slot the user's own turn needs, so a pass takes a small bite and leaves the rest for the next session. |
...
| **Malformed `agentskills.io` output** from the LLM | Failure | Skip **that cluster**, log, and mark its episodes `unusable` so the same call is not repeated on every session start. A response truncated at the model's output cap lands here too. `_synthesize_skills(force=True)` / `MemoryStore.reset_synthesis_progress()` re-opens them. |
