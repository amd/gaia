...
| Hand a shared file to the agent | The agent's own RAG and vision tools | Save the file to an inbox and tell the agent the path. Indexing in the bridge process would land in an index the agent child never sees. |
...
2. **Allowlist with no permissive default.** Refuse to start without one, mirroring
   `TelegramAllowlistError`. Exactly one member: the bridge has one agent and one
   conversation, so a second member would share the first's history and "always" grants. Pin
   the `team_id` too, so a token that ends up in another workspace is refused rather than
   served.
...
4. **Per-call approval over Block Kit.** A `tool_call` event renders as a message with
   **Allow** / **Always allow** / **Deny** buttons; the click becomes a `gaia_control`
   `tool_decision` line. No answer within ten minutes, or a prompt Slack refuses to post, is a
   deny — the agent itself waits forever, and one parked prompt would block the only agent.
   This is the single most valuable thing in the plan — it is what lets a remote surface hold
   real tools honestly.
...
6. **A read-only switch** (#690): the base agent already gates shell, file writes and code
   execution, so the default asks per call and `--deny-gated-tools` refuses that whole set
   outright. Reads stay ungated either way, and the setup confirmation says so.
7. **Outbound file upload is exfiltration.** Only a file a write tool was called with is sent,
   only after its result reports no failure, and only from an allowlisted set of roots — the
   approval covered the write, not the transfer off the machine.
...
- **Files in:** download the share into `~/.gaia/slack/inbox/` under a sanitized name, and
  tell the agent the path so it opens or indexes the file in its own process.
...
- **Liveness:** Socket Mode needs no inbound port, so there is no health server. `gaia slack
  status` checks the recorded pid with psutil and confirms the process is a Slack bridge, so a
  stale pid file cannot name an unrelated process.
- **Mid-run questions:** the stdio wire cannot deliver an answer, so a `needs_input` is posted
  and the turn cancelled; the conversation survives, and the user's reply continues it.
...
approval marshalling, and the streaming throttle. Per-platform adapters supply
