...
<Info>
  **Source Code:** [`src/gaia/agents/base/tool_loader.py`](https://github.com/amd/gaia/blob/main/src/gaia/agents/base/tool_loader.py) · bundles [`hub/agents/chat/python/gaia_agent_chat/tool_bundles.py`](https://github.com/amd/gaia/blob/main/hub/agents/chat/python/gaia_agent_chat/tool_bundles.py)
</Info>
...
2. **Place the dynamic tool block at the *end* of the prompt**, after all stable
   ...
   first byte that differs, so the volatile part must come last. On the native
   path the block's position is the *template's* choice, not ours — several
   chat templates render `tools=` **before** the system prompt, so the tool list
   is the prompt's prefix and everything after it re-prefills when it changes.
3. **Order the offered tools by admission, never alphabetically.** With the
   list sorted, admitting one tool mid-turn moved every tool after it and cost a
   re-prefill of the whole block. Appending costs only the tail. This is the one
   lever that works on the native path, where rule 2 is not ours to set.
...
[`tool_bundles.py`](https://github.com/amd/gaia/blob/main/hub/agents/chat/python/gaia_agent_chat/tool_bundles.py):
...
accumulated into a session-scoped *loaded set* that only grows, returned in
**admission order** (CORE in registry order, then each later admission appended)
and never re-sorted. The query embedded is the **previous user message +
current**, trailing 4,000 chars (Open Q4). Below the cap the set is monotonic;
at the cap a non-CORE tool is **LRU-evicted** (oldest `last_call_ts`, falling
back to load time for never-called tools); CORE and tools admitted this turn are
exempt; if nothing is evictable the candidate is skipped and logged. Each turn
emits one structured `TOOL_LOADER {json}` INFO line (scores, matched,
bundle_pulled, admitted, evicted, skipped, loaded) for Part-2 tuning and the
recall gate.

**Caching seam.** The system prompt is recomputed **only** when the selection
changes — compared as an ordered list — so steady-state turns serialize
byte-identically and the backend KV prefix stays warm. Admission order is what
makes a *growing* turn cheap too: a newly admitted tool is appended, so every
byte before it is unchanged and only the tail re-prefills. (Alphabetical order
re-shuffled everything after the insertion point, which is why one mid-turn
`load_tools` used to cost a near-full re-prefill of the tool block.) Eviction
therefore happens **only at a turn boundary**, inside `select`; mid-turn the set
is add-only. When a filter is active the tools block moves **after** the
response-format template (volatile content last); with no filter the legacy
...
[`DOC_CORE_TOOLS`](https://github.com/amd/gaia/blob/main/hub/agents/chat/python/gaia_agent_chat/tool_bundles.py)
...
**`load_bundle` is add-only.** It resolves a bundle name (or a bare tool name,
via the reverse index) and appends members in registry order. It does **not**
evict to stay under `max_tools`: it runs mid-turn, and dropping a tool from the
middle of the offered list re-prefills every tool after it — the cost the escape
hatch exists to avoid. The overshoot is bounded (one bundle per call), logged
with an `over_cap` count, and trimmed LRU-first by the next `select`. It emits a
same-turn `TOOL_LOADER {…, "event": "load_tools", …}` superset line.
...
1. **Bundle definitions and CORE membership** — *decided.* CORE = 10 names and 12
   bundles, in [`tool_bundles.py`](https://github.com/amd/gaia/blob/main/hub/agents/chat/python/gaia_agent_chat/tool_bundles.py)
