...
| `src/gaia/agents/tools/_email/gmail.py` | New — relocated from `gaia_agent_email/gmail_backend.py` (Phase 4) |
...
**Blocker first:** [#2983](https://github.com/amd/gaia/issues/2983) — the flagship's
scorecard adapter and release gate now exist (`release_agent_gaia.yml`), but no eval
run has produced a card yet, so there is still nothing to regress against. Committed
baselines exist only for `rag_quality`, `tool_selection`, and `context_retention`
(`tests/fixtures/eval_baselines/gemma-4-e4b-d71cd914/`). An email category for the
flagship has to exist before "no regression" means anything for email work.
...
**Phase 4** — Gmail / Google connector.
