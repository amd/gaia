...
The **credentials** are shared; the **code is not**. Each core backend is a lean
reimplementation of the read half of its sidecar counterpart, because the flagship
must not depend on the package it is meant to replace — and because the sidecar's
backends return raw provider resources, leaving the flattening to a tool layer that
does not come along. Phase 0's `graph.py` is 242 lines against
`outlook_backend.py`'s 722; Phase 4's `gmail.py` is 425 against
`gmail_backend.py`'s 1256.
...
| `src/gaia/agents/tools/_email/graph.py` | New — read-only Microsoft Graph backend (Phase 0) |
| `src/gaia/agents/tools/_email/gmail.py` | New — read-only Gmail backend, reimplemented (Phase 4) |
| `src/gaia/agents/tools/_email/scopes.py` | New — declared vs read-capable scopes; keeps the `https://mail.google.com/` exclusion (#2533) |
...
**Phase 0 — read-only Outlook triage from the TUI.** One PR. `EmailToolsMixin`
registers `check_mailbox_access`, `list_inbox`, `search_email`, `read_email`, and
`list_mail_folders` in `KNOWN_TOOLS`; the `inbox-triage` skill supplies the triage
procedure. The Microsoft connector requests `Mail.ReadWrite` to match its existing
consent contract, while these five tools expose reads only. No writes, no state
DB, no Gmail, no calendar. Acceptance: `triage my inbox` works end-to-end in
`gaia tui` against a real Outlook account, and `tool_selection` does not regress
...
**Phase 4** — Gmail / Google connector. The same five read tools, now backed by
either mailbox. Google's consent declares `gmail.readonly` only, while the scope
actually requested at token time resolves against what the connection already
carries, so an existing `gmail.modify` user is never sent through a browser
reconnect. When both mailboxes are usable, Gmail wins on registry order, and the
way to change that is to revoke the flagship's grant for it — selection stays
connector-derived, with no environment-variable override.

`read_email` caps one body at 12,000 characters and says so in the result when it
truncates — roughly 12% of the 32K NPU context at the worst measured density
(3.0 characters per token, for quoted thread history). **The cap is per message,
not per turn:** an agent that reads eight messages while triaging can still
exhaust the window, and nothing accounts for a turn's cumulative body budget.
Tracked separately.
