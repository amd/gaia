...
<Note>
✅ **Shipped.** Production agents (chat, doc, file, code, analyst, browser, email, jira, docker, sd, emr, docqa, routing, …) already live as standalone packages under `hub/agents/<id>/python/`; `src/gaia/agents/` retains only the framework (base classes, shared tool mixins, registry). The remainder of this document is the still-in-progress hub platform plan.
</Note>
...
1. Move agent code from `src/gaia/agents/<name>/` to `hub/agents/<name>/python/gaia_agent_<name>/`
