...
|---|---|---|---|
| 1 | **`gaia eval agent` scorecard** | Behaviour, not speed. Four LLM-affecting surfaces changed: the system prompt lost 2,832 tokens, its section order changed, the tool list is no longer restated in prose, and the model now sees ≤26 of 67 tools per turn. A quality regression is invisible to every unit test here. | `gaia eval agent --category <cat>`. Quiet box, one eval at a time. The `gemma-4-e4b-d71cd914/` baseline this row originally diffed against scored the `doc` ChatAgent profile and has since been deleted; once a flagship baseline lands in `tests/fixtures/eval_baselines/gaia-flagship/`, add `--compare tests/fixtures/eval_baselines/gaia-flagship/scorecard_<cat>.json <run>/scorecard.json`. Until then report the scores measured, not a verdict. |
