...
<Note>
`AGENT_MODELS` in [`src/gaia/api/agent_registry.py`](https://github.com/amd/gaia/blob/main/src/gaia/api/agent_registry.py) is empty by default — the server starts and `/v1/models` returns an empty list until you register an agent there. See [Adding New Agents](/reference/api-spec#adding-new-agents).
</Note>
...
`AGENT_MODELS` in [`src/gaia/api/agent_registry.py`](https://github.com/amd/gaia/blob/main/src/gaia/api/agent_registry.py) is empty by default — the server still starts and serves an empty `/v1/models` list. Register an agent there to expose it as a selectable model.
