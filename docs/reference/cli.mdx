...
`gaia config`, `gaia connectors`, `gaia daemon`, `gaia diagnostics`, `gaia hub`,
`gaia init`, `gaia knowledge`, `gaia lemonade`, `gaia memory` or `gaia skill` —
passing one there fails with `unrecognized arguments`.
