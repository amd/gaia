...
Drive a running TUI (start one with `--control-port` first — both modes score
that live session, not a hidden subprocess):
```bash
gaia eval code --control ~/.gaia/tui/control.json      # Code benchmark, scored by running each task's tests
gaia eval sessions --control ~/.gaia/tui/control.json  # Replay real Claude Code sessions, judge-scored
gaia eval code --workspace ./scratch                   # Materialize task projects somewhere you can inspect
gaia eval sessions --dataset-only                      # Build the dataset and stop
```
...
local-capability permission (`filesystem`, `database`, `desktop`, `env`)
is **refused at every tier**, including `verified` — the sandbox that would contain
it is deferred ([#1019](https://github.com/amd/gaia/issues/1019)). The one exception
is `shell:execute:<binary>`, which is bridged: it grants that single CLI, limited to
the read-only subcommands in `BINARY_POLICIES`. Unscoped `shell:execute` is refused.
A `verified` stamp is a provenance claim, not a runtime sandbox.
</Warning>
...
<Warning>
**A skill declaring an unbridged local-capability permission is refused, not sandboxed.** `network` and `mcp` permissions bridge to the connector grant model and load fine, and `shell:execute:<binary>` bridges to a single named CLI restricted to the read-only subcommands in `BINARY_POLICIES`. `filesystem`, `database`, `desktop`, `env`, and an unscoped `shell:execute` need the permission sandbox, which is deferred ([#1019](https://github.com/amd/gaia/issues/1019)) — until then `load_skill` fails with an actionable error rather than loading the skill unenforced.
</Warning>
