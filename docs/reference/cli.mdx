...
| `--sd-model` | string | SDXL-Base-1.0 | Model: SDXL-Base-1.0 (photorealistic), SDXL-Turbo (fast), SD-1.5, SD-Turbo |
...
Photorealistic image (uses SDXL-Base-1.0 with auto-settings):
```bash
gaia sd "a sunset over mountains, photorealistic, detailed, 4k"
```

Fast stylized image with SDXL-Turbo:
```bash
