...
|--------|------|---------|-------------|
| `--profile, -p` | string | chat | Profile to initialize (minimal, chat, code, rag, all) |
...
| `--remote` | flag | false | Use remote Lemonade server (skip local install/start, still checks version) |

**Available Profiles:**

| Profile | Models | Description | Approx Size |
|---------|--------|-------------|-------------|
| `minimal` | Qwen3-4B | Fast setup with lightweight model | ~2.5 GB |
| `chat` | Qwen3-Coder-30B, nomic-embed, Qwen2.5-VL-7B | Interactive chat with RAG and vision | ~25 GB |
| `code` | Qwen3-Coder-30B | Autonomous coding assistant | ~18 GB |
| `rag` | Qwen3-Coder-30B, nomic-embed, Qwen2.5-VL-7B | Document Q&A with retrieval and vision | ~25 GB |
| `all` | All models | All models for all agents | ~26 GB |

<Note>
All profiles also include the lightweight `Qwen2.5-0.5B` CPU model used by `gaia llm` for quick queries.
</Note>
...
<Card title="Image Generation" icon="image" href="/guides/sd">
  Generate images using Stable Diffusion with AMD NPU acceleration
</Card>
...
| `--sd-model` | string | SD-Turbo | Model: SD-Turbo or SDXL-Turbo |
| `--size` | string | 512x512 | Image size: 512x512, 768x768, 1024x1024 |
...
```bash
gaia sd "a sunset over mountains, golden hour, 4k"
```

Use SDXL-Turbo for higher quality:
```bash
gaia sd "cyberpunk city at night" --sd-model SDXL-Turbo --size 1024x1024
```
