...
<Tabs>
  <Tab title="Windows">
    ...
    3. Prepare the backend and model with GAIA setup:
...
1. Prepare Lemonade and the configured model:
```bash
gaia init
```
...
<Tip>
Use `lemonade list` to see all available models and their download status.
</Tip>
...
[→ Agent Skills Spec](/spec/agent-skills)
...
<Note>
  `gaia-tui` is the Go binary; `gaia` is the Python CLI. The Go binary also accepts
  `gaia-tui tui status` as an alias for `gaia-tui status`. This does not add a
  `tui` or `status` command to the Python CLI.
</Note>
...
gate screen is never rendered: a script has nobody to press a key. The flagship normally uses a subprocess gate; a Hub binary install selects the daemon transport instead. See [transport selection](/reference/tui#transport-selection).

When a dependency is unavailable, stderr identifies the failed readiness check
and prints a remedy. Backend startup guidance depends on the installed Lemonade
application. Run `gaia init` to prepare the local backend before retrying; inspect
the reported endpoint if you configured a remote server.
