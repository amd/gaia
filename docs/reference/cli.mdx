...
| `--skip-device-check` | Skip the hardware compatibility check (AMD Ryzen AI Max or Radeon ≥ 24 GB VRAM) |
| `--base-url <url>` | Use a remote Lemonade server (e.g. `https://host:8000/api/v1`). Skips the device check since inference runs remotely. |
...
```bash Remote Lemonade Server (device check skipped)
gaia --ui --base-url https://your-server:8000/api/v1
```

```bash Force bypass device check
gaia --ui --skip-device-check
```
</CodeGroup>

<Note>
  `gaia chat --ui` continues to work as an alias. The Agent UI requires an AMD Ryzen AI Max (Strix Halo) or an AMD Radeon GPU with ≥ 24 GB VRAM. Set `GAIA_SKIP_DEVICE_CHECK=1` to bypass this check via environment variable.
</Note>
...
| `--skip-device-check` | flag | false | Skip hardware compatibility check when launching Agent UI |
