...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `--input` | path | – | Input file or directory (required unless `--list-configs`). |
| `--output` | path | auto | Output file/directory (auto-adjusted to format). |
| `--type` | `transcript`\|`email`\|`pdf`\|`auto` | auto | Input type. |
| `--format` | `json`\|`pdf`\|`email`\|`both` | json | Output format (`both` = json + pdf). |
