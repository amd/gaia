...
```bash
gaia skill {list,info,create,import,export} [OPTIONS]           # author locally
...
<Note>
`gaia skill migrate` (Hermes / OpenClaw import) is **not implemented** and does not parse — it lands in Phase 3. `search` / `install` / `remove` / `publish` shipped with the marketplace ([#2467](https://github.com/amd/gaia/issues/2467)); `skill_sets:` is [#2466](https://github.com/amd/gaia/issues/2466).
</Note>
