...
<Note>
`GAIA_HOME` moves the directory this command deletes into, and the resolved path is printed above the plan and in the `--purge` confirmation prompt. It must name a directory GAIA owns: if it resolves to your home directory, a directory containing it, or a drive root, `gaia uninstall` refuses and exits `64` rather than deleting `$GAIA_HOME/documents` — which on Windows and macOS is your real `Documents` folder. `--purge` additionally requires an existing `$GAIA_HOME` to be named `.gaia` or to hold something GAIA created (`config.json`, `chat/`, `lemonade/`, `memory.db`, …), so it will not eat the `venv/` of a directory that just happens to be pointed at.
</Note>
