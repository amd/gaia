...
2. **Installs Lemonade** - Downloads and installs from GitHub releases (Windows/Linux only)
3. **Starts Server** - Ensures Lemonade server is running
4. **Downloads Models** - Pulls required models for the selected profile
5. **Verifies Setup** - Confirms everything is working correctly
...
<Tip>
**Version Mismatch Warning:** If your installed Lemonade version doesn't match the expected version, `gaia init` will warn you and suggest reinstalling. Use `--force-reinstall` to automatically update.
</Tip>
