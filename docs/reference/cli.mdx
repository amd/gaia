...
| `--trace` | flag | false | Write the run's request and full agent event stream to `./email_trace_<timestamp>_<run-id>.json` (path echoed on stderr). One file per turn in interactive mode; contains real email content. |
