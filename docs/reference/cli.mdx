...
```bash
gaia skill {list,info,create,import,export,audit} [OPTIONS]
```
...
<Tabs>
  ...
  <Tab title="audit">
    ...
    Runs the pre-publish security audit ([#2468](https://github.com/amd/gaia/issues/2468)) over a skill's code *and* its instruction body, printing an `ALLOW` / `REVIEW` / `BLOCK` verdict with `file:line` findings and the fix for each. This is the same engine the **Skill Audit** PR check runs, so a clean local audit is the whole requirement for a skill-submission PR: `BLOCK` fails the check, `REVIEW` holds it for maintainer sign-off, `ALLOW` passes. Skills are contributed by pull request — there is no self-serve publish.
