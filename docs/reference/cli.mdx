...
| `--purge` | flag | Remove venv + chats + documents + embedded Lemonade + config + logs + installer binaries (`~/.gaia/bin`) + traces + daemon state (`~/.gaia/host`), and undo the installer's PATH edits |
...
```bash Full Purge (venv + chats + documents + embedded Lemonade + config + installer leftovers)
...
embedded Lemonade downloads, agent traces, and the daemon's state in `~/.gaia/host` —
including its custody database (agent memory, sessions, and audit history). Use `--purge --dry-run` first to preview the purge tier; pair other cleanup tiers with `--dry-run` to preview those instead.
</Warning>

<Note>
`--purge` also undoes what the one-line installer set up. It stops a running GAIA daemon
before deleting its state, removes `gaia-tui` and `gaia-agent` from `~/.gaia/bin`, and
removes the installer's PATH entries: the `# Added by GAIA installer` block in
`~/.zshrc`, `~/.bashrc`, `~/.bash_profile` or `~/.profile`, or the `~\.gaia\venv\Scripts`
and `~\.gaia\bin` entries on the Windows user PATH. The rest of each file and every
other PATH entry are left exactly as they were. If you edited the installer's `export PATH`
line by hand, it is kept and the command tells you to remove it yourself. A `GAIA_HOME`
other than `~/.gaia` leaves these PATH edits alone, because they still point at `~/.gaia`.
Open a new terminal afterwards so it stops using the old PATH.
</Note>
...
behind. `~/.gaia/traces` is never pruned otherwise — delete old runs yourself, or
remove them all with `gaia uninstall --purge`.
