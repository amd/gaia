...
agent (e.g. `email`) on a machine with no UI. Running the agent (e.g.
...
| `POST /daemon/v1/agents/{id}/install` | 202, queue an install (`{"version": "..."}` optional) |
...
gaia hub install email             # install the hub's latest version
gaia hub install email --version 0.5.0
...
| `install <id>` | `--version` | Download + verify + install, printing each phase. Exit 1 on any failure, with the reason. |
