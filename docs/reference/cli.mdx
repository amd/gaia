...
Your token goes straight to Lemonade, which holds it in process memory only.
GAIA never writes it to disk.
...
gaia gateway logout                   # drop the session token
...
| `--auth-header-name` | `install` | Non-standard auth header (default `Authorization`) |
| `--auth-header-prefix` | `install` | Header value prefix (default `Bearer `; `""` for none) |
...
does. For a token that survives a Lemonade restart, set `LEMONADE_AMD_API_KEY`
in **Lemonade's** environment.
