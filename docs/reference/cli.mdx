...
<Note>
For the `gaia`, `chat`, `rag`, `minimal` and `all` profiles the chat model follows the
hardware. Where Lemonade reports enough memory for it — about 92 GB for models, as on a
128 GB Strix Halo — `gaia init` sets up `user.Qwen3.8-Flash-Next-GGUF` (an 82 GB download)
in place of `Gemma-4-E4B-it-GGUF`, prints why, and records it as `default_model`. Every
other PC gets Gemma. A `default_model` you set yourself always wins, and `--check` asks for
