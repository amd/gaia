...
| `--base-url <url>` | Use a remote Lemonade server (e.g. `https://host:13305/api/v1`). Also accepted by the subcommands that talk to Lemonade: `chat`, `prompt`, `talk`, `llm`, `api`, `stats`, and `mcp start`. `gaia email` rejects it — see [Email Command](#email-command). |
...
<Note>
  `gaia chat --ui` continues to work as an alias. The Agent UI does not gate on your processor model — the Settings panel reports your processor, memory and model status for information only.
</Note>
...
An agent that runs out of steps before finishing still answers: it gets one more model call in which it can't use tools, and reports what it found, what it couldn't finish and why, and what you can do next. If that last call fails, you get the step-limit notice plus a line saying why the summary couldn't be written.

- **Per-invocation:** pass `--max-steps <n>` to `gaia chat` (e.g. `gaia chat --max-steps 80`).
...
| `--skip-models` | flag | false | Skip model downloads (only set up Lemonade Server) |
...
| `--force-reinstall` | flag | false | Reinstall GAIA's own Lemonade Server |
...
| `--remote` | flag | false | Use the Lemonade Server `LEMONADE_BASE_URL` names instead of GAIA's own (checks it, downloads models via its API). Requires `LEMONADE_BASE_URL`; implied when it is a non-localhost URL. |
...
| `--check` | flag | false | Report whether the profile is already set up and exit — no install, no download. A stopped GAIA Lemonade Server is started through the GAIA daemon (started too if needed), as any GAIA command would. Exit code `0` means ready, `1` means `gaia init` still has work to do. Used by the Go TUI's first-boot check and `/setup` command |
...
<Note>
For the `gaia`, `chat`, `rag` and `all` profiles the chat model follows the
hardware. Where Lemonade reports enough memory for it — about 88 GB for models, as on a
128 GB Strix Halo — `gaia init` also sets up `user.Qwen3.8-Flash-Next-GGUF` (an 82 GB
download) as the chat model, prints why, and records it as `default_model`. Gemma is still
downloaded, since vision and document-image extraction load it by name. Every other PC
gets Gemma alone. Qwen is also skipped, with the reason printed, when the running Lemonade
is older than v2026.39.1 (its llama.cpp cannot load the model): `gaia init --yes` keeps an
older Lemonade that meets the profile minimum, so upgrade with `gaia init --force-reinstall`. A `default_model` you set yourself always wins, and `--check` asks for
the same model `gaia init` would download.
</Note>

<Note>
On the same big-memory PCs, `Qwen3-30B-A3B-Instruct-2507-GGUF` (a 17 GB Lemonade
built-in) is the alternative: published Strix Halo benchmarks report it generating
several times faster than Qwen3.8 Flash (not yet measured on GAIA's build), but it
reads text only. Switch to it with
`gaia config set default_model Qwen3-30B-A3B-Instruct-2507-GGUF`, or pick it in the
terminal UI's model picker; Lemonade downloads it on first use. To measure both on your
own PC, speed, tool calling and (with `--tasks core`) agent quality, run
`python util/compare_local_models.py` from a GAIA checkout.
</Note>

<Note>
`gaia llm` quick queries use `default_model` when set, else `Gemma-4-E4B-it-GGUF`.
</Note>
...
```bash Use Another Lemonade Server
LEMONADE_BASE_URL=http://192.168.1.100:13305/api/v1 gaia init
```

```bash Reinstall GAIA's Lemonade Server
...
GAIA runs its own Lemonade Server: a private copy under `~/.gaia/lemonade`, on a
port only GAIA uses, behind a generated API key. It needs no admin rights and
never touches a Lemonade app you installed yourself.

<Tabs>
  <Tab title="GAIA's own server (default)">
    1. **Starts Lemonade Server** — downloads GAIA's copy on first run (checksum-verified), then starts it. Re-running replaces an older copy or one that stopped answering.
    2. **Downloads Models** — pulls the models for the selected profile (the NPU profile also installs the FLM backend)
    3. **Verifies Setup** — tests each model with inference to detect corrupted downloads
  </Tab>
  <Tab title="Another Lemonade Server">
    Set `LEMONADE_BASE_URL` to use a Lemonade Server you run yourself, on this
    machine or another (e.g., `http://192.168.1.100:13305/api/v1`). `gaia init`
    then installs and starts nothing: it checks that server and downloads models
    through its API.

    If that server requires authentication, also set `LEMONADE_API_KEY` —
    GAIA sends it as `Authorization: Bearer <key>` on every Lemonade-bound request
    (see [Lemonade docs](https://lemonade-server.ai/docs/guide/configuration/#api-key-and-security)).

    1. **Checks the Server** — it must answer and meet the profile's minimum Lemonade version
    2. **Downloads Models** — downloads models on that server via its REST API
    3. **Verifies Setup** — tests each model with inference to detect corrupted downloads
  </Tab>
</Tabs>

<Warning>
**Platform Support:** GAIA's own Lemonade Server runs on Windows and Linux (x64 and
arm64) and on Apple Silicon Macs. Intel Macs have no upstream Lemonade build — point
`LEMONADE_BASE_URL` at a Lemonade Server on another machine.
</Warning>
...
| `--silent` | flag | Silent installation (no UI, no desktop shortcuts) |
...
```bash Silent (no UI, no shortcuts)
gaia install --lemonade --silent
```
</CodeGroup>
...
| `--base-url` | string | `$LEMONADE_BASE_URL` or local | Lemonade server to query |
...
| `--base-url` | string | `$LEMONADE_BASE_URL` or local | Lemonade server to send the prompt to (e.g. `https://host:13305/api/v1`) |
| `--use-claude` | flag | false | Answer with the Claude API instead of Lemonade. Needs `ANTHROPIC_API_KEY` |
| `--claude-model` | string | `claude-sonnet-5` | Claude model used with `--use-claude`; `--model` is ignored then |
...
| `--base-url` | – | – | Rejected (exit 2). The email agent runs inside the GAIA daemon and uses the daemon's `LEMONADE_BASE_URL`; to switch servers, run `gaia daemon stop`, set `LEMONADE_BASE_URL`, and re-run `gaia email`. |
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `--model` | string | `Gemma-4-E4B-it-GGUF` | Model for the conversation (also what the Lemonade pre-flight loads) |
...
| `--use-chatgpt` | flag | false | **Removed.** Passing it exits with migration guidance — configure the model in Lemonade and use `--base-url` |
| `--claude-model` | string | `claude-sonnet-5` | Claude model, used with `--use-claude` |
...
<Card title="Slack" icon="slack" href="/guides/slack">
  Drive the flagship GAIA agent from a Slack direct message, over Socket Mode.
</Card>
...
| Subcommand | Option | Default | Description |
|-----------|--------|---------|-------------|
| `setup` | `--no-browser` | false | Print the pre-filled create-app URL instead of opening a browser. |
| `setup` | `--print-url` | false | Print only the create-app URL and exit, for a caller running its own prompts (the TUI's setup panel). |
| `start` | `--allowed-users` (required) | – | The one Slack member ID allowed to use the agent. Starting with none, or with more than one, is refused. |
| `start` | `--agent-command` | `gaia-agent` | Command that starts the agent child. |
| `start` | `--deny-gated-tools` | false | Refuse every tool that would ask for approval, instead of offering buttons. |
| `start` | `--upload-root` | your home directory | Directory a written file may be uploaded from. Repeatable. |
| `start` | `--background` | false | Record a PID file so `gaia slack stop` can find the process. |
| `stop` | – | – | Stop a bridge started with `--background`; clears a stale PID file. |
| `status` | `--json` | false | Emit detection, configuration, and liveness as JSON. |
| `connect` | – | – | Store a pair of tokens read from stdin — app-level token first, bot token second. Tokens never go in argv, where `ps` would show them. |
| `decline` | `--never` | false | Record that you don't want Slack set up; `--never` stops the TUI offering it again. |

[→ Slack Guide](/guides/slack)
...
#### `gaia mcp serve` options

Wraps the Agent UI backend's REST API as MCP tools, so an MCP client (Claude
Code, for example) can chat through the same backend that powers the browser
UI — conversations and tool activity show up there in real time. The Agent UI
backend (`gaia chat --ui`) must already be running.

| Option | Default | Description |
|--------|---------|-------------|
| `--host` | `localhost` | Bind address |
| `--port` | `8766` | Port for the MCP server |
| `--backend` | `http://localhost:4200` | GAIA Agent UI backend URL |
| `--stdio` | off | Use stdio transport instead of HTTP (Claude Code / eval runner integration) |
...
gaia connectors configure mcp-github --set GITHUB_PERSONAL_ACCESS_TOKEN=ghp_xxx
```
...
Runs GAIA agents on gateway-hosted models — larger on-prem models and frontier
models — alongside local inference. Requires **Lemonade Server 11.8.0+**.

You enter your token once. Lemonade holds it in memory and forgets it on
restart, so GAIA keeps a copy in your **OS credential store** (DPAPI, Keychain,
SecretService) and replays it — encrypted at rest, never in a GAIA file.

```bash
gaia gateway install --base-url https://llm-api.amd.com/Unified/v1  # register the gateway
gaia gateway auth                     # prompts for the token without echo
gaia gateway status                   # registration, auth state, active model
gaia gateway models                   # what the gateway offers
gaia gateway enable amd.Claude-Opus-5 # add it to your enabled set
gaia gateway disable amd.Claude-Opus-5 # remove it from your enabled set
gaia gateway use amd.Claude-Opus-5    # make it the default for chat/llm/prompt
gaia gateway test "Say hello"         # end-to-end round trip
gaia gateway auth --no-remember       # ask me every restart instead
gaia gateway logout                   # drop Lemonade's copy, keep the stored one
gaia gateway logout --forget          # also delete it from the credential store
gaia gateway uninstall                # remove the provider
```

| Flag | Command | Purpose |
|------|---------|---------|
| `--base-url` | `install` | Gateway OpenAI-compatible base URL |
| `--auth-header-name` | `install` | Auth header name (default `Ocp-Apim-Subscription-Key`) |
| `--auth-header-prefix` | `install` | Header value prefix (default empty — the AMD gateway takes a bare key) |
| `--skip-probe` | `install` | Register without first testing the gateway |
| `--token` | `auth` | Token value — prefer the prompt, which keeps it out of shell history |
| `--token-stdin` | `auth` | Read the token from stdin — best for scripts |
| `--no-remember` | `auth` | Do not store the token; you are asked again after a restart |
| `--forget` | `logout` | Also delete the stored token |
| `--model` | `test` | Model to test (default: the active gateway model) |

Gateway models are namespaced `amd.<model>` and work anywhere a local model
does. `LEMONADE_AMD_API_KEY` in **Lemonade's** environment still takes
precedence over a stored token; if both are set, Lemonade says so rather than
silently choosing.

[→ LLM Gateway Guide](/guides/llm-gateway)
...
| Key | Default | Purpose |
|-----|---------|---------|
| `default_model` | _(unset)_ | Default model ID for `gaia chat` / `gaia llm` / `gaia prompt`, new Agent UI sessions, and every agent started without a model (including the TUI's flagship). `gaia init` sets it to `user.Qwen3.8-Flash-Next-GGUF` on a PC with the memory for it and never overwrites a value you set |
...
```
✅ Embedded Lemonade 2026.39.1 running on http://localhost:62232/api/v1
...
| Option | Default | Description |
|--------|---------|-------------|
| `-d, --eval-dir` | `./output/evaluations` | Directory containing `.eval.json` files to analyze |
| `-o, --output-file` | `LLM_Evaluation_Report.md` | Output filename for the markdown report |
| `--summary-only` | off | Only display the summary on the console; don't save a report file |

<Warning>
  `gaia report` is registered but not currently dispatched ([#3488](https://github.com/amd/gaia/issues/3488)) — it prints top-level help and exits 0 instead of generating a report.
</Warning>
...
| `--agent-type` | string | `gaia` | Agent registration ID to score. Every scenario runs against this one agent, so a scorecard names a single agent and two scorecards are comparable. Override only to measure a different agent — and never compare the result to a scorecard captured under another one |
...
the suite decides pass/fail — no LLM judge. A TUI must already be running with
a control port, since this drives *that* session so the run is visible.
...
| Option | Default | Description |
|--------|---------|-------------|
| `--control` | – | Path to the running TUI's `control.json` |
| `--out` | `eval/results/code-<timestamp>` | Output directory |
| `--workspace` | a temp directory | Where to materialize the task projects |

---

### Flagship Agent Tasks

Run the flagship GaiaAgent on real coding, data and Q&A tasks and score what the finished project does. CI gates on passes, tested changes, quality, honesty, tokens, steps and runtime, against main's committed results.

```bash
gaia eval tasks run --suite core                   # Run the core tasks, then judge them
gaia eval tasks run --suite adversarial            # Only the trap tasks (red suite, flaky test, hang, ...)
...
gaia skill {search,install,remove,lock,promote,publish,keygen,trust} [OPTIONS] # marketplace + trust
```
...
The running daemon reloads schedules every second, so adding, pausing, resuming, removing, or editing a schedule does not require a restart. Pausing or removing a schedule prevents future runs; it does not interrupt a run already in progress.

A run that comes due while the daemon is running but the machine is asleep or busy still fires once the daemon catches up, as long as that is within an hour of the due time; several missed runs collapse into one. A run more than an hour late, or one that comes due while the same schedule's previous run is still going, is skipped and logged at WARNING with the schedule name. A run that came due while the daemon itself was not running is not replayed on restart.

Only one `gaia schedule daemon` can run per schedule store. A second one exits immediately with an error instead of firing every schedule twice. The daemon holds `~/.gaia/schedules.lock` while it runs; the OS releases it when the process exits, even after a crash. On Windows that error names no pid — Windows refuses another process's read of the locked byte, so the holder is unknowable.
...
A sink decides where each scheduled run's output goes. Set it with `--sink` on `add` (default `stdout`). `add` rejects a sink that could never deliver (an unknown name, `telegram` without `--to`, bare `file` without a path, or `notification` on Windows) and stores nothing. The Telegram bot token is only checked when a run delivers, because the daemon reads it from its own environment.
...
#### GAIA's Lemonade Server

The daemon starts GAIA's own Lemonade Server (the one `gaia init` installs) when
it comes up, and again whenever a GAIA command finds it stopped, so it is back
after a reboot without running `gaia init`. `gaia daemon stop` stops the server
only if this daemon started it; one you started yourself keeps running.

The daemon never installs Lemonade, never restarts a server that is slow to
answer (it may be loading a model), and never replaces one running a different
version. Those need `gaia init`. With `LEMONADE_BASE_URL` set, it leaves Lemonade
alone entirely.
...
gaia daemon stop       # graceful shutdown (tree-kills daemon + every sidecar, and the Lemonade Server it started)
...
|--------|---------|-------------|
| `start` | — | Start the daemon or attach to the running one (two callers → one daemon), and start GAIA's Lemonade Server if it is installed and stopped. |
| `status` | — | Print pid / port / uptime and the supervised sidecar list, or report if no daemon is running. |
| `stop` | — | Shut the daemon down (tree-killing every sidecar, then the Lemonade Server if this daemon started it) and release `~/.gaia/host/instance.json`. |
...
| `POST /daemon/v1/lemonade/ensure` | Start GAIA's embedded Lemonade Server if stopped and return where it listens (409 if not managed, 503 on a start failure). Host API ≥ 1.2 |
| `POST /daemon/v1/lemonade/start` | Start or attach the local model server; returns 503 with actionable detail on refusal. Host API ≥ 1.3 |
| `GET /daemon/v1/lemonade/status` | Whether this daemon supervises a server, its PID, and its log path. Host API ≥ 1.3 |
...
the recommended Fireworks models, and gateway settings.
...
- Report killed, refused, and failed listeners; exit 1 on any refusal or failure. An empty port is a successful no-op (exit 0). Listing and forceful termination commands each have a five-second timeout.
...
Most commands support these options. They come from a shared parent parser, so
despite the name they are **not** accepted by `gaia agent`, `gaia cache`,
`gaia config`, `gaia connectors`, `gaia daemon`, `gaia diagnostics`, `gaia gateway`,
`gaia hub`, `gaia init`, `gaia knowledge`, `gaia lemonade`, `gaia memory` or
`gaia skill` — passing one there fails with `unrecognized arguments`.
...
| `--claude-model` | string | `claude-sonnet-5` | Claude model to use when `--use-claude` is set |
| `--no-lemonade-check` | flag | - | Skip the Lemonade server reachability check (CI/testing without Lemonade) |

<Note>
  On commands with a nested subcommand, put these **before** the subcommand —
  `gaia eval --no-lemonade-check agent`, not `gaia eval agent --no-lemonade-check` —
  because the nested parsers don't inherit the shared parent. `gaia mcp start` is
  the one exception: it declares them itself, so there they must come *after*
  (`gaia mcp start --no-lemonade-check`).
</Note>

Backend flags are accepted only by the commands that use them, and any other command rejects them with a usage error (exit code 2):

| Flags | Accepted by |
|-------|-------------|
| `--use-claude`, `--claude-model` | `chat`, `prompt`, `talk` |
| `--model` | `chat`, `prompt`, `talk`, `llm`, `email` (and eval subcommands that define their own) |
| `--base-url` | `chat`, `prompt`, `talk`, `llm`, `email`, `api`, `stats`, `mcp start` |
| `--trace` | `chat`, `email` |
| `--max-steps`, `--list-tools`, `--stream` | `chat` |
