...
|---------|--------|-------------|-------------|
| `minimal` | Qwen3-4B | Fast setup with lightweight model | ~4 GB |
| `chat` | Qwen3-Coder-30B, nomic-embed, Qwen2.5-VL | Full chat with RAG and vision | ~25 GB |
| `code` | Qwen3-Coder-30B | Autonomous coding assistant | ~18 GB |
| `rag` | Qwen3-Coder-30B, nomic-embed, Qwen2.5-VL | Document Q&A with retrieval | ~30 GB |
| `all` | All models | Complete installation | ~35 GB |
...
3. **Starts Server** - Ensures Lemonade server is running
4. **Verifies Models** - Tests each downloaded model with a quick inference to detect corrupted downloads
5. **Downloads Models** - Pulls required models for the selected profile. Automatically re-downloads corrupted models.
6. **Verifies Setup** - Confirms everything is working correctly
...
<Tip>
**Corrupted Model Detection:** `gaia init` automatically verifies each model with a quick inference test. If a model is corrupted (e.g., partial download), it will be deleted and re-downloaded automatically.
</Tip>
