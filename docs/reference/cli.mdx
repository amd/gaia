...
1. **Checks Lemonade Server** - Detects if installed and verifies version compatibility _(remote mode: checks remote server version via API)_
2. **Installs/Upgrades Lemonade** - Downloads and installs from GitHub releases (Windows/Linux only). Automatically uninstalls old version if version mismatch detected. _(remote mode: skipped)_
3. **Checks Server** - Ensures Lemonade server is running (prompts to start if not) _(remote mode: verifies remote server is reachable)_
4. **Downloads Models** - Pulls required models for the selected profile _(remote mode: downloads models on the remote server via REST API)_
...
<Tip>
**Remote Mode:** Set `LEMONADE_BASE_URL` to a remote server URL
(e.g., `http://192.168.1.100:8000/api/v1`). Remote mode is auto-detected
when the hostname is not localhost/127.0.0.1/::1. This is equivalent to
passing `--remote`. Models are downloaded on the remote server via REST API.
</Tip>
...
[→ Full MCP Client Guide](/guides/mcp/client)
