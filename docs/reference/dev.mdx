...
resolved. There is no config block that removes the need for it: uv's
`[tool.uv.sources]` and `[[tool.uv.index]]` apply only to the project interface
(`uv sync`, `uv lock`, `uv add`), never to `uv pip install`, and `[tool.uv.pip]`
was rejected for the same reason — see the note in `pyproject.toml`. Omit the
flag and PyTorch resolves from PyPI as a CUDA
...
**Changed a backend pydantic model?** Regenerate the frontend's API types, or CI fails:

```bash
python -m gaia.ui.export_openapi                     # writes src/types/api.schemas.json
cd src/gaia/apps/webui && npm run gen:api-types      # writes src/types/api.gen.ts
npx tsc --noEmit                                     # checks the hand-written types still match
```

If `tsc` then flags `src/types/apiContract.ts`, update the matching interface in `src/types/index.ts`.
...
python util/lint.py --test-lanes  # reject test files that no CI lane runs
python util/lint.py --tool-descriptions # hold tool docstrings to the schema budget
...
<Note>
`--test-lanes` fails when a test file under `tests/` or a hub agent's `tests/` is
not named by any workflow's pytest step, because a file nothing runs looks green on
every PR whether it works or not. Add a new file to a lane (offline integration
files go in `test_unit.yml`'s `offline-integration` job), or list it in
`util/test_lane_allowlist.yml` with the reason CI can't run it.

Being named isn't the same as running. A file with a module-level
`pytest.importorskip` (for a hub wheel like `gaia_agent_chat`, or an optional
extra) skips wholesale in any lane that lacks the package. The unit lane and the
lanes that install hub wheels run `util/check_module_skips.py` right after their
install step. It fails on such a skip unless the file is listed
under `module_skips:` in the same allowlist, with `runs_in` naming the lane that
installs the package. That lane must name the file and run the check too.
</Note>
...
The packaged email-agent tests are hermetic and must not invoke an interactive
OS keychain. Run them with the null keyring backend (the email test package's
`conftest.py` and CI workflow apply this setting automatically):

```bash
PYTHON_KEYRING_BACKEND=keyring.backends.null.Keyring \
  python -m pytest hub/agents/email/python/tests/
```

On PowerShell, set the equivalent environment variable first:
...
### Seeing which tools the model was given

A `--trace` run writes `agent_output_<timestamp>.json`, and its `tool_schema`
block records what went out to the backend as `tools=` on that run — the tool
names, the per-turn filter (`null` when the full registry was sent), the
serialized schema size in chars and tokens, and the schema text itself. With
the file's `system_prompt` and `conversation` alongside it, a prompt can be
split into system / tools / history shares from one artifact, and a tool that
never reached the model is visible by its absence from `tool_names`.

The schema text is the bulky part. Set `GAIA_TRACE_TOOL_SCHEMA=0` to drop just
that text — the names and sizes stay, and the artifact records a
`schemas_omitted` note so a smaller trace never looks like a full one.
