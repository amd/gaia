...
resolved. Project-level uv source mappings do not apply to `uv pip install`;
they belong to the project interface (`uv sync`, `uv lock`, `uv add`). Omit the flag and PyTorch resolves from PyPI as a CUDA
