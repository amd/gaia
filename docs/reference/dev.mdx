...
Use this guide to install an editable Python checkout and run its checks. For the
Go terminal client, continue with [TUI development](/reference/tui#build-and-test).

## Before You Start

- **Git** and **uv** for the checkout and Python environment.
- **Python 3.10+**; the instructions below use 3.12 for repository tooling.
- **Go**, at the version declared in `tui/go.mod`, when changing the TUI.
- A configured **model backend** for live agent queries. Local inference uses
  Lemonade; cloud choices are described in [AI providers](/guides/ai-providers).

A Ryzen AI processor or NPU is not required to edit the harness, build the TUI,
or run tests that mock inference. Hardware, drivers, model downloads, and memory
requirements depend on the backend you test. Use [NPU setup](/guides/npu) only when
working on that device path. Linux, Windows, and macOS development commands appear
below; individual hardware integrations still need their target platform.
...
<Note>
On Linux use the flag for each **GAIA** install, with or without
...
> **Note:** Run `gaia init` to prepare and start the local backend. Normal runtime readiness checks report a stopped server; they do not start it automatically.

### Run the terminal from source

Follow [the TUI checkout launch instructions](/reference/tui#run-the-flagship-from-your-checkout)
to build the Go client and run the flagship and chat packages from this checkout.
An editable core install alone does not replace an installed agent executable.
