...
`AGENT_MODELS` in [`src/gaia/api/agent_registry.py`](https://github.com/amd/gaia/blob/main/src/gaia/api/agent_registry.py) is empty by default. `GET /v1/models` returns an empty list until an agent is registered there — see [Adding New Agents](#adding-new-agents) below.
