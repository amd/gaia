...
<AccordionGroup>
  ...
  <Accordion title="How do I uninstall GAIA?" icon="trash">
    ...
    - **Windows:** Settings → Apps → Installed apps → GAIA Agent UI → Uninstall
    - **macOS:** Drag GAIA Agent UI from `/Applications` to the Trash
    - **Linux (Debian/Ubuntu):** `sudo apt remove gaia-ui` (or `apt purge` to also remove data)
  ...
  <Accordion title="How do I update GAIA?" icon="arrow-up">
    ...
    To disable auto-updates, set the environment variable `GAIA_DISABLE_UPDATE=1` before launching. To trigger a manual check, the developer command is `gaia update --check`. See the [installation guide](/guides/install#updating-gaia) for more.
  </Accordion>
