...
Bars + the enforce switch live in `tests/fixtures/gaia/quality_gate_thresholds.json` (with a perf sibling). The quality manifest ships **`enforce: true`** with deliberately loose absolute floors (judged pass rate ≥ 0.60, avg score ≥ 5.0), so a breach fails the build; the perf sibling ships `enforce: false` until the first self-hosted-runner baseline calibrates it. Expect the quality bars to be retuned after that first Gemma run rather than assumed correct. CI: `test_gaia_agent_eval.yml` (PR subset / dispatch full / weekly via `weekly_eval.yml`), `gaia_scorecard_refresh.yml` (dispatch-only card refresh with the same basis guards as email's), and the `scorecard-gate` + `gaia-eval` release gates in `release_agent_gaia.yml`.
