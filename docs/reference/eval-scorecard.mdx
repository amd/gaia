...
**`--experiments` defaults to 1.** Measured run-to-run stdev on the email corpus is 0.0023 (CV 0.27%), so repeats cost hours of serial-slot time to tighten the CI by ~0.26pp. A single run records `stdev: 0.0`, and the gate's `--regression-floor-pp` supplies the noise tolerance instead. Raise `--experiments` only when you are specifically characterising variance.

## Aggregate metric — GAIA flagship (#2983)

The flagship is the second adopter, and it measures a different kind of thing. Email runs `gaia eval benchmark` over a **labelled corpus** and scores predictions against ground truth. The flagship runs `gaia eval agent` — a **scenario harness** whose answers an LLM judge scores 0–10 — so there is no accuracy to report and its aggregate is named **`weighted_judge_score`**: one metric per eval category, each category's value the mean judge score over its scenarios normalised onto [0, 1], weighted by how many scenarios that category contributed.

The adapter is [`hub/agents/gaia/python/packaging/gen_scorecard.py`](https://github.com/amd/gaia/tree/main/hub/agents/gaia/python/packaging/gen_scorecard.py). It reads `<run-dir>/scorecard.json` and keeps only the scenarios whose recorded provenance says the flagship (`agent_type: gaia`) actually ran them — a run can mix agents, so the harness's own pooled `summary.by_category` would fold other agents' scores into the flagship's number.

The release gate on `release_agent_gaia.yml` enforces **presence + no-regression only**, with no `--min-aggregate` or `--min-urgent-recall`: those come from email's thresholds manifest, and the flagship has no equivalent manifest and no second measurement yet to derive a bar from. An absolute bar is a follow-up.
...
4. **Regression check** (variance-aware, #1894): a regression is flagged only when `candidate < baseline − band`, where the band is the **wider** of `k·stdev` (`--regression-k`, default 1; stdev is on the [0,1] scale, scaled ×100 to match `aggregate.value`) and an absolute floor (`--regression-floor-pp`, default 1.5). Single-run cards record a stdev of `0.0`, so the floor is what gives them tolerance — without it the band would collapse to a strict `<` and trip on sub-noise wobble. Pass `--regression-floor-pp 0` for the strict behaviour.
