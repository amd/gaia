...
Lemonade reports zero usage on a streamed Anthropic reply (observed 2026-09), so
the Claude Code leg on an open-weights model needs `--meter` for a cost figure.
