...
Expectations live in `eval/tasks/expectations/<model>.<suite>.json`. They are tied to the model, the suite and the judge model, because a different judge grades on a different scale. An illustrative file:
...
- `--propose FILE` writes expectations measured from a run, with headroom for one noisy run: one task may flip, quality may drop 0.5, and tokens and steps may grow 35%. CI uploads that proposal with its scorecard. Commit it to switch the gate on for that model and suite.
