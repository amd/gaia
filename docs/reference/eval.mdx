...
## Code Benchmark: scoring by running the tests

`gaia eval code` asks the agent to change a real project and lets that project's
own test suite decide. There is no LLM judge — a task passes when its suite
passes, which makes the score reproducible and cheap to trust.

Like Session Replay, this drives a TUI that is **already running** with
`--control-port`, so the run is visible while it happens rather than hidden in a
subprocess.

```bash
# Terminal 1 — the TUI under test
gaia-tui run gaia --control-port 8817
...
| `--out` | Output directory (default: `eval/results/code-<timestamp>`) |
| `--workspace` | Where to materialize the task projects (default: a temp directory) |

Each task ships a project plus its test suite; the scorecard reports how many
suites went green, so a regression names the task that broke rather than a
number that drifted.
