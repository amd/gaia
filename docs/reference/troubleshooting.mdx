...
<AccordionGroup>
  ...
  <Accordion title="Module not found errors" icon="file-circle-xmark">
    ...
    from gaia import Agent  # Also supported
    ```
  </Accordion>

  <Accordion title="Tools not registering" icon="wrench">
    Define instance-bound tools in `_register_tools()` and call mixin registration
    methods there. A module-level `@tool` also registers when its definition runs,
    but it does not bind to your agent's state. Registered tools are dispatched by
    the harness, not automatically exposed as methods on the agent. See
    [Python agent troubleshooting](/sdk/troubleshooting#tools-are-missing-or-call-the-wrong-agent).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Agent processing failed" icon="triangle-exclamation">
    Prepare the backend and model for the configured device profile:
    ```bash
    gaia init
    ```
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Dependency compatibility issues" icon="cubes">
    ...
    **Fix:**
    ```bash
    uv pip uninstall numpy pandas scikit-learn
