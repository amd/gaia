...
| HTTP client | `test_http_client.cpp` | `gaia::HttpClient` GET/POST/streaming, headers, timeouts, `HttpError` |
...
<Note>
  Three additional modules — `test_tui_markdown.cpp` (markdown renderer), `test_tui_transcript.cpp` (transcript model), and `test_tui_app.cpp` (event loop, streaming, confirmation modal) — are compiled into `tests_mock` only when `GAIA_BUILD_TUI=ON` (the default). They assert against real rendered frames via `ftxui::Screen::Create()` + `Render()` + `ToString()`, so a change that stops output reaching the screen fails the suite.
</Note>
