...
The entire agent is a single `.cpp` file with five sections. Let's walk through each one.
...
Diagnostic nodes (dark) gather data. Red nodes are destructive actions requiring confirmation. Blue is the Reanalyze path that clears history. Green nodes are conclusion displays where the user reads output.
...
Then update the system prompt's `## AVAILABLE TOOLS` section to reference the new tool so the LLM knows when to use it. Optionally, add an `ActionEntry` to `kActions[]` if you want a new numbered menu item (e.g., `[6] Startup Items`).
