...
The framework automatically appends registered tools to the system prompt — the new tool is available to the LLM immediately. Optionally, add an `ActionEntry` to `kActions[]` if you want a new numbered menu item (e.g., `[6] Startup Items`).
