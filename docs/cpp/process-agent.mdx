...
<Info>
  **Source Code:** [`cpp/examples/process_agent.cpp`](https://github.com/amd/gaia/blob/234-cua-system-health-check-workflow-cpp/cpp/examples/process_agent.cpp) — single-file, self-contained agent (~2,900 lines including 7 tools, action menu, and decision support).
</Info>
...
<video
  ...
  muted
  playsInline
  style={{ minHeight: '775px', objectFit: 'cover' }}
  src="https://assets.amd-gaia.ai/videos/process-analyzer-demo-01.webm"
/>
...
<Steps>
  <Step title="Build">
    <Tabs>
      ...
      <Tab title="Windows (Ninja)">
        ...
        1. **Ninja installed** — `winget install Ninja-build.Ninja` (then restart your terminal)
        2. **MSVC compiler in PATH** — Ninja does not locate `cl.exe` automatically the way the Visual Studio generator does. Open an [x64 Native Tools Command Prompt for VS 2022](https://learn.microsoft.com/en-us/visualstudio/ide/reference/command-prompt-powershell) — this puts `cl.exe` in PATH automatically. Do not use a plain PowerShell or CMD window.
        </Warning>
        ...
        <Note>
        If CMake reports a generator mismatch, a stale `build\` directory from a previous run is causing the conflict. Delete it and reconfigure:
        ```powershell
        Remove-Item -Recurse -Force build
        cmake -B build -G Ninja -DCMAKE_BUILD_TYPE=Release
        ```
        ...
        ```
        Binary: `build\process_agent.exe`
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Run the agent">
    <Tabs>
      <Tab title="Windows (MSVC)">
        ```bat
        build\Release\process_agent.exe
        ```
      </Tab>
      <Tab title="Windows (Ninja)">
        ```bat
        build\process_agent.exe
        ```
      </Tab>
    </Tabs>
    ...
    | `6` | Toggle background monitor (default 5 min interval) |
...
static gaia::json getTopProcesses(int topN = 20) {
    ...
    // ... enumerate processes, measure memory via K32GetProcessMemoryInfo ...
    // ... group by exe name, accumulate total memory and instance count ...

    for (auto& g : sorted) {
        // ... enrich with version info (company, description) via GetFileVersionInfoW ...

        // Factual flags for LLM classification
        gaia::json flags = gaia::json::array();
        if (company.empty()) flags.push_back("unknown_company");
        if (lowerPath.find("\\Downloads\\") != std::string::npos)
...
Tools are registered with a name, description, callback, and typed parameter list. The framework automatically includes registered tools in the system prompt sent to the LLM.
...
<video
  ...
  src="https://assets.amd-gaia.ai/videos/process-analyzer-demo-02.webm"
/>
...
**Starting and stopping.** Monitor is a toggle on `[6]`. Press it once to start, again to stop. You can also specify an interval: `6 5` starts monitoring every 5 minutes. Typing `monitor` or `monitor N` at the prompt does the same thing. The default interval is 5 minutes.
