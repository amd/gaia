...
<Tabs>
  <Tab title="Windows">
    ...
    Download and run the installer (v2026.39.1):
    ...
    Or download directly from the [Lemonade v2026.39.1 release page](https://github.com/lemonade-sdk/lemonade/releases/tag/v2026.39.1).

    How the server starts depends on the install, and there is no
    cross-platform `serve` command — Lemonade removed the `lemonade-server`
    CLI in 10.7. Windows starts `LemonadeServer.exe` from the tray icon or the
    Start menu; Linux runs the `lemond` systemd unit; macOS installs a
    LaunchDaemon.
  ...
  <Tab title="Linux">
    ...
    After installation, start the `lemond` systemd unit:
    ```bash
...
| Git | any | `git --version` |
| [Lemonade Server](https://lemonade-server.ai) | 2026.39.1 | `lemonade --version` |
