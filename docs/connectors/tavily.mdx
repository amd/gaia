...
`npx` (pinned to `tavily-mcp@0.2.22`) and routes tool calls (`tavily_search`,
`tavily_extract`, `tavily_crawl`, `tavily_map`, `tavily_research`) through it, so
...
Once configured, grant **and** activate the `tavily_search` /
`tavily_extract` MCP tools for each agent that should use them. For MCP
