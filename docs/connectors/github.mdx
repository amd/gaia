...
1. Store the token in your OS keyring (single slot, distinct from any
   other connector).
2. Write `~/.gaia/mcp_servers.json` with a `$keyring:gaia.connections:mcp-github:GITHUB_PERSONAL_ACCESS_TOKEN`
...
### Private repos are missing, or you hit the anonymous rate limit

Connections saved before GAIA passed the token as
`GITHUB_PERSONAL_ACCESS_TOKEN` still hand it to the server under the old
name, so the server calls GitHub without it. Click **Disconnect** on the
tile and paste the token again, or run
`gaia connectors configure mcp-github --set GITHUB_PERSONAL_ACCESS_TOKEN=ghp_…`.
