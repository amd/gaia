...
<Note>
  **Prefer the CLI?** The whole flow works headless without opening the
  ...
  then `gaia connectors connect google`. See the [Email guide's retired-agent
  setup](/guides/email#setup) for the exact commands.
</Note>
...
<Note>
  **`--scopes` is a two-command path, and it does not union with the
  ...
  The [Email guide's retired-agent setup](/guides/email#setup) has the full
