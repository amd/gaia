...
- **Git** — `mcp-git`: `status`, `diff`, `log`, `show`, branch, and commit
  tools for a local repository. It runs through `uvx`, so install
  [uv](https://docs.astral.sh/uv/) and make sure `uvx` is on your `PATH`.
