...
<CodeGroup>
```bash Linux Runtime
docker run -dit --name gaia -e LEMONADE_BASE_URL=http://host.docker.internal:8080 amd/gaia:linux
docker exec -it gaia zsh
```

```bash Windows Runtime
docker run -dit --name gaia -e LEMONADE_BASE_URL=http://host.docker.internal:8080 amd/gaia:windows
docker exec -it gaia powershell
```

```bash Development (with Claude Code)
docker run -dit --name gaia-dev -e LEMONADE_BASE_URL=http://host.docker.internal:8080 -e GITHUB_TOKEN=ghp_xxx amd/gaia:dev
...
- **Environment config**: Set `LEMONADE_BASE_URL` at container creation to connect to your host's Lemonade Server
