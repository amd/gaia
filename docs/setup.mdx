---
title: "Setup"
description: "Install GAIA and its dependencies"
---

## System Requirements

| Requirement | Supported |
|-------------|-----------|
| **NPU** | AMD Ryzen AI 300-series |
| **GPU** | AMD Radeon RX 7000/9000-series, Radeon PRO W-series |
| **CPU** | Any x86_64 processor (slower, but works without dedicated AI hardware) |
| **OS** | Windows 11, Ubuntu 24.04+ |
| **RAM** | 16GB minimum, 64GB recommended |
| **Python** | 3.10 - 3.12 (uv installs automatically) |

<Tip>
  **Recommended Configuration:** The SDK has been primarily tested on a GMKtec NucBox EVO-X2 with AMD Ryzen AI MAX+ 395 w/ Radeon 8060S (or comparable) with 128GB RAM, using the Qwen3-Coder-30B-A3B-Instruct-GGUF LLM running on the iGPU.
</Tip>

<Note>
  For detailed hardware specs, see the [Lemonade supported configurations](https://github.com/lemonade-sdk/lemonade?tab=readme-ov-file#supported-configurations).
</Note>

---

## Install GAIA

Choose your platform and installation type, then follow the steps below.

<Tabs>
  <Tab title="Windows (PyPI)">
    Installs [amd-gaia](https://pypi.org/project/amd-gaia/) from PyPI.

    ### Step 1: Install uv

    [uv](https://docs.astral.sh/uv/) is a fast Python package manager (10-100x faster than pip).

    Open **PowerShell** and run:
    ```powershell
    powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"
    ```

    <Tip>
      If `uv` is not recognized after install, restart PowerShell or run:
      ```powershell
      $env:Path = "$env:USERPROFILE\.local\bin;$env:Path"
      ```
    </Tip>

    Verify uv is installed:
    ```powershell
    uv --version
    ```

    ### Step 2: Create Project Directory

    ```powershell
    mkdir my-gaia-project
    ```
    ```powershell
    cd my-gaia-project
    ```

    ### Step 3: Create Virtual Environment

    ```powershell
    uv venv .venv --python 3.12
    ```

    <Note>uv will automatically download Python 3.12 if not already installed.</Note>

    ### Step 4: Activate the Environment

    ```powershell
    .\.venv\Scripts\Activate.ps1
    ```

    <Warning>
      If you see a script execution error, run this once:
      ```powershell
      Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
      ```
      Then retry the activation command.
    </Warning>

    You should see `(.venv)` in your terminal prompt when activated.

    ### Step 5: Install GAIA

    ```powershell
    uv pip install "amd-gaia[lemonade]"
    ```

    This installs GAIA and [Lemonade Server](https://lemonade-server.ai), which provides AMD-optimized LLM inference.

    <Tip>
      **Additional extras:** `uv pip install "amd-gaia[lemonade,talk,rag]"` for voice and document Q&A features.
    </Tip>

    <Note>
      **Already have Lemonade Server installed?** You can skip the `lemonade` extra: `uv pip install amd-gaia`
    </Note>

    ### Step 6: Verify Installation

    ```powershell
    gaia -v
    ```

    <Success>
      You're ready to build your first agent! Continue to the [Quickstart](/quickstart).
    </Success>
  </Tab>

  <Tab title="Linux (PyPI)">
    Installs [amd-gaia](https://pypi.org/project/amd-gaia/) from PyPI.

    ### Step 1: Install uv

    [uv](https://docs.astral.sh/uv/) is a fast Python package manager (10-100x faster than pip).

    ```bash
    curl -LsSf https://astral.sh/uv/install.sh | sh
    ```

    Verify uv is installed (restart terminal if needed):
    ```bash
    uv --version
    ```

    ### Step 2: Create Project Directory

    ```bash
    mkdir my-gaia-project
    ```
    ```bash
    cd my-gaia-project
    ```

    ### Step 3: Create Virtual Environment

    ```bash
    uv venv .venv --python 3.12
    ```

    <Note>uv will automatically download Python 3.12 if not already installed.</Note>

    ### Step 4: Activate the Environment

    ```bash
    source .venv/bin/activate
    ```

    You should see `(.venv)` in your terminal prompt when activated.

    ### Step 5: Install GAIA

    ```bash
    uv pip install "amd-gaia[lemonade]"
    ```

    This installs GAIA and [Lemonade Server](https://lemonade-server.ai), which provides AMD-optimized LLM inference.

    <Tip>
      **Additional extras:** `uv pip install "amd-gaia[lemonade,talk,rag]"` for voice and document Q&A features.
    </Tip>

    <Note>
      **Already have Lemonade Server installed?** You can skip the `lemonade` extra: `uv pip install amd-gaia`
    </Note>

    ### Step 6: Verify Installation

    ```bash
    gaia -v
    ```

    <Success>
      You're ready to build your first agent! Continue to the [Quickstart](/quickstart).
    </Success>
  </Tab>

  <Tab title="Windows (Clone)">
    For contributors. Clones [amd/gaia](https://github.com/amd/gaia) from GitHub.

    <Note>
      **Requires:** [Git for Windows](https://git-scm.com/download/win) must be installed.
    </Note>

    ### Step 1: Install uv

    [uv](https://docs.astral.sh/uv/) is a fast Python package manager (10-100x faster than pip).

    Open **PowerShell** and run:
    ```powershell
    powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"
    ```

    <Tip>
      If `uv` is not recognized after install, restart PowerShell or run:
      ```powershell
      $env:Path = "$env:USERPROFILE\.local\bin;$env:Path"
      ```
    </Tip>

    Verify uv is installed:
    ```powershell
    uv --version
    ```

    ### Step 2: Clone the Repository

    ```powershell
    git clone https://github.com/amd/gaia.git
    ```
    ```powershell
    cd gaia
    ```

    ### Step 3: Create Virtual Environment

    ```powershell
    uv venv .venv --python 3.12
    ```

    <Note>uv will automatically download Python 3.12 if not already installed.</Note>

    ### Step 4: Activate the Environment

    ```powershell
    .\.venv\Scripts\Activate.ps1
    ```

    <Warning>
      If you see a script execution error, run this once:
      ```powershell
      Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
      ```
      Then retry the activation command.
    </Warning>

    You should see `(.venv)` in your terminal prompt when activated.

    ### Step 5: Install in Editable Mode

    ```powershell
    uv pip install -e ".[dev,lemonade]"
    ```

    <Tip>
      **With all extras:** `uv pip install -e ".[dev,lemonade,talk,rag]"` for voice and document Q&A features.
    </Tip>

    <Note>
      **Already have Lemonade Server installed?** You can skip the `lemonade` extra: `uv pip install -e ".[dev]"`
    </Note>

    ### Step 6: Verify Installation

    ```powershell
    gaia -v
    ```

    <Note>
      Changes to GAIA source code will take effect immediately without reinstalling.
    </Note>

    <Success>
      You're ready to build your first agent! Continue to the [Quickstart](/quickstart).
    </Success>
  </Tab>

  <Tab title="Linux (Clone)">
    For contributors. Clones [amd/gaia](https://github.com/amd/gaia) from GitHub.

    <Note>
      **Requires:** Git must be installed (`sudo apt install git`).
    </Note>

    ### Step 1: Install uv

    [uv](https://docs.astral.sh/uv/) is a fast Python package manager (10-100x faster than pip).

    ```bash
    curl -LsSf https://astral.sh/uv/install.sh | sh
    ```

    Verify uv is installed (restart terminal if needed):
    ```bash
    uv --version
    ```

    ### Step 2: Clone the Repository

    ```bash
    git clone https://github.com/amd/gaia.git
    ```
    ```bash
    cd gaia
    ```

    ### Step 3: Create Virtual Environment

    ```bash
    uv venv .venv --python 3.12
    ```

    <Note>uv will automatically download Python 3.12 if not already installed.</Note>

    ### Step 4: Activate the Environment

    ```bash
    source .venv/bin/activate
    ```

    You should see `(.venv)` in your terminal prompt when activated.

    ### Step 5: Install in Editable Mode

    ```bash
    uv pip install -e ".[dev,lemonade]"
    ```

    <Tip>
      **With all extras:** `uv pip install -e ".[dev,lemonade,talk,rag]"` for voice and document Q&A features.
    </Tip>

    <Note>
      **Already have Lemonade Server installed?** You can skip the `lemonade` extra: `uv pip install -e ".[dev]"`
    </Note>

    ### Step 6: Verify Installation

    ```bash
    gaia -v
    ```

    <Note>
      Changes to GAIA source code will take effect immediately without reinstalling.
    </Note>

    <Success>
      You're ready to build your first agent! Continue to the [Quickstart](/quickstart).
    </Success>
  </Tab>
</Tabs>

<Tip>
  **Having issues?** Check the [Troubleshooting](/reference/troubleshooting) guide, [create an issue](https://github.com/amd/gaia/issues) on GitHub, or contact us at gaia@amd.com.
</Tip>

---

<small style="color: #666;">

**License**

Copyright(C) 2024-2025 Advanced Micro Devices, Inc. All rights reserved.

SPDX-License-Identifier: MIT

</small>
