...
<AccordionGroup>
  ...
  <Accordion title='"No models available"'>
    ...
    **Solutions**:
    - Confirm at least one agent is registered in `AGENT_MODELS` (`src/gaia/api/agent_registry.py`) — it's empty by default, so an empty picker is expected until you add one
