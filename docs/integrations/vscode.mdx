...
<Steps>
  ...
  <Step title="Start Lemonade Server">
    Prepare the LLM backend and configured model:
    ```bash
    gaia init
    ```
  </Step>
...
<Steps>
  <Step title="Start Lemonade Server (LLM Backend)">
    ...
    <Info>Wait for setup to complete before starting the API server.</Info>
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title='"No models available"'>
    ...
    **Solutions**:
    ...
    - Ensure Lemonade Server is running: `gaia init`
