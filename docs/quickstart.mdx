...
<Tabs>
  ...
  <Tab title="npm (One Command)">
    ...
    <Note>
      The `gaia-ui` npm command requires the Python GAIA backend (`pip install amd-gaia`) and Lemonade Server for LLM inference. Use `gaia-ui --serve` if running the backend separately.
    </Note>
  </Tab>
</Tabs>

<Tip>
  The desktop installer includes everything: Lemonade Server, models, and the chat interface. For document Q&A, drag PDFs or code files into the chat. See the [GAIA Chat Desktop guide](/guides/agent-ui) for all features.
</Tip>
