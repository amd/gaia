---
title: "Quickstart"
description: "Start chatting locally in minutes, or build your first AI agent"
icon: "rocket"
---

## Chat UI (Fastest)

**Just want to chat?** Get a privacy-first AI chat running locally in minutes -- no Python or development setup required.

<Tabs>
  <Tab title="Desktop Installer">
    <Steps>
      <Step title="Download and install">
        <Tabs>
          <Tab title="Windows">
            Download [`gaia-ui-setup.exe`](https://github.com/amd/gaia/releases) and double-click to install.
          </Tab>
          <Tab title="Ubuntu">
            Download [`gaia-ui-setup.deb`](https://github.com/amd/gaia/releases) and install:
            ```bash
      ...
      <Step title="Launch GAIA Chat">
        Open **GAIA Chat** from the Start Menu (Windows) or application menu (Ubuntu). On first launch, the app downloads models automatically. Start chatting -- your data never leaves your device.
      </Step>
    </Steps>
  </Tab>

  <Tab title="npm (One Command)">
    Already have Node.js? Install and run in one command:
    ...
    <Note>
      The `gaia-chat` npm command requires the Python GAIA backend (`pip install amd-gaia`) and Lemonade Server for LLM inference. Use `gaia-chat --serve` if running the backend separately.
    </Note>
  </Tab>
</Tabs>

<Tip>
  The desktop installer includes everything: Lemonade Server, models, and the chat interface. For document Q&A, drag PDFs or code files into the chat. See the [GAIA Chat Desktop guide](/guides/chat-ui) for all features.
</Tip>
