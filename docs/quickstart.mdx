...
<Tabs>
  <Tab title="Windows">
    ...
    <Tip>
      **Optional extras:** `uv pip install "amd-gaia[talk,rag]" --extra-index-url https://download.pytorch.org/whl/cpu` for voice and document Q&A features.
    </Tip>
    ...
    <Note>
      Use `--profile chat` for the full experience (~25GB) or `--profile minimal` for a quick start (~4GB).
  ...
  <Tab title="Windows (Dev)">
    ...
    <Tip>
      **With all extras:** `uv pip install -e ".[dev,talk,rag]" --extra-index-url https://download.pytorch.org/whl/cpu` for voice and document Q&A features.
    </Tip>
