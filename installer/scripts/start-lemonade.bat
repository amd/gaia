...
start /B "" ".venv\Scripts\lemonade-server.exe" serve --port %PORT% --ctx-size %CTX_SIZE% --no-tray > lemonade-server.log 2>&1
...
echo.
echo --- Pulling Model ---
".venv\Scripts\lemonade-server.exe" pull %MODEL_NAME%
if %ERRORLEVEL% neq 0 (
