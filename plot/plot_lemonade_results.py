...
def analyze_mmlu_accuracy(sort_order='original', show_labels=True):
    ...
    if show_labels:
        ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=2)  # Move legend to bottom
    output_folder = ensure_output_folder()
    plt.savefig(os.path.join(output_folder, 'mmlu_management_accuracy.png'), dpi=300, bbox_inches='tight', facecolor='black', edgecolor='none')
    plt.close()
...
def analyze_mmlu_accuracy_vs_size(show_labels=True):
    ...
    output_folder = ensure_output_folder()
    plt.savefig(os.path.join(output_folder, 'mmlu_accuracy_vs_size.png'), dpi=300, bbox_inches='tight', facecolor='black', edgecolor='none')
    plt.close()
...
def create_interactive_mmlu_accuracy_vs_size():
    ...
    fig.add_trace(go.Scatter(
        ...
        y=[accuracy for model, _, accuracy in results if not model.startswith('amd/')],
        mode='markers',
        name='Non-AMD Models',
    ...
    output_folder = ensure_output_folder()
    pio.write_html(fig, file=os.path.join(output_folder, 'mmlu_accuracy_vs_size_interactive.html'), auto_open=False)
...
def adjust_label_positions(positions, labels, min_gap=10, max_adjustment=50):
    ...
    for i in range(1, len(sorted_indices)):
        ...
        if adjusted_positions[curr_idx] - adjusted_positions[prev_idx] < min_gap:
            shift = min(min_gap - (adjusted_positions[curr_idx] - adjusted_positions[prev_idx]), max_adjustment)
            adjusted_positions[curr_idx] += shift

    return adjusted_positions

def plot_with_spaced_labels(data, x_key, y_key, title, xlabel, ylabel, filename, show_labels=True):
    plt.style.use('dark_background')
    ...
    if show_labels:
        ...
        bottom_legend = ax.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3)
    ...
    plt.subplots_adjust(bottom=0.2)  # Add space at the bottom for the legend
    output_folder = ensure_output_folder()
    plt.savefig(os.path.join(output_folder, filename), dpi=300, bbox_inches='tight', facecolor='black', edgecolor='none')
    plt.close()
...
def plot_input_tokens_vs_mean_tokens_per_second(show_labels=True):
    ...
    create_interactive_plot(
        ...
        'Input Tokens',
        'Mean Tokens per Second',
        os.path.join(output_folder, 'input_tokens_vs_mean_tokens_per_second_interactive.html')
    )
...
def plot_input_tokens_vs_time_to_first_token(show_labels=True):
    ...
    create_interactive_plot(
        ...
        'Input Tokens',
        'Time to First Token (seconds)',
        os.path.join(output_folder, 'input_tokens_vs_time_to_first_token_interactive.html')
    )
...
def plot_mmlu_accuracy_vs_tokens_per_second(show_labels=True):
    ...
    output_folder = ensure_output_folder()
    plt.savefig(os.path.join(output_folder, 'mmlu_accuracy_vs_tokens_per_second.png'), dpi=300, bbox_inches='tight', facecolor='black', edgecolor='none')
    plt.close()
...
def plot_mmlu_accuracy_vs_time_to_first_token(show_labels=True):
    ...
    for model, _ in MODEL_SIZES:
        if model in data:
            accuracy = data[model]['stats'].get('mmlu_management_accuracy')
            time_to_first_token = data[model]['stats'].get('seconds_to_first_token_2048')
    ...
    plt.subplots_adjust(bottom=0.3)
    output_folder = ensure_output_folder()
    plt.savefig(os.path.join(output_folder, 'mmlu_accuracy_vs_time_to_first_token.png'), dpi=300, bbox_inches='tight', facecolor='black', edgecolor='none')
    plt.close()

    print("Static plot for MMLU Accuracy vs Time to First Token generated successfully.")
...
def create_interactive_mmlu_accuracy_vs_tokens_per_second():
    ...
    output_folder = ensure_output_folder()
    pio.write_html(fig, file=os.path.join(output_folder, 'mmlu_accuracy_vs_tokens_per_second_interactive.html'), auto_open=False)

    print("Interactive plot for MMLU Accuracy vs Tokens per Second generated successfully.")
...
def create_interactive_mmlu_accuracy_vs_time_to_first_token():
    ...
    output_folder = ensure_output_folder()
    pio.write_html(fig, file=os.path.join(output_folder, 'mmlu_accuracy_vs_time_to_first_token_interactive.html'), auto_open=False)

    print("Interactive plot for MMLU Accuracy vs Time to First Token generated successfully.")
