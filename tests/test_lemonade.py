...
@pytest.mark.parametrize("model, backend, prompt", [
    (model, backend, prompt) for model, backend in MODELS_BACKEND for prompt in PROMPT_SIZES
])
...
@pytest.mark.parametrize("model, backend, prompt_path", [
    (model, backend, prompt_path) for model, backend in MODELS_BACKEND for prompt_path in PROMPTS
])
