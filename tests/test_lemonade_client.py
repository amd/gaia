...
class TestNormalizeSystemInfo:
    """Tests for LemonadeClient._normalize_system_info backward compatibility."""

    def test_v10_keys_get_legacy_aliases(self):
        """Lemonade v10 response (amd_npu, amd_igpu) gets npu/gpu aliases."""
        v10_response = {
            "OS Version": "Windows 11",
            "Processor": "AMD Ryzen AI 9 HX 375",
            "Physical Memory": "32.0 GB",
            "devices": {
                "cpu": {"name": "AMD Ryzen AI 9 HX 375", "available": True},
                "amd_igpu": {"name": "AMD Radeon 890M", "available": True},
                "amd_dgpu": [],
                "amd_npu": {"name": "XDNA2", "available": True},
            },
        }
        result = LemonadeClient._normalize_system_info(v10_response)
        devices = result["devices"]

        # Legacy aliases should be added
        assert "npu" in devices
        assert devices["npu"]["name"] == "XDNA2"
        assert devices["npu"]["available"] is True
        assert "gpu" in devices
        assert devices["gpu"]["name"] == "AMD Radeon 890M"

        # Original keys should still be present
        assert "amd_npu" in devices
        assert "amd_igpu" in devices

    def test_legacy_keys_get_v10_aliases(self):
        """Lemonade v9 response (npu, gpu) gets amd_npu/amd_igpu aliases."""
        v9_response = {
            "OS Version": "Linux",
            "Physical Memory": "64.0 GB",
            "devices": {
                "npu": {"name": "AMD Ryzen AI NPU", "available": True},
                "gpu": {"name": "AMD Radeon", "available": True},
            },
        }
        result = LemonadeClient._normalize_system_info(v9_response)
        devices = result["devices"]

        # v10 aliases should be added
        assert "amd_npu" in devices
        assert devices["amd_npu"]["name"] == "AMD Ryzen AI NPU"
        assert "amd_igpu" in devices
        assert devices["amd_igpu"]["name"] == "AMD Radeon"
    ...
    def test_no_devices_key(self):
        """Response without devices key is returned as-is."""
        info = {"OS Version": "Linux", "Physical Memory": "16.0 GB"}
        result = LemonadeClient._normalize_system_info(info)
        assert result == info

    def test_empty_devices(self):
        """Response with empty devices dict is returned as-is."""
        info = {"devices": {}}
        result = LemonadeClient._normalize_system_info(info)
        assert result["devices"] == {}

    def test_npu_not_available_v10(self):
        """NPU with available=False is still normalized."""
        info = {
            "devices": {
                "amd_npu": {"name": "XDNA2", "available": False},
            },
        }
        result = LemonadeClient._normalize_system_info(info)
        assert result["devices"]["npu"]["available"] is False

    def test_both_keys_present_no_overwrite(self):
        """When both legacy and v10 keys exist, neither is overwritten."""
        info = {
            "devices": {
                "npu": {"name": "legacy-npu", "available": True},
                "amd_npu": {"name": "v10-npu", "available": True},
                "gpu": {"name": "legacy-gpu", "available": True},
                "amd_igpu": {"name": "v10-igpu", "available": True},
            },
        }
        result = LemonadeClient._normalize_system_info(info)
        # Neither should be overwritten
        assert result["devices"]["npu"]["name"] == "legacy-npu"
        assert result["devices"]["amd_npu"]["name"] == "v10-npu"
        assert result["devices"]["gpu"]["name"] == "legacy-gpu"
        assert result["devices"]["amd_igpu"]["name"] == "v10-igpu"
