...
"""
T-X1: ``REQUIRED_CONNECTIONS`` discovery + agent integration smoke tests.
...
- **Custom-agent path**: an ``agent.py`` written under ``~/.gaia/agents/<id>/``
  is loaded via ``_load_python_agent``. Its ``REQUIRED_CONNECTIONS`` survive
...
Bridge tests live in ``tests/unit/connections/test_agent_bridge.py``; this
...
class TestAgentBaseClassDefault:
    """The base Agent class declares an empty REQUIRED_CONNECTIONS so any
    subclass that doesn't override it has a deterministic empty list."""

    def test_base_default_is_empty_list(self):
        assert Agent.REQUIRED_CONNECTIONS == []
        assert isinstance(Agent.REQUIRED_CONNECTIONS, list)
...
class TestCustomAgentPath:
    def test_required_connections_round_trip(self, tmp_path, monkeypatch):
        ...
        assert isinstance(cr, ConnectionRequirement)
