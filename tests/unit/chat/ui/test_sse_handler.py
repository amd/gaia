...
class TestStructuredRenderInjection:
    """Structured-render injection mechanism (HACK — see issue #1000).

    The ``_RENDER_TOOL_TO_LANG`` map is currently empty (#1653 rip-out removed
    the only entry, ``pre_scan_inbox``). These tests guard the injection
    infrastructure so future render tools can be added without re-introducing
    bugs.
    """

    def test_render_tool_map_is_empty_after_rip_out(self, handler):
        """``_RENDER_TOOL_TO_LANG`` has no entries after the #1653 rip-out.

        pre_scan_inbox was the only key; once the in-process email agent is
        gone, the map is empty until a new render tool is registered.
        """
        from gaia.ui.sse_handler import SSEOutputHandler

        assert SSEOutputHandler._RENDER_TOOL_TO_LANG == {}

    def test_empty_render_map_produces_no_fence(self, handler):
        """With an empty _RENDER_TOOL_TO_LANG, no fence is ever prepended."""
        handler.print_tool_usage("any_tool")
        _drain(handler)
        handler.pretty_print_json({"ok": True, "data": {"kind": "whatever"}}, title="R")
        _drain(handler)
        handler.print_final_answer("Plain answer.")
        events = _drain(handler)
        assert events[0]["content"] == "Plain answer."
        assert handler._pending_render_payloads == []

    def test_buffer_starts_empty(self, handler):
        """New handler has no pending render payloads."""
        assert handler._pending_render_payloads == []

    def test_buffer_cleared_after_final_answer(self, handler):
        """After print_final_answer drains the buffer, it is empty."""
        # Manually plant a payload to simulate a future render tool.
        handler._pending_render_payloads.append(("fake_lang", {"kind": "fake_lang"}))
        handler.print_final_answer("First turn.")
        _drain(handler)
        assert handler._pending_render_payloads == []

    def test_other_tool_results_do_not_inject_fence(self, handler):
        """Unknown tool names produce no fence injection."""
