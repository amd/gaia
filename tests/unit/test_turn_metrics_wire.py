...
class TestTimingHoldsUpOnTheUnhappyPaths:
    ...
    def test_a_nested_tool_call_is_timed_once(self, tmp_path):
        ...
        Timing both double-counts the inner one into tool_s."""
        recorder = self._recorder(tmp_path)
        agent = _bare_agent()
        ...
        def _impl(name, args):
            if name == "outer":
                time.sleep(0.02)
                return agent._execute_tool("inner", {})
            return {"status": "ok"}

        agent._execute_tool_impl = _impl
        agent._is_error_result = staticmethod(lambda r: False)

        agent._execute_tool("outer", {})
...
class TestTheRecordedModelIsTheOneUsed:
    """Under --use-claude the agent keeps its configured local model id and the
    SDK substitutes the Claude model at send time. A record built from the
    former names a model the turn never reached.
    """

    def test_the_sdk_effective_model_wins(self):
        agent = _bare_agent()
        agent.model_id = "Gemma-4-E4B-it-GGUF"
        agent.chat = SimpleNamespace(effective_model="claude-sonnet-5")
        assert agent._recorded_model_id() == "claude-sonnet-5"

    def test_the_local_id_stands_when_there_is_no_sdk(self):
        agent = _bare_agent()
        agent.model_id = "Gemma-4-E4B-it-GGUF"
        assert agent._recorded_model_id() == "Gemma-4-E4B-it-GGUF"
