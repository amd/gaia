...
"""
Tests for ``gaia.connections.errors``.

Acceptance: every error type subclasses ``ConnectionsError``, AuthRequiredError
...
class TestHierarchy:
    def test_every_error_is_a_connections_error(self):
        assert issubclass(AuthRequiredError, ConnectionsError)
        assert issubclass(ConnectionRevokedError, ConnectionsError)
        assert issubclass(ScopeMismatchError, ConnectionsError)
        assert issubclass(ConsentDeniedError, ConnectionsError)
        assert issubclass(FlowTimeoutError, ConnectionsError)
        assert issubclass(FlowInProgressError, ConnectionsError)
        assert issubclass(ConfigurationError, ConnectionsError)

    def test_connections_error_is_an_exception(self):
        assert issubclass(ConnectionsError, Exception)
