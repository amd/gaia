...
Autouse fixtures here apply to every test under ``tests/unit/connections/``
...
def _autouse_in_memory_keyring(in_memory_keyring):  # noqa: F811
    ...
    ``keyrings.alt`` fallback writes plaintext; ``gaia.connections.store``
...
def _autouse_reset_token_cache():
    ...
    """
    try:
        from gaia.connections import tokens
    except ImportError:
