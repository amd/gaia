...
- Backend allowlist: ``PlaintextKeyring`` and ``EncryptedKeyring`` are
  refused with ``ConnectionsError`` BEFORE any write happens (A4).
...
from gaia.connections.errors import AuthRequiredError, ConnectionsError
...
class TestKeyringFailureTranslated:
    def test_keyring_failure_raises_actionable_connections_error(self):
        ...
        # must catch that and surface a ConnectionsError naming what
