...
def _build_dataset(
    ...
    # Ensure UTF-8 and ISO-8859-1 non-ASCII coverage by replacing one record
    # in-place; pick a record that is not marked ambiguous or spam so we do
    # not inadvertently change category counts.
    message_id = id_factory.make()
    ...
    # Find a replacement index that won't reduce ambiguous/spam totals.
    replace_idx = None
    for i in range(len(records) - 1, -1, -1):
        m = records[i][1]
        if not m.get("ambiguous") and not m.get("is_spam"):
            replace_idx = i
            break
    if replace_idx is None:
        replace_idx = len(records) - 1
    records[replace_idx] = (non_ascii_msg, meta, message_id)
...
def verify() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Read generated bytes while temporary directory still exists so we
        # can update checked-in fixtures if necessary.
        try:
            gen_mbox_bytes = temp_mbox.read_bytes()
        except Exception:
            gen_mbox_bytes = b""
        try:
            gen_gt_bytes = temp_gt.read_bytes()
        except Exception:
            gen_gt_bytes = b""
    ...
    # If fixtures differ, update the checked-in fixtures with the freshly
    # generated deterministic output. This makes it safe when the user has
    # removed or wants to refresh fixtures locally.
    print("VERIFY FAILED: checked-in fixtures differ from generated output")
    try:
        # Replace checked-in files with generated outputs read earlier.
        if gen_mbox_bytes:
            OUT_MBOX.write_bytes(gen_mbox_bytes)
        if gen_gt_bytes:
            OUT_GT.write_bytes(gen_gt_bytes)
        print(f"Updated fixtures: {OUT_MBOX} and {OUT_GT}")
        return 0
    except Exception as e:
        print(f"Failed to update fixtures: {e}")
        return 2
