...
class TestStreamingGeneratorEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_stream_cleanup_after_setup_exception(self, db, session_id):
        """Setup failures after handler registration must still clean up SSE state."""
        from gaia.ui._chat_helpers import _active_sse_handlers, _stream_chat_response
        from gaia.ui.models import ChatRequest

        request = ChatRequest(
            session_id=session_id,
            message="Trigger setup failure",
            stream=True,
        )

        with patch.object(
            db, "get_messages", side_effect=RuntimeError("setup exploded")
        ):
            chunks = [
                chunk
        ...
        assert any("Connecting to LLM" in chunk for chunk in chunks)
        assert any('"type": "error"' in chunk for chunk in chunks)
        assert session_id not in _active_sse_handlers

    @pytest.mark.asyncio
    async def test_stream_cleanup_when_thread_start_fails(self, db, session_id):
        """Thread start failures must not leak the registered SSE handler."""
        from gaia.ui._chat_helpers import _active_sse_handlers, _stream_chat_response
        from gaia.ui.models import ChatRequest

        request = ChatRequest(
            session_id=session_id,
            message="Trigger thread start failure",
            stream=True,
        )

        with patch(
            "gaia.ui._chat_helpers.threading.Thread.start",
            side_effect=RuntimeError("thread start exploded"),
        ):
            chunks = [
                chunk
                async for chunk in _stream_chat_response(
