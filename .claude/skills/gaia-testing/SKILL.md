...
3. **Drive the real surface live, and capture it.** Drive the UI in a real browser (Chrome / Chromium via the browser tooling) and screenshot each step — a live browser representation, not a description — or exercise the CLI through a PTY and capture its output. Exercise the feature end-to-end; for input-type features (RAG document formats, etc.) exercise **every supported type named in the spec/release notes, not just the happy-path one**. Inject the planted facts.
