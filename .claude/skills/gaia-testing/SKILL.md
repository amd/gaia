...
- **GAIA's Agent UI MCP server — drive the agents without a browser.** `gaia mcp serve` wraps the Agent UI backend (`--backend http://localhost:4200`) and exposes its tools over MCP (Streamable HTTP on `:8766`, or `--stdio` for direct Claude Code / eval-runner integration). Use it to exercise an agent's tools programmatically — the cleanest surface for tool-level assertions, complementary to the browser (browser proves the UX/pixels; MCP proves the tools). `gaia mcp agent "<request>"` drives the MCP **orchestrator** agent against the **MCP bridge (`:8765`)** — not the `:4200` backend — so it needs the bridge running, not `gaia mcp serve`. `gaia eval agent` is the LLM-behaviour scorecard (run in Phase 4).
