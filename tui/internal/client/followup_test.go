...
func TestAnOlderPeerIsNeverSentAFollowUp(t *testing.T) {
	f := newFakeRelay(t)
	// One minor below the floor: the peer serves tool_decision and bypass but
	// not followup, so a gate that rounded down to "recent enough" would pass.
	f.contractVersion = "2.14"
	c, ch, stop := startedRun(t, f)

	if c.FollowUpSupported() {
		t.Error("a 2.14 peer was reported as taking mid-turn input")
	}
	err := c.SendFollowUp(context.Background(), "only the unread ones")
	if err == nil {
	...
	for _, want := range []string{"2.15", "gaia hub", "queued"} {
