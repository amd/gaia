...
func (m ChatModel) turnMetricsBlock(msg *Message) string {
	...
	// The model the turn's requests actually went to. Worth its own line: a
	// remote session keeps a local model id in config, so this is the only
	// place the two can be seen to disagree.
	if t.Model != "" {
		lines = append(lines, "model id "+t.Model)
	}
